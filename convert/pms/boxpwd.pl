...
# pmsconv.pl の出力するパスワード単語リスト（pl_pwdlist.dat）を参照する。
#
# GameFreak taya
#
# 08.04.17	プラチナ仕様に変更
...
my $LISTFILE_NAME = "pl_pwdlist.dat";	#新単語リスト
#my $LISTFILE_NAME = "pwdlist.dat";		#旧単語リスト


#A値にいれる固定値
my $BOXPWD_MAGIC_NUMBER = 6;

#壁紙ナンバー最大値
my $EX_WALLPAPER_MAX = 8;
...
sub main {
	...
	if( ($id > 65535) || ($wp_num >= $EX_WALLPAPER_MAX) )
#	if( ($id > 65535) || ($wp_num > 255) )
	...
	#D値E値はA,B,C値から生成
	my $num_ed = (($wp_num+$id_hi)*$id_lo) & 255;
	my $rand1 = ($num_ed>>4) & 0xf;
	my $rand2 = $num_ed & 0xf;
