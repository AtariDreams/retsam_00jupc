	...
	case 0:
		{
			...
			if (pos != 0xFF){
				...
				CATS_ObjectAffineSetCap(wk->cap, CLACT_AFFINE_DOUBLE);
				WazaTool_InitMoveOneSync(&wk->cmo, 0, +FX_GET_ROTA_NUM(ROT_266), WAIT_266);
	...
	case 1:
		{
			if (WazaTool_CalcMoveOne(&wk->cmo) == TRUE){
				...
				if (wk->cnt > 5){
					...
					switch(wk->cnt){
					case 1:
						WazaTool_InitMoveOneSync(&wk->cmo, +FX_GET_ROTA_NUM(ROT_266), -FX_GET_ROTA_NUM(ROT_266), WAIT_266);
						break;
					case 2:
						WazaTool_InitMoveOneSync(&wk->cmo, -FX_GET_ROTA_NUM(ROT_266), +FX_GET_ROTA_NUM(ROT_266), WAIT_266);
						break;
					...
					case 4:
						...
						break;
					case 5:
						WazaTool_InitMoveOneSync(&wk->cmo, +FX_GET_ROTA_NUM(ROT_266), 0, WAIT_266 / 2);
						break;
					}
				}
			}
