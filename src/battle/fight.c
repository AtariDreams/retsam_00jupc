...
//#define MADOKA_DEBUG					//オーバーレイバグの検証
#ifdef MADOKA_DEBUG					//オーバーレイバグの検証
typedef struct{
	int					cb_pos;
	u8					command_buffer[64];
}DEBUG_COMMAND_BUFFER;
extern	DEBUG_COMMAND_BUFFER *dcb_p;
#endif
#endif
...
#define	BATTLE_CELL_TRANS_MAX	(4)		///<戦闘で使用するCellTransのMAX
...
static	const	u32	BattleServerVersion=BATTLE_SERVER_VERSION;		//戦闘サーババージョン
													//履歴
													//0x100　国内版
													//0x110　北米版
													//0x120　欧州版
...
BOOL BattleSystemProc(PROC *proc,int * seq)
{
	...
	case SEQ_INIT:
		...
		if((bp->fight_type&FIGHT_TYPE_SIO)&&
		  ((bp->battle_status_flag&BATTLE_STATUS_FLAG_REC_BATTLE)==0)){
			*seq = SEQ_SIO_INIT;
		}
		else{
	...
	case SEQ_FIGHT_MAIN:
		...
		if(FightMain(proc)==1){
			...
			if(debug_battle_work&DBW_RESTART){
				debug_battle_work&=(DBW_RESTART^0xffffffff);
				*seq = SEQ_FIGHT_LIST_DEMO_INIT;
				Snd_BgmPlay(Snd_NowBgmNoGet());
			}
			else{
	...
	case SEQ_WIN_LOSE_MAIN:
		...
		if(FightWinLoseDemoMain(proc)==TRUE){
			Overlay_UnloadID(FS_OVERLAY_ID(vs_demo));
			sys_DeleteHeap(HEAPID_BATTLE);
		#if 0	//プラチナではVS画面でするようにした 2008.04.24(木) matsuda
...
{		
	...
	//場面によってロードするプログラムが違うのでフラグによって分岐
	if(bw->overlay_flag==FIGHT_OVERLAY_AI_TO_SKILL){
		Overlay_Load(FS_OVERLAY_ID(bc_common2), OVERLAY_LOAD_NOT_SYNCHRONIZE);
	}
	else{
		Overlay_Load(FS_OVERLAY_ID(battle_ai), OVERLAY_LOAD_NOT_SYNCHRONIZE);
	}
...
//	CATS_ResourceManagerInit(bw->csp,bw->crp,&BattleResourceList);
	...
	BINPUT_StockBallActorResourceLoad(hdl_obj, bw->bip);
 ...
 *	戦闘オーバーレイ切り替え
 *
 *	WifiとAIを共存させるためにDPではWifi領域に置いていたAIルーチンをオーバーレイで戦闘領域に置く
 
 * @param	bw		戦闘システムワークのポインタ
 * @param	flag	オーバーレイ切り替えフラグ
 */
//============================================================================================
void	FightOverlaySwitch(BATTLE_WORK *bw,int flag)
{
	GF_ASSERT(bw->overlay_flag!=flag);

	bw->overlay_flag=flag;

	if(flag==FIGHT_OVERLAY_AI_TO_SKILL){
		Overlay_UnloadID(FS_OVERLAY_ID(battle_ai));
		Overlay_Load(FS_OVERLAY_ID(bc_common2), OVERLAY_LOAD_NOT_SYNCHRONIZE);
	}
	else{
...
static	const	int	EncountFirstColor[][3]={
	...
	{0x7fff,0x7fff,0x7fff},						//BG_ID_BIG_FOUR_A		//四天王A プラチナから追加
	{0x7fff,0x7fff,0x7fff},									//BG_ID_BIG_FOUR_B		//四天王B
	{0x7fff,0x7fff,0x7fff},									//BG_ID_BIG_FOUR_C		//四天王C
	{0x7fff,0x7fff,0x7fff},									//BG_ID_BIG_FOUR_D		//四天王D
	{0x7fff,0x7fff,0x7fff},									//BG_ID_CHAMPION		//チャンピオン
	{0x7fff,0x7fff,0x7fff},									//BG_ID_HAKAI			//破れた世界
	{0x7fff,0x7fff,0x7fff},									//BG_ID_TOWER		//バトルタワー
	{0x7fff,0x7fff,0x7fff},									//BG_ID_FACTORY	//バトルファクトリー
	{0x7fff,0x7fff,0x7fff},									//BG_ID_ROULETTE	//バトルルーレット
	{0x7fff,0x7fff,0x7fff},									//BG_ID_CASTLE		//バトルキャッスル
	{0x7fff,0x7fff,0x7fff},									//BG_ID_STAGE		//バトルステージ
};
...
static	void	FightInit(PROC *proc)
{
	...
	{		
		...
		bw->bip=BINPUT_SystemInit(hdl_bg, hdl_obj, bw,BattleWorkMySexGet(bw,BattleWorkCommIDGet(bw)), bw->battle_cursor);
	...
	if((bw->fight_type&FIGHT_TYPE_SIO)&&
	  ((bw->battle_status_flag&BATTLE_STATUS_FLAG_REC_BATTLE)==0)){
		...
		if(bw->server_flag){
			...
			ScioBattleRecvData(bw,SCIO_CLIENT);
		}
		for(i=0;i<bw->client_set_max;i++){
			ClientMain(bw,bw->client_param[i]);
			ScioBattleRecvData(bw,SCIO_SERVER);
		}
		if(bw->fight_end_flag==0){
			if(bw->server_flag){
				bw->fight_end_flag=ServerMain(bw,bw->server_param);
				ScioBattleRecvData(bw,SCIO_CLIENT);
			}
...
static	void	FightEnd(PROC *proc)
{
	...
	//対戦録画再生時に不正終了すると、オーバーレイ領域にメモリ開放に必要な関数がのらないときがあるので、
	//チェックしてオーバーレイを切り替える
	if(bw->overlay_flag!=FIGHT_OVERLAY_AI_TO_SKILL){
...
#ifdef PM_DEBUG
#ifndef DEBUG_PRINT_BATTLE
	if((debug_battle_work&DBW_RESTART)==0){
		for(i=0;i<CLIENT_MAX;i++){
...
static	void	FightSystemBoot(BATTLE_WORK *bw,BATTLE_PARAM *bp)
{
	...
	//ランダムの種を取得(ServerInitより前にコールする必要がある）
	bw->rand = bp->rand;
	bw->rand_temp = bp->rand;

#ifdef DEBUG_ONLY_FOR_sogabe
	OS_TPrintf("rand_temp:%08x\n",bw->rand_temp);
#endif
...

	{
		...
		for(i=0;i<CLIENT_MAX;i++){
			for(j=0;j<PokeParty_GetPokeCount(bp->poke_party[i]);j++){
				...
				sex=PokeSexGet(pp);
				PokeParaPut(pp,ID_PARA_sex,&sex);
			}
		}
	}
	...
	if(bw->fight_type&FIGHT_TYPE_SIO){
		CommCommandBattleInitialize(bw);
		{
			...
			if(bw->fight_type&FIGHT_TYPE_TOWER){
				...
				for(i=0;i<CLIENT_MAX;i++){
					fsbp.client_no=i;
					fsbp.client_type=ClientTypeTableSIOMulti[BattleWorkCommStandNoGet(bw,sio_id)][BattleWorkCommStandNoGet(bw,i)];
					bw->client_param[i]=ClientInit(bw,&fsbp);
				}
				bw->client_set_max=i;
				//手持ちポケモンセット
...
static void FightPinchSECheck(TCB_PTR tcb, void *work)
{
	...
	for(client_no=0;client_no<client_set_max;client_no++){
		...
		if( ((CT_CPClientBootGet(cp)==CLIENT_BOOT_NORMAL)&&
			((BattleWorkBattleStatusFlagGet(bw)&BATTLE_STATUS_FLAG_REC_BATTLE)==0))||
			((BattleWorkMineEnemyCheck(bw,client_no)==0)&&
			 (BattleWorkBattleStatusFlagGet(bw)&BATTLE_STATUS_FLAG_REC_BATTLE))){
...
static	BOOL	FightSioMain(PROC *proc)
{
	...
	case FSM_SEQ_FADE_INIT:
		WIPE_ResetBrightness(WIPE_DISP_MAIN);
		CommSetWifiBothNetAndVChat(TRUE); // 戦闘の通信を非同期から同期に変える
		bsw->seq_no++;
		break;
	...
	case FSM_SEQ_SEND_DATA_END:
		if(PaletteFadeCheck(bsw->pfd)==0){	//輝度変更フェード終了待ちして色々削除
			...
			CommSetWifiBothNetAndVChat(FALSE); // 戦闘の通信を同期から非同期に変える
		}
		break;
	}
...
static	BOOL	FightListDemoInit(PROC *proc)
{
	...
	//通信対戦でなければデモはなし、バトルタワーもなし、対戦録画再生もなし
...
static	BOOL	FightWinLoseDemoInit(PROC *proc)
{
	...
	//通信対戦でなければデモはなし、バトルタワーもなし、録画再生もなし
...
static	void	FightSioServerBootCheck(BATTLE_WORK *bw,BATTLE_PARAM *bp)
{
	...
	if(id[0]==sio_id){
		...
		if((bw->fight_type&FIGHT_TYPE_TOWER)==0){
			if(bw->fight_type&FIGHT_TYPE_MULTI){
				work=CommGetStandNo(sio_id);
				switch(CommGetStandNo(id[0])){
				case 0:
				case 2:
					if(work&1){
						bw->battle_status_flag|=BATTLE_STATUS_FLAG_REC_CHILD;
					}
					break;
				case 1:
				case 3:
...
//--------------------------------------------------------------
/**
 * @brief	戦闘での通信アイコン表示	2008.05.15 goto
 *
 * @param	none	
 *
 * @retval	none	
 ...
 */
//--------------------------------------------------------------
static	void FightWirelessIconSet( void )
{
	WirelessIconEasy();
