...
//#define	FRONTIER_LOOP_ACT				//フロンティアでループチェック
#endif
#endif PM_DEBUG
...
#ifdef FRONTIER_LOOP_ACT
	ee_loop_count++;
	OS_TPrintf("EncountEffect OK:%d回目\n",ee_loop_count);
#endif
...
void	CT_PokemonEncountSet(BATTLE_WORK *bw,CLIENT_PARAM *cp,POKEMON_ENCOUNT_PARAM *pep)
{
	...
	if(tpe->dir == PARA_FRONT 
		...
		//地面のスライドと合わせるのでXを基本にオフセットYを決める
		offset_y = (GROUND_ENEMY_X - ENCOUNT_X_TYPE_BB) / 2;	// /2 = 速度を半分にするのでオフセットも半分にする
		//Ground_EnableSet(tpe->gw, CATS_ENABLE_FALSE);					//地面非表示
		SoftSpriteParaSet(tpe->ss,SS_PARA_SHADOW_SIZE,SS_SHADOW_NONE);	//影非表示
		SoftSpriteParaSet(tpe->ss,SS_PARA_POS_X,APPEAR_X_TYPE_BB);		//Xは到着位置
		SoftSpriteParaSet(tpe->ss,SS_PARA_POS_Y,appear_y - offset_y);	//Yは頭上高く
	...
	Snd_PMVoiceDoubleFlagSet( 1 );			///< 2匹分の鳴き声
}
...
#endif
	default:
		Snd_PMVoiceDoubleFlagSet( 0 );			///< 2匹分の鳴き声
...
//============================================================================================
/**
 *	エンカウントポケモン動作シーケンス用TCB（ギラティナ特殊エンカウント専用）
 *
 * @param[in]	tcb		TCBシステムワーク
 * @param[in]	work	ワークエリアポインタ
 */
			...
			if ( BattleWorkFightTypeGet( tpa->bw ) & FIGHT_TYPE_MULTI ){					
				if ( ( BattleWorkBattleStatusFlagGet( tpa->bw ) & BATTLE_STATUS_FLAG_REC_CHILD )==0 )
				{
					if ( tpa->client_type == CLIENT_TYPE_A ){
						///< サーバー で CLIENT_TYPE_A の時
						OS_Printf( " OAM 落とした -- 0\n" );
				...
				if ( ( BattleWorkBattleStatusFlagGet( tpa->bw ) & BATTLE_STATUS_FLAG_REC_CHILD )==0 )
				{
					...
					{
						///< サーバー で CLIENT_TYPE_A の時
						OS_Printf( " OAM 落とした -- 1\n" );
						tpa->ods = OAM_DropAll_Init( tpa->bw, HEAPID_BATTLE );
					}
					else if ( tpa->client_type == CLIENT_TYPE_AA  )
					{
						///< TYPE_AA = 1vs1 なので、野生もトレーナーも落ちる
						OS_Printf( " OAM 落とした -- 2\n" );
						tpa->ods = OAM_DropAll_Init( tpa->bw, HEAPID_BATTLE );
					}
					else {
						OS_Printf( " OAM 落としてない\n" );
...
static	void	TCB_PokemonReturn(TCB_PTR tcb,void *work)
{
	...
	case 6:
		...
		if(SoftSpriteParaGet(tpr->ss,SS_PARA_AFF_X)<=AFF_APPEAR_X_S){
			...
			///	tpr->seq_no=8;		///< 戻るエフェクトが出るので終了待ちする
				tpr->seq_no=7;
			}
		}
		break;
...
static	void	TCB_TrainerEncount(TCB_PTR tcb,void *work)
{
	...
	switch(tte->seq_no){
	case SEQ_TE_INIT:
		//3つ目のアニメーションが存在するならば、それをエンカウント時のアニメーションとして再生する
...
//プラチナではWifiにAIが載ったので、この処理は必要ないはず
#if 0
#ifdef PM_DEBUG
...
static	void	TCB_CommandSelectAuto(TCB_PTR tcb,void *work)
{
	...
	//コマンドがバッグ（アイテムが使用できるFIGHT_TYPEは録画しない）
...
#ifdef PM_DEBUG
		///< 技エフェクトデバッグメニュー起動
		///< OVERLAY で AI_TO_SKILL を起動して、seq_no = 4にする → OVERLAYを戻すための仮シーケンス
		if (sys.trg & PAD_BUTTON_START){
			tws->seq_no = 4;
...
#ifdef PM_DEBUG
	///< このTCBに復帰したときに SKILL_TO_AIを実行するため
	///< このような気持ちの悪いつくりになってます・・・。
	case 4:
...
static	void	TCB_ItemSelect(TCB_PTR tcb,void *work)
{
	...
	case SEQ_ITEM_SEL_CONDRCV_EFFECT_MSG_WAIT:
		if(GF_MSG_PrintEndCheck(tis->msg_index)==0){
			if(--tis->wait==0){
				//アイテム使用エフェクトが出せるように切り替えたオーバーレイを元に戻す
...
static	const	u8	EncountEffectTable[]={
	...
	ECE_SPA_FLOOR_001,			//四天王地面A	プラチナから追加
	ECE_SPA_FLOOR_001,			//四天王地面B
	ECE_SPA_FLOOR_001,			//四天王地面C
	ECE_SPA_FLOOR_001,			//四天王地面D
	ECE_SPA_FLOOR_001,			//チャンピオン地面
	ECE_SPA_ON_THE_ICE_001,		//破れた世界
	ECE_SPA_FLOOR_001,			//バトルタワー
	ECE_SPA_FLOOR_001,			//バトルファクトリー
	ECE_SPA_FLOOR_001,			//バトルルーレット
	ECE_SPA_FLOOR_001,			//バトルキャッスル
	ECE_SPA_FLOOR_001,			//バトルステージ
...
static	const	int	EncountAfterColor[]={
	...
	0x7fff,		//BG_ID_BIG_FOUR_A		//四天王A	プラチナから追加
	0x7fff,		//BG_ID_BIG_FOUR_B		//四天王B
	0x7fff,		//BG_ID_BIG_FOUR_C		//四天王C
	0x7fff,		//BG_ID_BIG_FOUR_D		//四天王D
	0x7fff,		//BG_ID_BIG_CHAMPION	//チャンピオン
	0x7fff,		//BG_ID_HAKAI			//破れた世界
	0x7fff,		//BG_ID_TOWER			//バトルタワー
	0x7fff,		//BG_ID_FACTORY			//バトルファクトリー
	0x7fff,		//BG_ID_ROULETTE		//バトルルーレット
	0x7fff,		//BG_ID_CASTLE			//バトルキャッスル
	0x7fff,		//BG_ID_STAGE			//バトルステージ
};
...
//		if(tee->wait==20){
		...
		//サブ画面を黒から白へフェードさせる 2008.02.09(土) matsuda
...
//--------------------------------------------------------------
/**
 * @brief   トレーナーアクターの表示設定
 * @param   cp		クライアントワークへのポインタ
 ...
 * @param[in]	pos_x	初期位置X
 * @param[in]	pos_x	初期位置Y
 */
//============================================================================================
static	CATS_ACT_PTR	CT_TrainerCLACTResourceLoad(BATTLE_WORK *bw,int type,int trtype,int id,s16 pos_x,s16 pos_y)
{
...
//============================================================================================
/**
 *	トレーナーアニメに設定されているエフェクトの発動チェック
 *
 * @param[in]	tte		トレーナーエンカウントエフェクトワーク構造体へのポインタ
 * @param[in]	cap		トレーナーキャラCLACT構造体
 *
 * @retval	FALSE アニメーション継続　TRUE　アニメーション途中でもサーバに終了と伝える（先に進ませる）
 */
//============================================================================================
enum{
...
{
	if(BattleWorkFightTypeGet(bw)&FIGHT_TYPE_SIO){
		//ロムコードが０の場合は、クライアントがDPなので、トレーナータイプを変更する
