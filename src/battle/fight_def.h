...
struct battle_work
{
	...
	u8					overlay_flag;				///<bc_common2とbattle_aiの切り替わりを保存
	u16					comm_id;					///<自分の通信ID

	u32					rand;						///<戦闘専用乱数の種
	u32					rand_temp;					///<戦闘専用乱数の種（退避用）
	u16					rec_send_pos[CLIENT_MAX];	///<対戦録画用バッファの送信位置インデックスワーク
	u16					rec_recv_pos[CLIENT_MAX];	///<対戦録画用バッファの受信位置インデックスワーク
	u16					rec_play_pos[CLIENT_MAX];	///<対戦録画用バッファのインデックスワーク
	int					comm_stand_no[CLIENT_MAX];	///<通信対戦部屋での自分の立ち位置

	u32					battle_rec_stop_flag:1;		///<対戦録画再生中断フラグ
	u32										:31;

	TCB_PTR				pbsb;						///<対戦録画再生中断ボタンタスク
	u8					voice_waza_param[CLIENT_MAX];	///<ぺラップのおしゃべり使用時のパラメータ（録画再生時に使用）
};
