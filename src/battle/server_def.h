...
///<対戦録画用フラグ
#define	SELECT_COMMAND	(0x01)
#define	SELECT_WAZA		(0x02)
#define	SELECT_DIR		(0x04)
...
typedef struct{
	...
	u8					reshuffle_seq_no[CLIENT_MAX];
	u8					reshuffle_sel_mons[CLIENT_MAX];
...
struct server_param{
	...
	u8		rec_select_flag[CLIENT_MAX];				///<戦闘録画用フラグ（技選択、攻撃対象選択の有無を示す）

	int		client_working_count;						///<未実行のクライアントカウント

	u32		battle_progress_flag	:1;					///<戦闘進行フラグ
	u32								:31;
