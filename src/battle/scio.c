...
void	SCIO_SioWaitMessageWithRecData(BATTLE_WORK *bw,int send_client);
...
BOOL	SCIO_CommRecvData(BATTLE_WORK *bw,void *data);
...
void	ScioBattleRecvData(BATTLE_WORK *bw,int flag);
 ...
 * @brief   バトルコマンド受信TCB
 *
 * @param   bw		戦闘ワーク構造体へのポインタ
 * @param   flag	取得するバッファの振り分けフラグ
 *
 * @retval  none
 */
//--------------------------------------------------------------
void	ScioBattleRecvData(BATTLE_WORK *bw,int flag)
{
	...
	if(flag==SCIO_CLIENT){
		...
		src=BattleWorkSioSendBufGet(bw);
		read=BattleWorkSioSendReadGet(bw);
		write=BattleWorkSioSendWriteGet(bw);
		over=BattleWorkSioSendOverGet(bw);
	}
...
void	SCIO_EncountEffectSet(BATTLE_WORK *bw,int send_client)
{
	ENCOUNT_EFFECT_PARAM	eep;

	eep.command_code=CL_ENCOUNT_EFFECT;
	eep.rand_temp=BattleWorkRandTempGet(bw);

	SCIO_Interface(bw,SCIO_CLIENT,send_client,&eep,sizeof(ENCOUNT_EFFECT_PARAM));
}
 ...
 * @param[in]	bw				戦闘システムワークのポインタ
 * @param[in]	send_client		データを送るクライアント
 * @param[in]	sel_mons_no		繰り出しているポケモンが手持ちの何番目か
 * @param[in]	esc_can_flag	2vs2戦のときにタッチ画面のにげるボタンをにげる／もどるどちらにするかフラグ
 *
 */
//============================================================================================
void	SCIO_CommandSelectSet(BATTLE_WORK *bw,SERVER_PARAM *sp,int send_client,int sel_mons_no)
...
void	SCIO_PSPtoPPCopy(BATTLE_WORK *bw,SERVER_PARAM *sp,int send_client)
{
	...
	//プラチナで追加
	ptpcp.form_no=sp->psp[send_client].form_no;
	ptpcp.speabi=sp->psp[send_client].speabino;
	...
	//プラチナで追加（フォルムナンバーセットを指示するフラグのセット）
	if(sp->server_status_flag2&SERVER_STATUS_FLAG2_FORM_CHG){
		ptpcp.form_flag=1;
		sp->server_status_flag2&=SERVER_STATUS_FLAG2_FORM_CHG_OFF;
	}
	else{
		ptpcp.form_flag=0;
	}

	//プラチナで追加（再計算を指示するフラグのセット）
	if(sp->server_status_flag2&SERVER_STATUS_FLAG2_POKEPARA_CALC){
		ptpcp.calc_flag=1;
		ptpcp.form_flag=1;
		sp->server_status_flag2&=SERVER_STATUS_FLAG2_POKEPARA_CALC_OFF;
	}
	else{
		ptpcp.calc_flag=0;
	}
...
void	SCIO_SioWaitMessageWithRecData(BATTLE_WORK *bw,int send_client)
{
	...
	wrd.command_code=CL_SIO_WAIT_MESSAGE;
	wrd.size=0;

	//通信対戦の時のみ有効
	if((fight_type&FIGHT_TYPE_SIO)&&
	   (BattleRec_DataExistCheck()==TRUE)&&
	  ((bw->battle_status_flag&BATTLE_STATUS_FLAG_REC_BATTLE)==0)){
		wrd.size=BattleWorkBattleRecordSendDataMake(bw,&wrd.buf[0]);
		GF_ASSERT(wrd.size<REC_DATA_SEND_SIZE);
		SCIO_Interface(bw,SCIO_CLIENT,send_client,&wrd,sizeof(WITH_REC_DATA));
	}
}
...
void	SCIO_EscapeMessage(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	if((fight_type&FIGHT_TYPE_SIO)&&
	   (BattleRec_DataExistCheck()==TRUE)&&
	  ((bw->battle_status_flag&BATTLE_STATUS_FLAG_REC_BATTLE)==0)){
		emp.size=BattleWorkBattleRecordSendDataMake(bw,&emp.buf[0]);
		GF_ASSERT(emp.size<REC_DATA_SEND_SIZE);
	}
...
void	SCIO_GiveUpMessage(BATTLE_WORK *bw)
{
	GIVEUP_MESSAGE_PARAM	gmp;
	u32						fight_type=BattleWorkFightTypeGet(bw);

	gmp.command_code=CL_GIVEUP_MESSAGE;
	gmp.size=0;

	if((fight_type&FIGHT_TYPE_SIO)&&
	   (BattleRec_DataExistCheck()==TRUE)&&
	  ((bw->battle_status_flag&BATTLE_STATUS_FLAG_REC_BATTLE)==0)){
		gmp.size=BattleWorkBattleRecordSendDataMake(bw,&gmp.buf[0]);
		GF_ASSERT(gmp.size<REC_DATA_SEND_SIZE);
	}

	SCIO_Interface(bw,SCIO_CLIENT,CLIENT_NO_MINE,&gmp,sizeof(GIVEUP_MESSAGE_PARAM));
}
...
void	SCIO_WinLoseFlagSet(BATTLE_WORK *bw)
{
	...
	if((fight_type&FIGHT_TYPE_SIO)&&
	   (BattleRec_DataExistCheck()==TRUE)&&
	  ((bw->battle_status_flag&BATTLE_STATUS_FLAG_REC_BATTLE)==0)){
		wlsp.size=BattleWorkBattleRecordSendDataMake(bw,&wlsp.buf[0]);
		GF_ASSERT(wlsp.size<=REC_DATA_SEND_SIZE);
	}
 ...
 * @param[in]	data		受信したデータ
 *
 * @retval	TRUE:書き込み正常　FALSE:書き込めなかった
 */
//============================================================================================
BOOL	SCIO_CommRecvData(BATTLE_WORK *bw,void *data)
{
	...
	if(access==SCIO_SERVER){
		...
		if(bw->client_param[para]->client_buffer_flag==0){
			bw->client_param[para]->client_buffer_flag=1;
			for(i=0;i<size;i++){
				bw->client_param[para]->client_buffer[i]=src[i];
			}
		}
