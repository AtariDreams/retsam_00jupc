...
#define	VS_CLA_BALL00M_PX	( 24 - 4)	// VS画面：自分側前半３つのボール表示X座標（マルチ）
#define	VS_CLA_BALL00M_PY	( 140 + 6 )	// VS画面：自分側前半３つのボール表示Y座標（マルチ）
...
#define	VS_CLA_BALL03M_PY	(VS_CLA_BALL00M_PY + 16)	//( 164 )

// VS画面：敵側前半３つのボール表示X座標（マルチ）
#define	VS_CLA_BALL10M_PX	( 136 - VS_CLA_BALL_M_SX - 4)
...
#define	RESULT_WIN_CHG_PAL	( 0 )	// 勝敗画面：フェードで変更するパレット（勝ち）
#define	RESULT_LOSE_CHG_PAL	( 3 )	// 勝敗画面：フェードで変更するパレット（負け）
#define	RESULT_LOSE_LOAD_PAL	( 4 )	// 勝敗画面：フェードで変更するパレット（負け）
...
///勝敗画面で会話ウィンドウに被らないように全体的にずらす為のオフセット値Y
#define END_DISP_OFFSET_Y		(24)
///勝敗画面で会話ウィンドウに被らないように全体的にずらす為のオフセット値Y(左側)
#define END_DISP_OFFSET_LEFT_Y		(32)
///勝敗画面で会話ウィンドウに被らないように全体的にずらす為のオフセット値Y(右側)
#define END_DISP_OFFSET_RIGHT_Y		(16)

#define	RESULT_OBJ_PY	( 96 - END_DISP_OFFSET_Y )		// 勝敗OBJ表示Y座標
