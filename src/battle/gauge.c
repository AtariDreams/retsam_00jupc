...
static s32 GaugeOBJ(GAUGE_WORK *gauge, int req_data)
{
	...
	if(req_data == GAUGE_REQ_HP){
		...
		//EXPゲージは計算数値が大きいので、ドットのオフセットから何Syncで終わらせるかを決める
 ...
 * @param   add_dec		ゲージを加減算していく値 GaugeDottoがGaugeMax以下の場合は無視する
 *
...
static s32 GaugeProc(s32 MaxHP, s32 NowHP, s32 beHP, s32 *HP_Work, u8 GaugeMax, u16 add_dec)
{
	...
	if(MaxHP < GaugeMaxDot){			//↓少数部分もチェック
	...
	if(MaxHP < GaugeMaxDot){
		...
		if(beHP < 0){	//ゲージ回復
			*HP_Work += add_hp;
			ret = *HP_Work >> 8;
			if(ret >= endHP){	//少数を扱っているのでオーバーフローチェック
				*HP_Work = endHP << 8;
				ret = endHP;
			}
		}
 ...
 * @retval  表示するドット数
 *
 * gauge_chrに入る値は、0:空っぽのゲージ  1:1ドットのゲージ 2:2ドットのゲージ ・・・ 8:8ドット(満タン)のゲージ
 */
//--------------------------------------------------------------
static u8 PutGaugeProc(s32 MaxHP, s32 NowHP, s32 beHP, s32 *HP_Work, u8 *gauge_chr, u8 GaugeMax)
{
	...
	for(i = 0; i < GaugeMax; i++){	//空白ゲージで初期化
		gauge_chr[i] = 0;
	}
	...
	dot_ret = put_dot;
	//ドット計算では0でも、実際のHPが1以上あるなら1ドットのゲージを表示する
...
//--------------------------------------------------------------
/**
 * @brief   ゲージ計算前と計算後の最終的なドットの差分を求める
 *
 * @param   nowHP		現在HP(計算前のHP)
 * @param   beHP		ダメージ(-でダメージ、+なら回復)
 * @param   MaxHP		最大HP
