...
//	BINPUT_DefaultFrameSet(bgl);
	...
	//パレット読み込み
	{
		...
		if(BackGroundPaletteID[bg_id].base_nclr_index != BACK_GROUND_PALETTE_NONE){
			PaletteWorkSet_Arc(pfd, ARC_BATT_BG,
				BackGroundPaletteID[bg_id].base_nclr_index, HEAPID_BATTLE, FADE_SUB_BG, 0x20, 0);
		}
	...
	//背景パレット
	{
		...
		bip->background_pal_normal = sys_AllocMemory(HEAPID_BATTLE, 0x40);
		bip->background_pal_touch = sys_AllocMemory(HEAPID_BATTLE, 0x40);
		...
		arc_data = ArcUtil_PalDataGet(ARC_BATT_BG, ani_index, &pal_data, HEAPID_BATTLE);
		if(BattleWorkFightTypeGet(bw) & FIGHT_TYPE_TOWER){
