...
static	void	ServerBattleRecordStop(BATTLE_WORK *bw,SERVER_PARAM *sp);		///<対戦録画再生中断
...
static	BOOL	ServerDirSelectCheck(BATTLE_WORK *bw,SERVER_PARAM *sp,u8 client_no,u32 fight_type,int *range,int waza_pos,u32 *dir); ///<攻撃対象選択チェック
...
static	void	ServerWazaAIInit(BATTLE_WORK *bw,SERVER_PARAM *sp);				///<技AIの初期化
static	void	ServerBattleRecordCommand(BATTLE_WORK *bw,SERVER_PARAM *sp);	///<コマンド選択後の情報を録画

static	BOOL	ServerBattleRecordStopCheck(BATTLE_WORK *bw,SERVER_PARAM *sp);	///<対戦録画再生を中断するかチェック
...
//	ServerQueCheck,				///<サーバキューをチェックして終了を待つ
	...
	ServerBattleRecordStop,		///<対戦録画再生中断
...
void	*ServerInit(BATTLE_WORK *bw)
{
	...
	//Wifiの時は、AIがロードされていないので無視（PLでは、WifiにもAIが載るので関数を呼ぶ
...
static	void	ServerCommandSelect(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	for(client_no=0;client_no<client_set_max;client_no++){
		...
		case SCSSEQ_COMMAND_SELECT_INIT:
#ifdef FREEZE_BUG_CHECK
			OS_TPrintf("SCSSEQ_COMMAND_SELECT_INIT:%d\n",client_no);
			...
			//AIの処理が重いときにコマンド選択が横からスライド処理でウエイトがかかってしまう現象の回避処理
			if((CT_CPClientBootGet(BattleWorkClientParamGet(bw,client_no))==CLIENT_BOOT_AI)||(sp->total_turn)){
				...
				sp->com_seq_no[client_no]=SCSSEQ_COMMAND_SELECT2;
			}
			break;
		case SCSSEQ_COMMAND_SELECT2:
#ifdef FREEZE_BUG_CHECK
			...
			//AIの処理が重いときにコマンド選択が横からスライド処理でウエイトがかかってしまう現象の回避処理
			{
				...
				for(client_no_check=0;client_no_check<client_set_max;client_no_check++){
					if(client_no_check==client_no) continue;
					if(CT_CPClientBootGet(BattleWorkClientParamGet(bw,client_no_check))!=CLIENT_BOOT_AI) continue;
					if(sp->com_seq_no[client_no_check]!=SCSSEQ_COMMAND_SELECT_END) break;
				}
				if(client_no_check==client_set_max){
					SCIO_CommandSelectSet(bw,sp,client_no,sp->sel_mons_no[client_no]);
					sp->com_seq_no[client_no]=SCSSEQ_COMMAND_SELECT;
				}
				else{
...
#ifdef FREEZE_BUG_CHECK
			OS_TPrintf("SCSSEQ_COMMAND_SELECT:%d\n",client_no);
#endif FREEZE_BUG_CHECK
			if(ST_ServerBufferResGet(sp,client_no)){
				...
				if(fight_type&FIGHT_TYPE_POKE_PARK){
					...
					if(ST_ServerBufferResGet(sp,client_no)!=SELECT_CANCEL){
						sp->rec_select_flag[client_no]|=SELECT_COMMAND;
					}
					switch(ST_ServerBufferResGet(sp,client_no)){
			...
			if(ST_ServerBufferResGet(sp,client_no)==SELECT_CANCEL){
				...
				if((sp->server_buffer[client_no][0]-1)==AI_ENEMY_ESCAPE){
					...
					if(ST_ServerWazaNGCheck(bw,sp,client_no,sp->server_buffer[client_no][0]-1,&mp)==FALSE){
						//録画再生中にNGCheckに引っかかることはないので、不正データとみなす
						if(BattleWorkBattleStatusFlagGet(bw)&BATTLE_STATUS_FLAG_REC_BATTLE){
...
#if AFTER_MASTER_070117_FIX
				if(ServerDirSelectCheck(bw,sp,client_no,fight_type,&range,
		...
		case SCSSEQ_ESCAPE_SELECT_INIT:
			...
			//トレーナー戦は逃げれない
				...
				//録画再生中にNGCheckにひっかかるはずはないので、不正データとみなす
				if(BattleWorkBattleStatusFlagGet(bw)&BATTLE_STATUS_FLAG_REC_BATTLE){
					BattleWorkBattleRecStopFlagSet(bw,BI_SYSMSG_ERROR);
					ST_BattleRecordStopCheck(bw,BattleWorkServerParamGet(bw));
				}
	...
	do{
		...
		case SBA_AGI_CALC:
			for(client_no=0;client_no<CLIENT_MAX;client_no++){
				sp->agi_rand[client_no]=BattleWorkRandGet(bw);	///<素早さチェックに使用する乱数値
			}
			sp->sba_seq_no++;
			break;
		case SBA_END:
			...
			if(sp->psp[client_no].condition2&CONDITION2_SAWAGU){
				{
					...
					for(sleep_client=0;sleep_client<client_set_max;sleep_client++){
						if((sp->psp[sleep_client].condition&CONDITION_NEMURI)&&
						   ...
						   (sp->psp[sleep_client].hp)&&
#endif //AFTER_MASTER_070409_44_EUR_FIX
...
static	void	ServerSafariEsaCommand(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	sp->server_seq_no=SERVER_WAZA_SEQUENCE_NO;
	sp->next_server_seq_no=SERVER_WAZA_END_NO;
	sp->temp_work=BattleWorkRandGet(bw)%SAFARI_ATARI;
...
//	if(MyStatus_GetBadgeFlag(status,BADGE_ID_C06)){
	...
	i=((BattleWorkRandGet(bw)&0xff)*(sp->psp[sp->attack_client].level+level))>>8;
	...
	if(i<level){
		...
		if(sp->defence_client==NONE_CLIENT_NO){
			sp->client_act_work[sp->attack_client][ACT_PARA_CLIENT_NO]=ST_ServerDirClientGet(bw,sp,sp->attack_client);
		}
		else{
...
static	BOOL	ServerDefenceCheck(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//ためターンのときにNONE_CLIENT_NOのままスルーされるとフリーズする可能性があるので、
	//自分自身をdefenceにいれておく
	if((sp->defence_client==NONE_CLIENT_NO)&&
	   (ST_TameWazaCheck(sp,sp->waza_no_now)==TRUE)&&
	   (ret==FALSE)&&
	   (solar_beam==FALSE)&&
	   (ST_ServerSoubiEqpGet(sp,sp->attack_client)!=SOUBI_2TURNWAZAWO1TURN)&&
	  ((sp->psp[sp->attack_client].condition2&CONDITION2_KEEP)==0)){
		sp->defence_client=sp->attack_client;
	}
...
//		if((sp->field_condition&FIELD_CONDITION_HARE_ALL)&&(WT_WazaDataParaGet(waza_no,ID_WTD_battleeffect)==152)){
	...
	if((BattleWorkRandGet(bw)%100)+1>hit){
...
//		if((sp->field_condition&FIELD_CONDITION_ARARE_ALL)&&(WT_WazaDataParaGet(waza_no,ID_WTD_battleeffect)==260)){
	...
	//まきびし系は、チェックしない
		...
		if(
			...
			(
			((sp->server_status_flag&SERVER_STATUS_FLAG_SHADOW_DIVE_HIT)==0)&&
...
//別の修正方法に変更（AFTER_MASTER_070202_BT3_FIXの箇所）
#if 0
#if AFTER_MASTER_070123_BT4_FIX
	//まもるのチェック前に起きてるチェックをして、まもるチェックをスルーさせる
	//あくむチェック
	if(((sp->waza_status_flag&WAZA_STATUS_FLAG_NO_OUT)==0)&&
...
//	    (WT_WazaDataParaGet(sp->waza_no_now,ID_WTD_battleeffect)==92)){
	    (sp->AIWT.wtd[sp->waza_no_now].battleeffect==92)){
		sp->waza_status_flag|=WAZA_STATUS_FLAG_UMAKUKIMARAN;
	}
#endif //AFTER_MASTER_070123_BT4_FIX
#endif
...
static	void	ServerWazaOutAfterMessage(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	case SWOAM_NORMAL:
		...
		case SEQ_NORMAL_FORM_CHG_CHECK:
			sp->swoam_seq_no++;
			ST_ServerSequenceLoad(sp,ARC_SUB_SEQ,SUB_SEQ_FORM_CHG_CHECK);
			sp->next_server_seq_no=sp->server_seq_no;
			sp->server_seq_no=SERVER_WAZA_SEQUENCE_NO;
			return;
...
#if AFTER_MASTER_070213_BT_FIX
			  ((sp->waza_no_temp==WAZANO_TONBOGAERI)&&(sp->server_status_flag2&SERVER_STATUS_FLAG2_TONBOGAERI))||
			  ((sp->waza_no_temp==WAZANO_BATONTATTI)&&(sp->server_status_flag2&SERVER_STATUS_FLAG2_WAZA_OUT))){
#else AFTER_MASTER_070213_BT_FIX
			  ((sp->waza_no_temp==WAZANO_BATONTATTI)&&(sp->server_status_flag2&SERVER_STATUS_FLAG2_WAZA_OUT))){
#endif //AFTER_MASTER_070213_BT_FIX
				;
			}
			else{
...
static	void	ServerWinLose(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	else if(BattleWorkWinLoseGet(bw)==FIGHT_POKE_GET){
		sp->server_seq_no=SERVER_WAZA_SEQUENCE_NO;
		sp->next_server_seq_no=SERVER_FIGHT_END_NO;
	}
	else if(BattleWorkWinLoseGet(bw)==FIGHT_ESCAPE){
		sp->server_seq_no=SERVER_FIGHT_END_NO;
	}
#else //AFTER_MASTER_070409_60_EUR_FIX
 ...
 */
//============================================================================================
static	void	ServerBattleRecordStop(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
...
static	BOOL	ServerHirumaseruCheck(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	if(sp->defence_client!=NONE_CLIENT_NO){
		...
		if((eqp==SOUBI_HIRUMASERU)&&
		 ...
		 (((BattleWorkRandGet(bw)%100)<atk)||(sp->dbp.debug_flag&DEBUG_FLAG_SOUBI))&&
...
static	void	ServerWazaAIInit(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	int	i;
	int	cli;
	...
	if((fight_type&FIGHT_TYPE_TRAINER)&&((fight_type&FIGHT_TYPE_NO_ITEM_AI)==0)){
		for(cli=0;cli<CLIENT_MAX;cli++){
			if(cli&1){
				for(i=0;i<4;i++){
					item_no=BattleWorkTrainerUseItemGet(bw,cli,i);
					if(item_no!=0){
						sp->AIWT.AI_HAVEITEM[cli>>1][sp->AIWT.AI_ITEMCNT[cli>>1]]=item_no;
	...
	for(client_no=0;client_no<client_set_max;client_no++){
		...
		if(sp->client_act_work[client_no][ACT_PARA_ACT_NO]!=SERVER_WAZA_END_NO){
			...
			case SERVER_ITEM_COMMAND_NO:			///<道具使用処理
				...
				BattleWorkBattleRec(bw,client_no,data);
				break;
			case SERVER_POKEMON_COMMAND_NO:			///<ポケモン選択処理
				data=sp->client_act_work[client_no][ACT_PARA_WORK]+1;
				BattleWorkBattleRec(bw,client_no,data);
				break;
			case SERVER_ESCAPE_COMMAND_NO:			///<にげる動作処理
				//逃げるのYes/No選択結果を記録（Yesしかないけど）
				BattleWorkBattleRec(bw,client_no,TRUE);
				break;
			default:								///<録画するコマンドではない
				break;
			}
		}
	}
