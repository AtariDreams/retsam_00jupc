 ...
 * @param[in]	no_reshuffle_client	コマンド選択できないクライアントがビット格納されている
 *
 * @retval	COMSEL_ESCAPE:「にげる」 COMSEL_BACK:「もどる」
 */
//============================================================================================
u8		BattleWorkEscCanCheck(BATTLE_WORK *bw,int client_no,u8 no_reshuffle_client)
{
...
void	BattleWorkFormChgCheck(BATTLE_WORK *bw)
{
	...
	for(i=0;i<BattleWorkPokeCountGet(bw,CLIENT_NO_MINE);i++){
		...
		if((monsno==MONSNO_MINOMUTTI)&&(bw->appear_flag[CLIENT_NO_MINE]&No2Bit(i))){
			...
			case GROUND_ID_ROCK:
			case GROUND_ID_CAVE:
			case GROUND_ID_HAKAI:		//破れた世界(相手地面有り)
			case GROUND_ID_HAKAI_GIRA:	//破れた世界(ギラティナ戦用。相手地面無し)
				form=FORMNO_MINOMUTTI_SUNA;
				break;
			case GROUND_ID_FLOOR:
			case GROUND_ID_BRIDGE:
			case GROUND_ID_BIG_FOUR_A:	//四天王地面A
			case GROUND_ID_BIG_FOUR_B:	//四天王地面B
			case GROUND_ID_BIG_FOUR_C:	//四天王地面C
			case GROUND_ID_BIG_FOUR_D:	//四天王地面D
			case GROUND_ID_CHAMPION:	//チャンピオン地面
			case GROUND_ID_TOWER:		//バトルタワー
			case GROUND_ID_FACTORY:		//バトルファクトリー
			case GROUND_ID_ROULETTE:	//バトルルーレット
			case GROUND_ID_CASTLE:		//バトルキャッスル
			case GROUND_ID_STAGE:		//バトルステージ
...
//============================================================================================
/**
 *	戦闘専用ランダム取得関数（対戦録画のためにずれないように専用の関数を用意）
 *
 * @param[in]	bw			戦闘ワーク構造体
 */
//============================================================================================
...
//============================================================================================
/**
 *	戦闘専用ランダム格納関数（テンポラリワーク）
 *
 * @param[in]	bw			戦闘ワーク構造体
 * @param[in]	rand		格納する乱数値
 */
//============================================================================================
void	BattleWorkRandTempSet(BATTLE_WORK *bw,u32 rand)
{
 ...
 *
 * @param[in]	bw			戦闘システムワークの構造体ポインタ
 * @param[in]	client_no	再生を行うClientNo
 */
//============================================================================================
BOOL	BattleWorkBattlePlay(BATTLE_WORK *bw,int client_no,REC_DATA *data)
{
 ...
 *	通信で送る戦闘録画データを生成する
 *
 * @param	buf		生成したデータを格納するバッファのポインタ
 *
 * @return	生成したデータのサイズ
 *
 */
...
//---------------------------------------------------------------------------------------------
/**
 *	通信で受け取った戦闘録画データをバッファに展開する
 *
 * @param	buf		展開するデータが格納されたバッファのポインタ
 *
 */
//---------------------------------------------------------------------------------------------
void	BattleWorkBattleRecordRecvDataSet(BATTLE_WORK *bw,u16 size,u8 *buf)
...
#if 0
	//すべてがプラチナじゃなければ、何もしない（実際は、金銀を考慮するとDPが含まれていたら何もしない）
...
//============================================================================================
/**
 *	指定された通信IDの対戦部屋での立ち位置を取得
 *
 * @param[in]	bw		戦闘ワーク構造体
 * @param[in]	id		取得する通信ID
 *
 * @retval	立ち位置
 */
//============================================================================================
 ...
 *
 * @param[in]	bw			戦闘ワーク構造体
 * @param[in]	client_no	取得するトレーナーのClientNo
 * @param[in]	pos			取得するアイテム位置
 *
 * @retval	アイテムナンバー
 */
//============================================================================================
 ...
 *
 * @param[in]	bw			戦闘ワーク構造体
 * @param[in]	flag		中断メッセージの種類（BI_SYSMSG_STOP:通常 BI_SYSMSG_ERROR:不正データ中断）
 *
 * @retval	FALSE:中断できなかった　TRUE:中断した
 */
//============================================================================================
void	BattleWorkBattleRecStopFlagSet(BATTLE_WORK *bw,int flag)
{
