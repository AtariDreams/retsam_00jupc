...
BOOL		ST_ServerTenkiyaCheck(BATTLE_WORK *bw,SERVER_PARAM *sp,int *seq_no);
...
BOOL		ST_YubiwohuruNGCheck(BATTLE_WORK *bw,SERVER_PARAM *sp,int client_no,u16 waza_no);
...
void	ST_PokemonParamGet(BATTLE_WORK *bw,SERVER_PARAM *sp,int client_no,int sel_mons_no)
{
	...
	//フォルムナンバーで取得するデータを分岐
	ZKN_GIRA_Init(sp->psp[client_no].form_no);
	//図鑑の高さ重さ情報ワークを確保
...
#else
	if(eqp1==SOUBI_ITIDODAKESENSEI){
		if(ST_ServerTokuseiGet(sp,cl1)==TOKUSYU_KUISINBOU){
			atk1/=2;
		}
		if(sp->psp[cl1].hp<=(sp->psp[cl1].hpmax/atk1)){
			sensei1=1;
			//技のプライオリティを見ない時は、効果エフェクトの発動はなし
			if(flag==0){
				sp->psp[cl1].wkw.once_agi_up=1;
			}
		}
	}
#endif
...
#else
	if(eqp2==SOUBI_ITIDODAKESENSEI){
		if(ST_ServerTokuseiGet(sp,cl2)==TOKUSYU_KUISINBOU){
			atk2/=2;
		}
		if(sp->psp[cl2].hp<=(sp->psp[cl2].hpmax/atk2)){
			sensei2=1;
			//技のプライオリティを見ない時は、効果エフェクトの発動はなし
			if(flag==0){
				sp->psp[cl2].wkw.once_agi_up=1;
			}
		}
	}
#endif
		...
		case SPAC_TRACE:
			{
				...
				for(i=0;i<client_set_max;i++){
					...
					if((sp->psp[client_no].trace_flag==0)&&
					   ...
					   (sp->psp[client_no].hp)&&
					   (sp->psp[client_no].item!=ITEM_HAKKINDAMA)&&
...
//										switch(WT_WazaDataParaGet(waza_no,ID_WTD_battleeffect)){
		...
		//特性てんきや
		case SPAC_TENKIYA:
			if(ST_ServerTenkiyaCheck(bw,sp,&seq_no)==TRUE){
...
BOOL	ST_ServerSinkuroCheck(BATTLE_WORK *bw,SERVER_PARAM *sp,int server_seq_no)
{
	...
	//防御側チェック
	if((sp->defence_client!=NONE_CLIENT_NO)&&
	   (ST_ServerTokuseiGet(sp,sp->defence_client)==TOKUSYU_SINKURO)&&
	...
	ret=ST_ServerTenkiyaCheck(bw,sp,&seq_no);
	if(ret==TRUE){
		ST_ServerSequenceLoad(sp,ARC_SUB_SEQ,seq_no);
		sp->next_server_seq_no=server_seq_no;
		sp->server_seq_no=SERVER_WAZA_SEQUENCE_NO;
	...
	//防御側チェック
	if((sp->defence_client!=NONE_CLIENT_NO)&&
	   (ST_ServerSoubiEqpGet(sp,sp->defence_client)==SOUBI_MEROMERODETEKIMOMEROMERO)&&
	   (sp->defence_client==sp->tsuika_client)&&
 ...
 *	@param[in]	bw		戦闘システムワーク構造体のポインタ
 *	@param[in]	sp		サーバパラメータ構造体のポインタ
 *	@param[out]	seq_no	起動するサブシーケンスナンバー
 *
...
BOOL	ST_ServerTenkiyaCheck(BATTLE_WORK *bw,SERVER_PARAM *sp,int *seq_no)
{
	...
	for(i=0;i<BattleWorkClientSetMaxGet(bw);i++){
		...
		if((sp->psp[sp->client_work].monsno==MONSNO_KIMAIRAN)&&
		   (sp->psp[sp->client_work].hp)&&
		   (sp->psp[sp->client_work].form_no==FORMNO_GIRATINA_ORIGIN)){
			//ギラティナのフォルムを戻すのは
			//へんしんしているギラティナ
			//破れた世界ではないのに、はっきんだまを所持していない
			if((sp->psp[sp->client_work].condition2&CONDITION2_HENSHIN)||
			 (((BattleWorkBattleStatusFlagGet(bw)&BATTLE_STATUS_FLAG_YABURETA_SEKAI)==0)&&
			   (sp->psp[sp->client_work].item!=ITEM_HAKKINDAMA))){
				if(sp->psp[sp->client_work].condition2&CONDITION2_HENSHIN){
					POKEMON_PARAM	*pp;
					int				defence;
					int				work;
					...
					if(BattleWorkFightTypeGet(bw)&FIGHT_TYPE_2vs2){
						defence=sp->client_act_work[sp->client_work][ACT_PARA_CLIENT_NO];
					}
					else{
						defence=sp->client_work^1;
					}
					PokeCopyPPtoPP(BattleWorkPokemonParamGet(bw,defence,sp->sel_mons_no[defence]),pp);
					work=0;
					PokeParaPut(pp,ID_PARA_item,&work);
					work=FORMNO_GIRATINA_NORMAL;
					PokeParaPut(pp,ID_PARA_form_no,&work);
					PokeParaGirathinaFormChange(pp);
					sp->psp[sp->client_work].pow=		PokeParaGet(pp,ID_PARA_pow,		0);
					sp->psp[sp->client_work].def=		PokeParaGet(pp,ID_PARA_def,		0);
					sp->psp[sp->client_work].agi=		PokeParaGet(pp,ID_PARA_agi,		0);
					sp->psp[sp->client_work].spepow=	PokeParaGet(pp,ID_PARA_spepow,	0);
					sp->psp[sp->client_work].spedef=	PokeParaGet(pp,ID_PARA_spedef,	0);
					sp->psp[sp->client_work].speabino=	PokeParaGet(pp,ID_PARA_speabino,0);
					sp->psp[sp->client_work].form_no=	FORMNO_GIRATINA_NORMAL;
					sp->server_status_flag2|=SERVER_STATUS_FLAG2_FORM_CHG;
					...
					*seq_no=SUB_SEQ_GIRATINA_FORM_CHG;
					ret=TRUE;
					break;
				}
			}
...
//	wdcp_d.atc=ItemParamGet(item,ITEM_PRM_ATTACK,HEAPID_BATTLE);
	...
	if((wdcp_a.eqp==SOUBI_DORAGONGHOSTUP)&&
	  ...
	  ((ST_ServerPokemonServerParamGet(sp,attack,ID_PSP_condition2,NULL)&CONDITION2_HENSHIN)==0)&&
...
#ifdef ASSERT_CHECK
	AssertCheck(client_no,61);
#endif
	i=0;

	//じゅうりょくとかいふくふうじもあわせてチェック
	if((ST_ServerJuuryokuCheck(bw,sp,client_no,waza_no)==TRUE)||
	   (ST_ServerHealblockCheck(bw,sp,client_no,waza_no)==TRUE)){
		return FALSE;
	}
...
void	ST_ServerPressurePPDecCheck(SERVER_PARAM *sp,int attack,int defence)
{
	if((defence!=NONE_CLIENT_NO)&&
	   (ST_ServerTokuseiGet(sp,defence)==TOKUSYU_PURESSYAA)&&
...
#ifdef ASSERT_CHECK
static	void	AssertCheck(int client_no,int num){
	GF_ASSERT_MSG(client_no!=NONE_CLIENT_NO,"num:%d\n",num);
