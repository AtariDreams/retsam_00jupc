...
static void BPL_BgGraphicSet( BPLIST_WORK * wk )
{
	...
	hdl  = ArchiveDataHandleOpen( ARC_BPLIST_GRA,  wk->dat->heap ); 
	
	ArcUtil_HDL_BgCharSet(
		hdl, NARC_b_plist_gra_b_plist_NCGR,
...
static int BPL_TPCheck( BPLIST_WORK * wk, const RECT_HIT_TBL * tbl )
{
	...
	if(BattleWorkBattleStatusFlagGet(wk->dat->bw)&BATTLE_STATUS_FLAG_COM_SELECT){
		return DebugFightCommandBufferGet();
	}
	ret = GF_TP_RectHitTrg( tbl );
