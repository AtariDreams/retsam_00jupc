...
#include "skill_intp.h"
//#define DEBUG_PRINT_BATTLE	///<戦闘用デバッグプリントを有効にする（メモリオーバーするので、有効にできない）
#endif

#define BATTLE_SPEED_UP			///<戦闘高速化実験フラグ
...
//プラチナから追加
static	BOOL	WS_WAIT_NO_SKIP(BATTLE_WORK *bw,SERVER_PARAM *sp);
static	BOOL	WS_WAZA_TYPE_CHECK(BATTLE_WORK *bw,SERVER_PARAM *sp);
static	BOOL	WS_PERSONAL_DATA_GET(BATTLE_WORK *bw,SERVER_PARAM *sp);
static	BOOL	WS_POKEPARA_RELOAD(BATTLE_WORK *bw,SERVER_PARAM *sp);
...
static	void	PokeExpCalc(POKEPARTY *ppt,int pos,int mons_no,int form_no);
...
BOOL	SkillIntp(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
#ifndef BATTLE_SPEED_UP
	return SkillIntpTable[sp->SkillSeqWork[sp->skill_seq_no]](bw,sp);
#else
	BOOL	ret;
	do{
		ret=SkillIntpTable[sp->SkillSeqWork[sp->skill_seq_no]](bw,sp);
	}while((sp->battle_progress_flag==0)&&((BattleWorkFightTypeGet(bw)&FIGHT_TYPE_SIO)==0));
...
static	BOOL	WS_DAMAGE_CALC(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	sp->damage=ST_DamageLossCalc(bw,sp,sp->damage);
	sp->damage*=-1;
...
#if AFTER_MASTER_070330_BT_FIX
		ST_ServerDefenceClientTokuseiCheck(bw,sp,sp->attack_client,sp->waza_work);
#endif //AFTER_MASTER_070330_BT_FIX
...
static	BOOL	WS_CRITICAL_CHECK(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	if((BattleWorkFightTypeGet(bw)&FIGHT_TYPE_GET_DEMO)||
	   (BattleWorkBattleStatusFlagGet(bw)&BATTLE_STATUS_FLAG_FIRST_BATTLE)){
		...
		sp->critical=ST_CriticalCheck(bw,sp,sp->attack_client,sp->defence_client,sp->critical_count,
...
static	BOOL	WS_POKEMON_LIST_WAIT(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	if(reshuffle_count==0){
		for(client_no=0;client_no<client_set_max;client_no++){
			if((sp->client_status[client_no]&CLIENT_STATUS_POKE_RESHUFFLE)&&(ST_ServerBufferResGet(sp,client_no))){
				//入れ替え情報を録画
				BattleWorkBattleRec(bw,client_no,sp->server_buffer[client_no][0]);
			}
		}
	...
	if(sp->renzoku_count_temp==0){
		if(para==0){
			...
			if(ST_ServerTokuseiGet(sp,sp->attack_client)==TOKUSYU_SUKIRURINKU){
				...
				if((para=BattleWorkRandGet(bw)&3)<2){
					para+=2;
				}
				else{
					para=(BattleWorkRandGet(bw)&3)+2;
				}
			}
		}
...
static	BOOL	WS_RANDOM_GET(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	sp->calc_work=(BattleWorkRandGet(bw)%range)+ofs;
...
static	BOOL	WS_OUMU_CHECK(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	if(sp->waza_no_oumu[sp->attack_client]){
		...
		if(fight_type&FIGHT_TYPE_2vs2){
			...
			if(waza_no){
				do{
					waza_no=sp->waza_no_oumu_hit[sp->attack_client][BattleWorkRandGet(bw)%4];
				}while(waza_no==0);
			}
		}
	}
...
//			hit=WT_WazaDataParaGet(sp->waza_no_now,ID_WTD_hitprobability)+
			...
			if(((BattleWorkRandGet(bw)%100)<hit)&&(sp->psp[sp->attack_client].level>=sp->psp[sp->defence_client].level)){
				hit=1;
			}
			else{
				hit=0;
...
static	BOOL	WS_YUBIWOHURU(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	while(1){
		...
		if((sp->dbp.debug_flag&DEBUG_FLAG_YUBIHURU_DEBUG_SET)&&(sp->dbp.yubihuru_waza[0])){
			...
			waza_no=(BattleWorkRandGet(bw)%WAZANO_MAX)+1;
		}
#else
		waza_no=(BattleWorkRandGet(bw)%WAZANO_MAX)+1;
#endif
		...
		if(ST_YubiwohuruNGCheck(bw,sp,sp->attack_client,waza_no)==FALSE){
			continue;
		}
		//別の技シーケンスへジャンプ
...
static	BOOL	WS_KANASIBARI(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	if((sp->psp[sp->defence_client].wkw.kanashibari_wazano==0)&&
	   ...
	   (sp->waza_no_old[sp->defence_client])){
		...
		sp->psp[sp->defence_client].wkw.kanashibari_count=BattleWorkRandGet(bw)%4+3;	//２～５（実際は３～６にする）
	}
	else{
		SkillSeqInc(sp,adrs);
	}
...
#if AFTER_MASTER_070221_BT_FIX
		sp->psp[sp->defence_client].wkw.encore_count=BattleWorkRandGet(bw)%5+2;		//２～６
#else AFTER_MASTER_070221_BT_FIX
		sp->psp[sp->defence_client].wkw.encore_count=BattleWorkRandGet(bw)%5+3;		//２～６（実際は、３～７）
#endif //AFTER_MASTER_070221_BT_FIX
	}
	else{
		sp->waza_status_flag|=WAZA_STATUS_FLAG_UMAKUKIMARAN;
		SkillSeqInc(sp,adrs);
		...
		if((ST_TameWazaCheck(sp,sp->waza_no_texture2[sp->attack_client]))&&
		   (sp->psp[sp->waza_no_texture2_client[sp->attack_client]].condition2&CONDITION2_KEEP)){
		...
		else{
			{
				...
				for(i=0;i<1000;i++){
					ST_ServerTypeCheckTableParaGet(bw,0xffff,&type_w,&type_p,&value);
				...
				i=0;
				while(ST_ServerTypeCheckTableParaGet(bw,i,&type_w,&type_p,&value)==TRUE){
					if((type_w==wazatype)&&(value<=5)&&
					   (ST_ServerPokemonServerParamGet(sp,sp->attack_client,ID_PSP_type1,NULL)!=type_p)&&
					   (ST_ServerPokemonServerParamGet(sp,sp->attack_client,ID_PSP_type2,NULL)!=type_p)){
					   sp->psp[sp->attack_client].type1=type_p;
...
static	BOOL	WS_DOROBOU(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//はっきんだまの場合は失敗する
	else if(sp->psp[sp->defence_client].item==ITEM_HAKKINDAMA){
		SkillSeqInc(sp,adrs1);
	}
	//先制発動している場合は失敗する
	else if((sp->psp[sp->defence_client].wkw.once_agi_up)||
			(sp->psp[sp->defence_client].wkw.sensei_flag)){
		SkillSeqInc(sp,adrs1);
	}
	else{
...
static	BOOL	WS_GUARD_SUCCESS_CHECK(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	if((GSC_Table[sp->psp[sp->attack_client].wkw.success_count]>=BattleWorkRandGet(bw))&&(flag)){
		//まもる、みきり
...
#else //AFTER_MASTER_070122_BT2_FIX
//============================================================================================
/**
 * @param[in]	bw	戦闘用ワーク構造体
 * @param[in]	sp	サーバ用パラメータワーク構造体
	...
	if(fight_type&FIGHT_TYPE_TRAINER){
		{
			...
			if(cnt<=cntmax){
				...
				if(ST_ServerHukitobasiCheck(bw,sp)){
					do{
						do{
							i=BattleWorkRandGet(bw)%rnd_max;
							i+=start;
		...
		if(ST_ServerHukitobasiCheck(bw,sp)==FALSE){
			SkillSeqInc(sp,adrs);
		}
	}
...
static	BOOL	WS_PRESENT(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	present=BattleWorkRandGet(bw)&0xff;
...
static	BOOL	WS_MAGNITUDE(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	if(sp->magnitude==0){
		sp->magnitude=BattleWorkRandGet(bw)%100;
		if(sp->magnitude<5){
			sp->damage_power=10;
			sp->magnitude=4;
		}
...
static	BOOL	WS_MIRAIYOCHI(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	if(sp->fcc.miraiyochi_count[sp->defence_client]==0){
		...
		sp->fcc.miraiyochi_damage[sp->defence_client]=ST_DamageLossCalc(bw,sp,damage);
...
static	BOOL	WS_NEKONOTE(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
			...
			if((PokeParaGet(pp,ID_PARA_monsno_egg,NULL)!=0)&&
			   (PokeParaGet(pp,ID_PARA_monsno_egg,NULL)!=MONSNO_TAMAGO)){
				for(j=0;j<WAZA_TEMOTI_MAX;j++){
					...
					if((ST_RendouWazaCheck(waza)==FALSE)&&(ST_YubiwohuruNGCheck(bw,sp,sp->attack_client,waza)==TRUE)){
	...
	if(waza_count){
		sp->waza_work=waza_no[BattleWorkRandGet(bw)%waza_count];
	}
	else{
		SkillSeqInc(sp,adrs);
	}
...
#if AFTER_MASTER_070409_30_EUR_FIX
		pos=sp->waza_no_pos[sp->attack_client];
		sp->psp[sp->attack_client].pp[pos]=0;
		sp->waza_work=sp->psp[sp->attack_client].waza[pos];
#else //AFTER_MASTER_070409_30_EUR_FIX
		sp->waza_work=sp->waza_no_temp;
		pos=ST_ServerWazaPosGet(&sp->psp[sp->attack_client],sp->waza_no_temp);
		sp->psp[sp->attack_client].pp[pos]=0;
#endif AFTER_MASTER_070409_30_EUR_FIX
			...
			if(waza_no){
				...
				if((sp->AIWT.wtd[waza_no].battleeffect==128)&&
				   (sp->psp[client_no].pp[pos])){
					...
					sp->damage_value=20;
//別の修正方法に変更
#if AFTER_MASTER_070320_BT2_EUR_FIX
//					sp->waza_no_temp=waza_no;
#endif //AFTER_MASTER_070320_BT2_EUR_FIX
					sp->waza_no_now=waza_no;
...
static	BOOL	WS_MANEKKO(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	if((ST_RendouWazaCheck(sp->waza_no_last)==FALSE)&&
	   (sp->waza_no_last)&&
	   (ST_YubiwohuruNGCheck(bw,sp,sp->attack_client,sp->waza_no_last)==TRUE)){
...
static	BOOL	WS_MONOHIROI(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	for(i=0;i<BattleWorkPokeCountGet(bw,CLIENT_NO_MINE);i++){
		...
		//ものひろいチェック
		if((speabi==TOKUSYU_MONOHIROI)&&(monsno!=0)&&(monsno!=MONSNO_TAMAGO)&&(itemno==0)&&((BattleWorkRandGet(bw)%10)==0)){
			rnd=BattleWorkRandGet(bw)%100;
		...
		if((speabi==TOKUSYU_MITUATUME)&&(monsno!=0)&&(monsno!=MONSNO_TAMAGO)&&(itemno==0)){
			...
			GF_ASSERT(j<10);
			if(BattleWorkRandGet(bw)%100<MitsuatsumeProb[j]){
				j=ITEM_AMAIMITU;
...
static	BOOL	WS_HOGOSYOKU(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	if(ground>GROUND_ID_END){
		ground=GROUND_ID_END;
	}
...
static	BOOL	WS_TSUIBAMU(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//先制発動している場合は失敗する
			  ...
			  (sp->psp[sp->defence_client].wkw.once_agi_up))||
			  (ST_ServerTsuibamuCheck(bw,sp,sp->defence_client)!=TRUE)){
		SkillSeqInc(sp,adrs2);
	}
...
static	BOOL	WS_YES_NO_SELECT_WAIT(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	OS_TPrintf("YES_NO_SELECT_WAIT adrs1:%d adrs2:%d\n",adrs1,adrs2);
		...
		//選択情報を録画
		BattleWorkBattleRec(bw,CLIENT_NO_MINE,res);
	}
	...
	if(sp->dbp.osyaberi_rank){
	...
	else{
		{
			...
			if((sp->psp[sp->attack_client].monsno==MONSNO_PERAPPU)&&
			   (sp->psp[sp->defence_client].hp)&&
			  ((sp->psp[sp->attack_client].condition2&CONDITION2_HENSHIN)==0)){
				if((BattleWorkBattleStatusFlagGet(bw)&BATTLE_STATUS_FLAG_REC_BATTLE)==0){
					param=Snd_PerapVoiceWazaParamGet(BattleWorkPerapVoiceGet(bw,sp->attack_client));
				}
				else{
					param=BattleWorkVoiceWazaParamGet(bw,sp->attack_client);
				}
#if DEBUG_ONLY_FOR_sogabe
				OS_TPrintf("voice_waza_param:%d\n",param);
...
//			    (fight_type==FIGHT_TYPE_BATTLE_TOWER_AI_MULTI)||
				...
				(BattleWorkClientTypeGet(bw,client_no)==CLIENT_TYPE_A)){
				;
			}
			else{
...
static	BOOL	WS_TUBOWOTUKU(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	if(cnt){
		sp->add_status_flag_direct=ADD_COND2_POWUP2+abirity[BattleWorkRandGet(bw)%cnt];
...
#else
	if((eqp==SOUBI_HPITIDETAERU)&&((BattleWorkRandGet(bw)%100)<atk)){
#endif
		flag=1;
	}
...
static	BOOL	WS_WAIT_NO_SKIP(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	if(wait>sp->wait_cnt){
		...
		sp->wait_cnt=0;
#ifdef DEBUG_PRINT_BATTLE
		OS_TPrintf("WAIT_NO_SKIP wait:%d\n",wait);
#endif
...
#ifdef DEBUG_PRINT_BATTLE
	OS_TPrintf("WAZA_TYPE_CHECK type:%d adrs:%d\n",type,adrs);
#endif

	if(sp->AIWT.wtd[sp->waza_no_now].wazatype==type){
		SkillSeqInc(sp,adrs);
	}
...
static	BOOL	WS_PERSONAL_DATA_GET(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//mons_noをロード
	mons_no=SkillSeqDataRead(sp);

	//form_noをロード
	form_no=SkillSeqDataRead(sp);
...
#ifdef DEBUG_PRINT_BATTLE
	OS_TPrintf("PERSONA_DATA_GET mons_no:%d form_no:%d para:%d\n",mons_no,form_no,para);
#endif

	form_no_p=WS_SrcPointerGet(bw,sp,form_no);

	sp->calc_work=PokeFormNoPersonalParaGet(mons_no,form_no_p[0],para);
...
#ifdef DEBUG_PRINT_BATTLE
	OS_TPrintf("POKEPARA_RELOAD side:%d\n",side);
#endif

	client_no=SideClientNoGet(bw,sp,side);

	ST_PokemonParamReload(bw,sp,client_no,sp->sel_mons_no[client_no]);
	...
	case SEQ_GE_INIT:
		{
			...
			if((PokeParaGet(pp,ID_PARA_hp,NULL))&&(PokeParaGet(pp,ID_PARA_level,NULL)!=100)){
				...
				PokeExpCalc(BattleWorkPokePartyGet(tsiw->bw,exp_client_no),
							...
							tsiw->sp->psp[tsiw->sp->kizetsu_client].form_no);
			}
			...
			if(get_exp_total){
				...
				tsiw->work[GE_MSG_WAIT]=MSG_WAIT/4;
				tsiw->seq_no++;
			}
			else{
 ...
 * @param[in]	form_no	もらえる努力値計算の元となるポケモンのフォルムナンバー
 */
//============================================================================================
static	void	PokeExpCalc(POKEPARTY *ppt,int pos,int mons_no,int form_no)
{
	...
	ppd=PokeFormNoPersonalPara_Open(mons_no,form_no,HEAPID_BATTLE);
		...
		if(PaletteFadeCheck(pfd)==0){
			{
				...
				Trainer_EnableSet(BattleWorkClientParamGet(tsiw->bw,CLIENT_NO_MINE),CATS_ENABLE_FALSE);
		...
		if(PaletteFadeCheck(pfd)==0){
			{
				...
				BattleWorkGaugeDelete(tsiw->bw);
				{
					int i;			
					CLIENT_PARAM* cp;
					for ( i = 0; i < BattleWorkClientSetMaxGet( tsiw->bw ); i++ ){
						cp = BattleWorkClientParamGet( tsiw->bw, i );
						if( cp->cap ){
		...
		if(ProcMain(tsiw->work_p[GP_NICKNAME_PROC])){
			{
				...
				if(np->cancel==0){
					...
					///< ニックネームをつけた回数
					BattleWorkIncRecord( tsiw->bw, RECID_NICKNAME );
				}
		...
		if(PaletteFadeCheck(pfd)==0){
			{
				...
				if(PokeParty_Add(poke_party,pp)==TRUE){
				...
				else{
					{
						...
						//転送するポケモンがギラティナで、はっきんだまを所持してなくて、
						//フォルムがオリジンのときは、アナザーにする
						if(PokeParaGirathinaFormChange(pp) != -1){
							FT_ZukanFlagSetGet(tsiw->bw,get_client_no);
						}
...
//サファリボールならエサとドロを投げた回数で捕獲率を変動
	...
	if(hokakuritsu>=255){
		...
		for(i=0;i<4;i++){
			if(BattleWorkRandGet(bw)>=hokakuritsu) break;
		}
...
static	int	SideClientNoGet(BATTLE_WORK *bw,SERVER_PARAM *sp,int side)
{
	...
	GF_ASSERT(client_no!=NONE_CLIENT_NO);
