...
struct _DEMO_TRADE_WORK {
	...
	u16				sendPokeNumber;
	u16				sendPokeFormNumber;
	u16				recvPokeNumber;
	u16				recvPokeFormNumber;
...
PROC_RESULT TradeDemoProc_Init( PROC * proc, int * seq )
{
	if( WIPE_SYS_EndCheck() )
	{
		...
		if(wk)
		{
			...
			wk->sendPokeFormNumber = PokePasoParaGet((POKEMON_PASO_PARAM*)(wk->param->sendPoke), ID_PARA_form_no, NULL);
			wk->recvPokeNumber = PokePasoParaGet((POKEMON_PASO_PARAM*)(wk->param->recvPoke), ID_PARA_monsno, NULL);
			wk->recvPokeFormNumber = PokePasoParaGet((POKEMON_PASO_PARAM*)(wk->param->recvPoke), ID_PARA_form_no, NULL);
...
static BOOL check_reverse_ok( POKEMON_PASO_PARAM* ppp )
{
	...
	int form_no = PokePasoParaGet( ppp, ID_PARA_form_no, NULL );
	return ( PokeFormNoPersonalParaGet( monsno, form_no, ID_PER_reverse ) == FALSE );
}
...
//		cwk->ctrlTask = TCB_Add( WavePalAnmCtrlTask, cwk, TASKPRI_WAVE_CTRL );
		cwk->ctrlTask = VWaitTCB_Add( WavePalAnmCtrlTask, cwk, TASKPRI_WAVE_CTRL );
		if( cwk->ctrlTask )
		{
...
u16 DemoTrade_GetSendPokeFormNumber( DEMO_TRADE_WORK* wk )
{
	return wk->sendPokeFormNumber;
}
u16 DemoTrade_GetRecvPokeNumber( DEMO_TRADE_WORK* wk )
{
	return wk->recvPokeNumber;
}
u16 DemoTrade_GetRecvPokeFormNumber( DEMO_TRADE_WORK* wk )
{
	return wk->recvPokeFormNumber;
}
