 ...
 * @param   msgMan		
 *
 * @retval  PL_ENDING_LIST_WORK*		
 */
//------------------------------------------------------------------
PL_ENDING_LIST_WORK* PlEnding_ListWork_Create( GF_BGL_INI* bgl, int startLine, int bgFrame, int palPos, MSGDATA_MANAGER* msgMan )
{
	PL_ENDING_LIST_WORK* wk = sys_AllocMemory( HEAPID_ENDING_DEMO, sizeof(PL_ENDING_LIST_WORK) );
	if( wk )
	{
		wk->bgl = bgl;
 ...
 */
//------------------------------------------------------------------
void PlEnding_ListWork_Delete( PL_ENDING_LIST_WORK* wk )
{
	if( wk->task )
	{
		TCB_Delete( wk->task );
 ...
 */
//------------------------------------------------------------------
BOOL PlEnding_ListWork_Scroll( PL_ENDING_LIST_WORK* wk, int height )
{
...
static void ListWorkTask( TCB_PTR tcb, void* wk_adrs )
{
	PL_ENDING_LIST_WORK* wk = wk_adrs;
...
//--------------------------------------------------------------
/**
 * @brief   「おわり」のstr_idを取得
 *
 * @retval  「おわり」のstr_id
 */
//--------------------------------------------------------------
int PlEnding_FinStrIDGet(void)
{
	return StaffListDataTable[NELEMS(StaffListDataTable) - 1].strID;
}
 ...
 */
//------------------------------------------------------------------
void PlEnding_ListWork_WriteEndStr( PL_ENDING_LIST_WORK* wk, BOOL fillFlag )
{
	GF_PRINTCOLOR  color;
	int p, xpos;
 ...
 */
//------------------------------------------------------------------
void PlEnding_ListWork_WriteCopyLights( PL_ENDING_LIST_WORK* wk )
{
	wk->listSeq = LISTSEQ_ALLOUT;
	wk->topLine = 0;
	wk->transReq = FALSE;
