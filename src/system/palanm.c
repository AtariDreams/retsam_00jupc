...
static void FadeReqSet(
				...
				s8 wait, u8 start_evy, u8 end_evy, u16 next_rgb )
{
	// waitがマイナスの時はvalueを大きくしてフェードを早くする
	if( wait < 0 ){
...
//	グローバル変数
//----------------------------------------------------------
u16 PaletteWork[0x200] = {};			//オリジナル　　BG&OBJパレットバックアップ
u16 PaletteWorkTrans[0x200] = {};		//転送用
PalAnmWorkData PalAnmWork[PAL_ANM_MAX] = {};	//パレットアニメ用ワーク
PlttFadeData FadeData = {};				//フェードアウト、イン用構造体
u32 PalAnmFadeBit = 0;	//パレットアニメ、フェードイン、アウトさせるパレット 各ビットがパレットNoに対応
...
{
	65535,			//id_no
	0, //*PalData		転送するカラーパレットアドレス
	PA_BEFORE,      //type			BEFORE:カラー加減算対象　　AFTER:カラー加減算非対象
	PA_BG0+PA_COL0, //pal_no		0〜511
	0,              //TransValue	col_noから いくつまでのパレットを対象とするか
	0,              //TransWaitDef	次のパレットアニメに移るまでのウェイト
	0,              //TransValueLoopDef	何回転送を繰り返すか
	PA_END,         //PalAnmSeq		アニメ終了後の処理
	0,             //LoopWaitDef
};
...
//		pal_no = クリアーする場所  PA_BG0+PA_COL5  PA_OBJ3+PA_COL0 ...等
//		size = バイトサイズ
//=========================================================================
...
<<<<<<< palanm.c
//=========================================================================
//	パレットアニメ		  ※メインループ内で呼んでください
// 戻り値: 0=フェードリクエストがかかってない、又はリクエストが終了した
//         1=フェード実行中
//         2=フェードリクエストはかかっているがウェイト中なので(フェードの)処理はしなかった
//      0xff=前回のリクエストデータがまだ転送されていない
//=========================================================================
u8 PaletteAnime(void)
{
	u8 temp;
...
//----------------------------------------------------------
//	PLTT_VRAMのデータを全てPaletteWorkにコピー
//----------------------------------------------------------
void PlttCopy(void)
{
	u16 i;
...
<<<<<<< palanm.c
//----------------------------------------------------------
//	パレット加減算リクエスト  ※リクエストが重なった場合は先勝ちです
//
// fade_bit : パレットを指定（各ビットがパレット番号に対応
//			  ビット0 = BGの0パレット、 Bit16 = OBJの0パレット)
// wait : 値を大きくするとゆっくりフェードアウト、インします。
//		  値を小さくすると早くフェードアウト、インします。
// start_evy : 最初の色の濃度を指定します(0〜16)
// 			   0(元の色)〜2,3..(中間色)〜16(指定した色)
// end_evy : フェード計算後の最終的な色の濃度を指定します。引数の値は↑と同じです
// next_rgb : 変更後の色を指定
//
// 戻り値: 1=正常終了  0=エラー
//----------------------------------------------------------
u8 PaletteFadeReq(u32 fade_bit, s8 wait, u8 start_evy, u8 end_evy, u16 next_rgb)
{
	...
	if(wait < 0){	//waitがマイナスの時はvalueを大きくしてフェードを早くする
		FadeData.fade_value += abs(wait);
		wait = 0;
	}
	
	FadeData.fade_bit = fade_bit;
	FadeData.wait = wait;			//初回はリクエストかけた瞬間に実行
	...
	PaletteAnime();		//add 2001.06.26(火) リクエストしてすぐに反映させるよう変更
...
//	BIT_SET(*PalAnmBit, paw->pal_no_def >> 4);	//変更したパレットのビットをセット
}

//----------------------------------------------------------
//	フェードとパレットアニメが同時に動くときの加減算補正
//----------------------------------------------------------
static void FadeCheck(PalAnmWorkData *paw, u32 *PalAnmBit)
{
	u8 i;
	...
	if(paw->PalAnmDat->type == PA_BEFORE)
	{
		if(FadeData.wait == FadeData.wait_def)
			return;		//フェードで勝手に減色されるのでOK
		
		//フェードがウェイトで実行されないのでここでやる
	...
	{
		if(FadeData.wait > 0)
			return;		//フェードが実行されないので減色されない
		
		if(paw->TransWait == paw->PalAnmDat->TransWaitDef)
			return;		//データが転送されたのでOK
		
		//フェードで減色されてしまっているので転送しなおす
		now_pal = paw->TransValueLoop * paw->PalAnmDat->TransValue;
...
//----------------------------------------------------------
//	ソフトフェードイン、アウト(パレットアニメルーチン用)
// 戻り値: 0=フェードリクエストがかかってない、又はリクエストが終了した
//         1=フェード実行中
//         2=フェードリクエストはかかっているがウェイト中なので処理はしなかった
//----------------------------------------------------------
#include "softfade.h"
u8 ColorAddSub(void)
{
	...
	{
		evy = FadeData.now_evy;	//31を超えるとオーバーフローを起こすので
	...
	{
		evy = FadeData.now_evy;	//マイナスを取るため
		evy -= FadeData.fade_value;
		if(evy < FadeData.end_evy)
			evy = FadeData.end_evy;
		FadeData.now_evy = evy;
...
//=========================================================================
//	カラー反転						2001.06.26(火)
// 引数:　bit = カラー反転する対象パレット   0bit:BGPalette0   16bit:ObjPalette0
//=========================================================================
...
//=========================================================================
//	カラー加算						2001.06.29
// 引数:　bit = カラー加算する対象パレット   0bit:BGPalette0   16bit:ObjPalette0
//=========================================================================
...
// 引数:　bit = 戻す対象パレット   0bit:BGPalette0   16bit:ObjPalette0
//=========================================================================
...
// fade_type = どのフェードを行うか
//			   W_to_N:白→ノーマル  N_to_W:ノーマル→白  B_to_N:黒→ノーマル  N_to_B:ノーマル→黒
//=========================================================================
...
static void LightFadeCommon(u8 fade_type)
{
	...
	PaletteAnime();		//リクエストしてすぐに反映させるよう変更
}
...
//	ハードによるパレット加減算リクエスト
//
// fade_bit : 加減算の対象にするBG,OBJを指定(BLDCNTレジスタの第1対象ピクセルに設定するビット)
//				6,7bit:光度変更アップ、ダウンビット(10:UP	 11:DOWN)
//				5bit:BD(バックグラウンド), 4bit:OBJ, 3:BG3, 2:BG2, 1:BG1, 0:BG0
//
// wait : 値を大きくするとゆっくりフェードアウト、インします (MAX:63)
// start_evy : 最初の色の濃度を指定します(0〜16)
// 			   0(元の色)〜2,3..(中間色)〜16(黒、もしくは白)
// end_evy : 最終的な色の濃度を指定します。引数の値は↑と同じです
// reset_flg : フェード終了時にレジスターの初期化をするか(0:しない  1:する)
//----------------------------------------------------------
...
//=========================================================================
//	フェード終了後、一定Syncウェイトを入れる
//	戻り値: 0=まだフェード終了してない		1=ウェイト中
//=========================================================================
static u8 FadeEndWaitCheck(void)
{
