 ...
 * @param   font		フォントタイプ
 * @param   str			文字列
 * @param   str			圧縮文字列の場合の一時展開先
 *
 * @retval  TRUE:全て正常
 * @retval  FALSE:不明な文字コードが入っている
 */
//------------------------------------------------------------------
u32 FontProc_ErrorStrCheck( FONT_TYPE font, STRBUF* str, STRBUF *temp_str )
{
	...
	STRBUF_Clear(temp_str);
	STRBUF_AddCompStr(temp_str, str);
	return FontDataMan_ErrorStrCheck(WorkPtr->fontMan[font],STRBUF_GetStringCodePointer(temp_str));
}
