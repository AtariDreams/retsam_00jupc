 ...
 */
//------------------------------------------------------------------
void WORDSET_RegisterTowerTrainerName( WORDSET* wordset, u32 bufID, u32 strID )
{
	MSGDATA_MANAGER *man = MSGMAN_Create(MSGMAN_TYPE_DIRECT, ARC_MSG, NARC_msg_btdtrname_dat, wordset->heapID);
	if( man )
	{
		MSGMAN_GetString( man, strID, wordset->tmpBuf );
		RegisterWord( wordset, bufID, wordset->tmpBuf, NULL );
...
void WORDSET_RegisterPokeGetPlace( WORDSET* wordset, u32 bufID, u32 placeNumber )
{
	...
	if( man )
	{
		if( strID < MSGMAN_GetMessageCount(man) && (!(type == 0 && strID == 0)) )
		{
			MSGMAN_GetString( man, strID, wordset->tmpBuf );
			RegisterWord( wordset, bufID, wordset->tmpBuf, NULL );
			MSGMAN_Delete( man );
		...
		{
			u32 file_id, place_id;
			
			MSGMAN_Delete( man );
			if(type == 0 && strID == 0){
				file_id = placenameFileID[1];
				place_id = MAPNAME_NEWWORLD;
			}
			else{
				file_id = placenameFileID[2];
				place_id = MAPNAME_ANOTHERGAME;
			}
			man = MSGMAN_Create(MSGMAN_TYPE_DIRECT, ARC_MSG, file_id, wordset->heapID);
			if( man )
			{
				MSGMAN_GetString( man, place_id, wordset->tmpBuf );
				RegisterWord( wordset, bufID, wordset->tmpBuf, NULL );
				MSGMAN_Delete( man );
			}
		}
...
{
	MSGDATA_MANAGER *man = MSGMAN_Create(MSGMAN_TYPE_DIRECT, ARC_MSG, NARC_msg_wflby_game_dat, wordset->heapID);
	if( man )
	{
		MSGMAN_GetString( man, type, wordset->tmpBuf );
		RegisterWord( wordset, bufID, wordset->tmpBuf, NULL );
		MSGMAN_Delete(man);
	}
}
...
{
	MSGDATA_MANAGER *man = MSGMAN_Create(MSGMAN_TYPE_DIRECT, ARC_MSG, NARC_msg_wflby_event_dat, wordset->heapID);
	if( man )
	{
		MSGMAN_GetString( man, type, wordset->tmpBuf );
		RegisterWord( wordset, bufID, wordset->tmpBuf, NULL );
...
{
	MSGDATA_MANAGER *man = MSGMAN_Create(MSGMAN_TYPE_DIRECT, ARC_MSG, NARC_msg_wflby_item_dat, wordset->heapID);
	if( man )
	{
		MSGMAN_GetString( man, type, wordset->tmpBuf );
		RegisterWord( wordset, bufID, wordset->tmpBuf, NULL );
...
{
	MSGDATA_MANAGER *man = MSGMAN_Create(MSGMAN_TYPE_DIRECT, ARC_MSG, NARC_msg_wifi_aisatu_japan_dat, wordset->heapID);
	if( man )
	{
		MSGMAN_GetString( man, time_zone, wordset->tmpBuf );
		RegisterWord( wordset, bufID, wordset->tmpBuf, NULL );
		MSGMAN_Delete(man);
	}
}
...
{
	MSGDATA_MANAGER *man = MSGMAN_Create(MSGMAN_TYPE_DIRECT, ARC_MSG, NARC_msg_wifi_aisatu_english_dat, wordset->heapID);
	if( man )
	{
		MSGMAN_GetString( man, time_zone, wordset->tmpBuf );
		RegisterWord( wordset, bufID, wordset->tmpBuf, NULL );
...
{
	MSGDATA_MANAGER *man = MSGMAN_Create(MSGMAN_TYPE_DIRECT, ARC_MSG, NARC_msg_wifi_aisatu_france_dat, wordset->heapID);
	if( man )
	{
		MSGMAN_GetString( man, time_zone, wordset->tmpBuf );
		RegisterWord( wordset, bufID, wordset->tmpBuf, NULL );
...
{
	MSGDATA_MANAGER *man = MSGMAN_Create(MSGMAN_TYPE_DIRECT, ARC_MSG, NARC_msg_wifi_aisatu_italy_dat, wordset->heapID);
	if( man )
	{
		MSGMAN_GetString( man, time_zone, wordset->tmpBuf );
		RegisterWord( wordset, bufID, wordset->tmpBuf, NULL );
...
{
	MSGDATA_MANAGER *man = MSGMAN_Create(MSGMAN_TYPE_DIRECT, ARC_MSG, NARC_msg_wifi_aisatu_germany_dat, wordset->heapID);
	if( man )
	{
		MSGMAN_GetString( man, time_zone, wordset->tmpBuf );
		RegisterWord( wordset, bufID, wordset->tmpBuf, NULL );
...
{
	MSGDATA_MANAGER *man = MSGMAN_Create(MSGMAN_TYPE_DIRECT, ARC_MSG, NARC_msg_wifi_aisatu_spain_dat, wordset->heapID);
	if( man )
	{
		MSGMAN_GetString( man, time_zone, wordset->tmpBuf );
		RegisterWord( wordset, bufID, wordset->tmpBuf, NULL );
 ...
 */
//------------------------------------------------------------------
void WORDSET_RegisterFurniture( WORDSET * wordset, u32 bufID, u32 furnitureID )
{
	MSGDATA_MANAGER * man;
	
	man = MSGMAN_Create( MSGMAN_TYPE_DIRECT, ARC_MSG, NARC_msg_fum_dat, wordset->heapID );
	
	MSGMAN_GetString( man, furnitureID, wordset->tmpBuf );
	RegisterWord( wordset, bufID, wordset->tmpBuf, NULL );
