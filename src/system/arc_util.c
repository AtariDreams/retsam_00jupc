 ...
 * @param   dataIndex		アーカイブデータインデックス
 * @param   bgl				BGLデータ
 ...
 *
 * @param   fileIdx		[in] アーカイブファイルインデックス
 * @param   dataIdx		[in] アーカイブデータインデックス
 * @param   type		[in] 転送先タイプ
 * @param   offs		[in] 転送オフセット
 * @param   heapID		[in] ヒープID
 * @param   proxy		[out]作成するプロキシアドレス
 *
 *	[ type ]
 ...
 * @param   mapType			[in] マッピングタイプ
 * @param   transSize		[in] 転送サイズ。０なら全転送。
 * @param   vramType		[in] 転送先タイプ
 * @param   offs			[in] 転送オフセット
 * @param   heapID			[in] テンポラリに使うヒープID
 * @param   proxy			[out] 作成するプロキシアドレス
 *
 *
 *	[ mapType ]
 ...
 * @param   offs			[in] 転送オフセット
 * @param   heapID			[in] テンポラリに使うヒープID
 * @param   proxy			[out] 作成するプロキシのアドレス
 * @param   charData		[out] キャラデータアドレスを保持するポインタのアドレス
 *
 ...
 *
 * @param   fileIdx			[in] アーカイブファイルインデックス
 * @param   arcIndex		[in] アーカイブデータインデックス
 * @param   compressedFlag	[in] 圧縮されているデータか？
 * @param   scrnData		[out] スクリーンデータアドレスを保持するポインタのアドレス
 * @param   heapID			[in] ヒープID
 *
 * @retval  void*		ロードしたデータの先頭ポインタ
 */
 ...
 * @param   heapID			ヒープID
 * @param   pSize			セルバンクのバイトサイズを受け取る変数ポインタ（要らなければNULLで）
 *
 * @retval  void*		ロードしたデータの先頭ポインタ
 */
//--------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
/**
 *	@brief	キャラデータ の VRAM 転送＆ NITRO System イメージプロキシ を作成。VRAM転送型の画像素材用。
 *
 * ※ この関数を使っても、VRAMに画像は転送されません
 *    この関数でロードしたデータは解放されません。戻り値のconst void*を管理して、
 *    不要になったら解放処理を行ってください。
