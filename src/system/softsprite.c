...
//
// -----------------------------------------
//#define SS_PAT_ANM_OFS_Y_ON			///< 有効 = パターンアニメのオフセットYを反映する
...
void	SoftSpriteAnimeSet(SOFT_SPRITE *ss,int anm_no)
{
	...
	if(ss->ssanm[ss->anm_seq].patno==-1){
		...
		ss->ssp.pos_ox	= ss->ssanm[ss->anm_seq].ox;
		
	#ifdef SS_PAT_ANM_OFS_Y_ON
		ss->ssp.pos_oy	= ss->ssanm[ss->anm_seq].oy;
	#endif
		
	//	OS_Printf("anm offset \( x, y \)=\(%3d,%3d\)\n", ss->ssanm[ss->anm_seq].ox, ss->ssanm[ss->anm_seq].oy);
	}
}
...
void	SoftSpriteAnimeDataSet( SOFT_SPRITE *ss, SOFT_SPRITE_ANIME *ssanm )
{
	MI_CpuCopy8( ssanm, &ss->ssanm, sizeof( SOFT_SPRITE_ANIME ) * SS_ANM_SEQ_MAX );
}
...
static void SoftSpriteAnimeCoreEx( u8* anm_flag, u8* pat, u8* seq, u8* wait, s16* ox, s16* oy, u8* loop_cnt, const SOFT_SPRITE_ANIME* anm_tbl )
{
	if(*anm_flag)
	{
		if( (*wait) == 0 )
		{
			...
			if(	( ( *seq ) >= SS_ANM_SEQ_MAX ) || ( anm_tbl[ *seq ].patno == -1 ) )
			{
				(*pat) = 0;
				(*anm_flag) = 0;
				(*ox)	= 0;
			...
			{
				u8	seqno		= *seq;
				u8	temp_patno	= anm_tbl[ seqno ].patno;
				u8	temp_wait	= anm_tbl[ seqno ].wait;
				s8	temp_ox		= anm_tbl[ seqno ].ox;
...
//static void SoftSpriteAnimeCoreEx( u8* anm_flag, u8* pat, u8* seq, u8* wait, s16* ox, s16* oy, u8* loop_cnt, const SOFT_SPRITE_ANIME* anm_tbl )
static void SoftSpriteAnimeCoreSS( SOFT_SPRITE *ss )
{
#if 1
	if( ss->anm_no )
	{
		if( ss->anm_wait == 0 )
		{
			...
			while( ss->ssanm[ ss->anm_seq ].patno < -1 )
			{
				...
				if(	(ss->ssanm[ ss->anm_seq ].wait == ss->anm_loop[ ss->anm_seq ]) || (ss->ssanm[ ss->anm_seq ].wait == 0) )
				{
					ss->anm_loop[ ss->anm_seq ] = 0;
					ss->anm_seq++;
				}
				else
				{
					ss->anm_seq = ( ss->ssanm[ ss->anm_seq ].patno * -1 ) - 2;
				}
			}

			if(	( ( ss->anm_seq ) >= SS_ANM_SEQ_MAX ) || ( ss->ssanm[ ss->anm_seq ].patno == -1 ) )
			{
				ss->anm_pat 	= 0;
				ss->anm_no		= 0;
				ss->ssp.pos_ox	= 0;
...
//	SoftSpriteAnimeCore( &(ss->anm_no), &(ss->anm_pat), &(ss->anm_seq), &(ss->anm_wait), ss->anm_loop, ss->ssanm );
	...
	SoftSpriteAnimeCoreSS( ss );
#if 0	
	SoftSpriteAnimeCoreEx( &(ss->anm_no),
						   &(ss->anm_pat),
						   &(ss->anm_seq),
						   &(ss->anm_wait),
						   &(ss->ssp.pos_ox),
...
static	void	SoftSpriteTextureFieldMake(SOFT_SPRITE_MANAGER *ssm_p)
{
	...
	for(i=0;i<SOFT_SPRITE_MAX;i++){
		if((ssm_p->ss[i].exist)&&(ssm_p->ss[i].make_req)){
			...
			ssm_p->cd_dest.characterFmt=cd_src->characterFmt;
			src=cd_src->pRawData;
			SoftSpriteChrMask_ArcID(src, ssm_p->ss[i].ssa.arc_no);
...
{
	if(arc_id == ARC_DP_POKE_GRA || arc_id == ARC_DP_OTHER_POKE){
