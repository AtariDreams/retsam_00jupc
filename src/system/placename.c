...
{
	if((number >= PLACE_RANGE_DP_NORMAL_LOW && number <= PLACE_RANGE_DP_NORMAL_HIGH) 
			|| (number >= PLACE_RANGE_DP_TOKUSYU_LOW && number <= PLACE_RANGE_DP_TOKUSYU_HIGH)
			|| (number >= PLACE_RANGE_DP_HAIFU_LOW && number <= PLACE_RANGE_DP_HAIFU_HIGH)){
		return TRUE;
	}
	return FALSE;
}
