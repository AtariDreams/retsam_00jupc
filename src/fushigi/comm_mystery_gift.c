...
static void CreateWindowMenuDataWiFi(PROC *proc, LISTDATA *ld, int num, GF_BGL_BMPWIN *win, u32 msg);
static void CreateMenuWindow(PROC *proc, int base, u32 msg);

static void DisplaySequence(PROC *proc, GF_BGL_BMPWIN *win, u32 msgid);
static void DisplaySequenceWiFi(PROC *proc, GF_BGL_BMPWIN *win, u32 msgid);
...
static void CMG_BmpTalkWinClear(GF_BGL_BMPWIN * win, u8 trans_sw)
{
  if(GF_BGL_BmpWinAddCheck(win) == TRUE)
...
static void CreateWindowMenuDataWiFi(PROC *proc, LISTDATA *ld, int num, GF_BGL_BMPWIN *win, u32 msg)
{
  ...
  wk->msgman = MSGMAN_Create( MSGMAN_TYPE_NORMAL, ARC_MSG, NARC_msg_mystery_dat, HEAPID_MYSTERYGIFT );		///< いつもの
  for(i = 0; i < num; i++)
    BMP_MENULIST_AddArchiveString(wk->bmd, wk->msgman, ld[i].id, ld[i].param);
  MSGMAN_Delete(wk->msgman);
  /* desc作成 → 変更 → 登録 */
...
//  if ( wk->wifi_dl_seq == WIFI_MYSTERYGIFT_DOWNLOAD_COMPLETE ){
//		DisplaySequenceWiFi(proc, &wk->msgwin, dwc_message_0011 );
//	}
...
//--------------------------------------------------------------------------------------------
  ...
  case MYSTERYGIFT_BEACON_DOWNLOAD:
    ...
    if(MovieEffectModeCheck(deli) == TRUE){
		...
		{
			DEMO_WORK* demo_wk = sys_AllocMemory( HEAPID_MYSTERYGIFT, sizeof( DEMO_WORK ) );
