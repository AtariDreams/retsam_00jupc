...
static	void	DebugFightPokemonParamMake(DEBUG_FIGHT_PARAM *dfp,DEBUG_POKEMON_PARAM *dpp,POKEPARTY *party);
...
void	DebugFightRecDataEditScreenCreate(DEBUG_FIGHT_PARAM *dfp);
void	DebugFightRecDataEdit2ScreenCreate(DEBUG_FIGHT_PARAM *dfp,GF_PRINTCOLOR col);
...
static	void	DebugFightMinePut(DEBUG_FIGHT_PARAM *dfp);
static	void	DebugFightEnemyPut(DEBUG_FIGHT_PARAM *dfp);
...
static	void	DebugFightOrderDataPut(DEBUG_FIGHT_PARAM *dfp,DEBUG_POKEMON_PARAM *dpp);

static	void	DebugFightTrainer1IDPut(DEBUG_FIGHT_PARAM *dfp);
static	void	DebugFightTrainer1IDDataPut(DEBUG_FIGHT_PARAM *dfp);
static	void	DebugFightTrainer2IDPut(DEBUG_FIGHT_PARAM *dfp);
static	void	DebugFightTrainer2IDDataPut(DEBUG_FIGHT_PARAM *dfp);
static	void	DebugFightBT_Trainer1IDPut(DEBUG_FIGHT_PARAM *dfp);
static	void	DebugFightBT_Trainer1IDDataPut(DEBUG_FIGHT_PARAM *dfp);
static	void	DebugFightBT_Trainer2IDPut(DEBUG_FIGHT_PARAM *dfp);
static	void	DebugFightBT_Trainer2IDDataPut(DEBUG_FIGHT_PARAM *dfp);
...
void	DebugFightDefaultPokeSet(DEBUG_FIGHT_PARAM *dfp,DEBUG_POKEMON_PARAM *dpp);
...
const	MENU_LIST_PARAM	mlp_m[]={
	...
	//MS_SAVE
	{
		SAVE_CUR_X,				//カーソルX位置
		SAVE_CUR_Y,				//カーソルY位置
		MS_TIMEZONE,			//上キーを押したときの移動先
		MS_ENEMY,				//下キーを押したときの移動先
		MS_LOAD,				//左キーを押したときの移動先
		MS_BRD_SAVE,			//右キーを押したときの移動先
		DebugFightSaveAct,		//Aボタンを押したときのアクション
	},
	//MS_BRD_SAVE
	{
		BRD_SAVE_CUR_X,			//カーソルX位置
		BRD_SAVE_CUR_Y,			//カーソルY位置
		MS_FIGHT_TYPE,			//上キーを押したときの移動先
		MS_ENEMY,				//下キーを押したときの移動先
		MS_SAVE,				//左キーを押したときの移動先
		MS_VERSION,				//右キーを押したときの移動先
		DebugFightBRDSaveAct,	//Aボタンを押したときのアクション
	},
	//MS_VERSION,
	{
	...
	//MS_MINE,
	{
		...
		NO_MOVE,				//左キーを押したときの移動先
		MS_ENEMY,				//右キーを押したときの移動先
		DebugFightMineAct,		//Aボタンを押したときのアクション
	},
	//MS_ENEMY,
	{
		ENEMY_CUR_X,			//カーソルX位置
		ENEMY_CUR_Y,			//カーソルY位置
		MS_BRD_SAVE,			//上キーを押したときの移動先
		MS_ENEMY_POKE_1,		//下キーを押したときの移動先
		MS_MINE,				//左キーを押したときの移動先
		NO_MOVE,				//右キーを押したときの移動先
		DebugFightEnemyAct,		//Aボタンを押したときのアクション
	},
};
...
static	const	MENU_LIST_PARAM	mlp_s3[]={
	...
	//MENU3_ORDER
	{
		...
		NO_MOVE,				//左キーを押したときの移動先
		NO_MOVE,				//右キーを押したときの移動先
		DebugFightOrderAct,		//Aボタンを押したときのアクション
	},
};
...
static	const	u32	fight_type_table[][2]={
	{FIGHT_TYPE_YASEI,						(0x01<<0)|(0x01<<6)},
	{FIGHT_TYPE_TRAINER,					(0x01<<0)|(0x01<<6)},
	{FIGHT_TYPE_2vs2_TRAINER,				(0x03<<0)|(0x03<<6)},
	{FIGHT_TYPE_1vs1_SIO,					(0x01<<0)|(0x01<<6)},
	{FIGHT_TYPE_1vs1_SIO,					(0x01<<0)|(0x01<<6)},
	{FIGHT_TYPE_2vs2_SIO,					(0x03<<0)|(0x03<<6)},
	{FIGHT_TYPE_2vs2_SIO,					(0x03<<0)|(0x03<<6)},
	{FIGHT_TYPE_MULTI_SIO,					(0x01<<0)|(0x01<<6)},
	{FIGHT_TYPE_MULTI_SIO,					(0x01<<0)|(0x01<<6)},
	{FIGHT_TYPE_1vs1_SIO,					(0x01<<0)|(0x01<<6)},
	{FIGHT_TYPE_1vs1_SIO,					(0x01<<0)|(0x01<<6)},
	{FIGHT_TYPE_SAFARI,						(0x01<<0)|(0x01<<6)},
	{FIGHT_TYPE_POKE_PARK,					(0x01<<0)|(0x01<<6)},
	{FIGHT_TYPE_GET_DEMO,					(0x01<<0)|(0x01<<6)},
	{FIGHT_TYPE_2vs2_YASEI,					(0x01<<0)|(0x01<<6)|(0x01<<12)|(0x01<<18)},
	{FIGHT_TYPE_TAG_BATTLE,					(0x03<<0)|(0x01<<6)|(0x01<<18)},
	{FIGHT_TYPE_AI_MULTI,					(0x01<<0)|(0x01<<6)|(0x01<<12)|(0x01<<18)},
	{FIGHT_TYPE_BATTLE_TOWER_SIO_MULTI,		(0x01<<0)|(0x01<<6)|(0x01<<12)|(0x01<<18)},
	{FIGHT_TYPE_BATTLE_TOWER_SIO_MULTI,		(0x01<<0)|(0x01<<6)|(0x01<<12)|(0x01<<18)},
};
...
void	DebugFightAutoScreenCreate(DEBUG_FIGHT_PARAM *dfp)
{
	...
	for(i=DF_PP_NO_DEC;i<DF_COM_SELECT+1;i++){
		cnt=i-DF_PP_NO_DEC;
		MSGMAN_GetString(dfp->man_msg,i,dfp->msg_buf);
		GF_STR_PrintSimple(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,MINE_X,MINE_Y+16*cnt,MSG_NO_PUT,NULL);
	...
	DebugFightTrainer1IDPut(dfp);
	DebugFightTrainer1IDDataPut(dfp);
	DebugFightTrainer2IDPut(dfp);
	DebugFightTrainer2IDDataPut(dfp);
	DebugFightBT_Trainer1IDPut(dfp);
	DebugFightBT_Trainer1IDDataPut(dfp);
	DebugFightBT_Trainer2IDPut(dfp);
	DebugFightBT_Trainer2IDDataPut(dfp);
...
void	DebugFightRecDataEditScreenCreate(DEBUG_FIGHT_PARAM *dfp)
{
	...
	MSGMAN_GetString(dfp->man_msg,DF_CLIENT0+dfp->brdedit_client_no,dfp->msg_buf);
...
void	DebugFightRecDataEdit2ScreenCreate(DEBUG_FIGHT_PARAM *dfp,GF_PRINTCOLOR col)
{
	int		i,j,y=0,pos=0,data;

	DebugFightRecDataEditScreenCreate(dfp);

	MSGMAN_GetString(dfp->man_msg,DF_ADRS000+dfp->brdedit_page/4,dfp->msg_buf);
	GF_STR_PrintSimple(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,MINE_X,MINE_Y+16,MSG_NO_PUT,NULL);

	for(i=0;i<8;i++){
		MSGMAN_GetString(dfp->man_msg,DF_ADRS00+i+8*(dfp->brdedit_page%4),dfp->msg_buf);
		GF_STR_PrintSimple(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,MINE_X-16,MINE_Y+32+16*y,MSG_NO_PUT,NULL);
		for(j=0;j<8;j++){
			data=BattleRec_DataPlay(dfp->brdedit_client_no,0x40*dfp->brdedit_page+pos);
			STRBUF_SetNumber(dfp->msg_buf,data,2,NUMBER_DISPTYPE_ZERO,NUMBER_CODETYPE_DEFAULT);
			if(pos==dfp->brdedit_pos){
				GF_STR_PrintColor(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,MINE_X+16+24*j,MINE_Y+32+16*y,MSG_NO_PUT,col,NULL);
			}
			else{
				GF_STR_PrintColor(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,MINE_X+16+24*j,MINE_Y+32+16*y,MSG_NO_PUT,COL_NORMAL,NULL);
			}
			pos++;
		}
		y++;
...
void	DebugFightComSelectEditScreenCreate(DEBUG_FIGHT_PARAM *dfp,GF_PRINTCOLOR col)
{
	...
	for(i=0;i<8;i++){
		for(j=0;j<8;j++){
			data=dfp->dsp.command_buffer[i*8+j];
			STRBUF_SetNumber(dfp->msg_buf,data,2,NUMBER_DISPTYPE_ZERO,NUMBER_CODETYPE_DEFAULT);
			if(pos==dfp->brdedit_pos){
				GF_STR_PrintColor(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,MINE_X+16+24*j,MINE_Y+32+16*y,MSG_NO_PUT,col,NULL);
			}
...
void	DebugFightMainScreenCreate(DEBUG_FIGHT_PARAM *dfp)
{
	...
	DebugFightMinePut(dfp);
	DebugFightEnemyPut(dfp);
	...
	MSGMAN_GetString(dfp->man_msg,DF_BRD_SAVE,dfp->msg_buf);
	GF_STR_PrintSimple(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,BRD_SAVE_X,BRD_SAVE_Y,MSG_NO_PUT,NULL);
...
static	void	DebugFightMinePut(DEBUG_FIGHT_PARAM *dfp)
{
	GF_BGL_BmpWinFill(dfp->win_m,0x0f,MINE_X,MINE_Y,64,16);
	MSGMAN_GetString(dfp->man_msg,DF_MINE1+dfp->mine,dfp->msg_buf);
	GF_STR_PrintSimple(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,MINE_X,MINE_Y,MSG_NO_PUT,NULL);
...
static	void	DebugFightEnemyPut(DEBUG_FIGHT_PARAM *dfp)
{
	GF_BGL_BmpWinFill(dfp->win_m,0x0f,ENEMY_X,ENEMY_Y,64,16);
	MSGMAN_GetString(dfp->man_msg,DF_ENEMY1+dfp->enemy,dfp->msg_buf);
...
static	void	DebugFightMonsNamePut(DEBUG_FIGHT_PARAM *dfp,int pos,GF_PRINTCOLOR col)
{
	DEBUG_POKEMON_PARAM	*dpp=DebugFightDPPGet(dfp,dfp->mine,dfp->enemy,pos);
	...
	case 2:
		{
			...
			DebugFightOrderDataPut(dfp,dpp);
		}
		break;
	default:
//		dfp->flag=page;
	...
	if(dpp->mons_no){
		{
			...
			PokeParaPut(dfp->pp,ID_PARA_form_no,&dpp->form_no);
...
static	void	DebugFightSpeAbiDataPut(DEBUG_FIGHT_PARAM *dfp,DEBUG_POKEMON_PARAM *dpp,GF_PRINTCOLOR col)
{
	...
	if(dpp->speabi){
		dpp->speabino=PokeFormNoPersonalParaGet(dpp->mons_no,dpp->form_no,ID_PER_speabi2);
	}
	else{
...
static	void	DebugFightOrderDataPut(DEBUG_FIGHT_PARAM *dfp,DEBUG_POKEMON_PARAM *dpp)
{
	GF_BGL_BmpWinFill(dfp->win_s,0x0f,ORDER_X,ORDER_Y,12*9,16);

	MSGMAN_GetString(dfp->man_msg,DF_ORDER_OK+dpp->order,dfp->msg_buf);
	GF_STR_PrintSimple(dfp->win_s,FONT_SYSTEM,dfp->msg_buf,ORDER_X,ORDER_Y,MSG_NO_PUT,NULL);
	dfp->put_req=1;
}

static	void	DebugFightTrainer1IDPut(DEBUG_FIGHT_PARAM *dfp)
{
	MSGMAN_GetString(dfp->man_msg,DF_TRAINER1_ID,dfp->msg_buf);
	GF_STR_PrintSimple(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,TRAINER1_ID_X,TRAINER1_ID_Y,MSG_NO_PUT,NULL);
	dfp->put_req=1;
}
...
static	void	DebugFightTrainer1IDDataPut(DEBUG_FIGHT_PARAM *dfp)
{
	...
	GF_BGL_BmpWinFill(dfp->win_m,0x0f,TRAINER1_ID_DATA_X,TRAINER1_ID_DATA_Y,256-TRAINER1_ID_DATA_X,16);
	STRBUF_SetNumber(dfp->msg_buf,dfp->dsp.trainer1_id,3,NUMBER_DISPTYPE_ZERO,NUMBER_CODETYPE_DEFAULT);
	GF_STR_PrintColor(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,TRAINER1_ID_DATA_X,TRAINER1_ID_DATA_Y,MSG_NO_PUT,COL_NORMAL,NULL);

	if(dfp->dsp.trainer1_id){
		{
			MSGDATA_MANAGER	*man;

			man=MSGMAN_Create(MSGMAN_TYPE_DIRECT,ARC_MSG,NARC_msg_trtype_dat,HEAPID_DEBUG_APPLICATION);

			trtype=TT_TrainerDataParaGet(dfp->dsp.trainer1_id,ID_TD_tr_type);
			MSGMAN_GetString(man,trtype,dfp->msg_buf);
			GF_STR_PrintSimple(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,TRAINER1_ID_DATA_X+12*2,TRAINER1_ID_DATA_Y,MSG_NO_PUT,NULL);
			MSGMAN_Delete(man);

			man=MSGMAN_Create(MSGMAN_TYPE_DIRECT,ARC_MSG,NARC_msg_trname_dat,HEAPID_DEBUG_APPLICATION);
			MSGMAN_GetString(man,dfp->dsp.trainer1_id,dfp->msg_buf);
			GF_STR_PrintSimple(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,TRAINER1_ID_DATA_X+12*11,TRAINER1_ID_DATA_Y,MSG_NO_PUT,NULL);
			MSGMAN_Delete(man);
		}
	}
...
static	void	DebugFightTrainer2IDPut(DEBUG_FIGHT_PARAM *dfp)
{
	MSGMAN_GetString(dfp->man_msg,DF_TRAINER2_ID,dfp->msg_buf);
	GF_STR_PrintSimple(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,TRAINER2_ID_X,TRAINER2_ID_Y,MSG_NO_PUT,NULL);
	dfp->put_req=1;
}
...
static	void	DebugFightTrainer2IDDataPut(DEBUG_FIGHT_PARAM *dfp)
{
	...
	GF_BGL_BmpWinFill(dfp->win_m,0x0f,TRAINER2_ID_DATA_X,TRAINER2_ID_DATA_Y,256-TRAINER2_ID_DATA_X,16);
	STRBUF_SetNumber(dfp->msg_buf,dfp->dsp.trainer2_id,3,NUMBER_DISPTYPE_ZERO,NUMBER_CODETYPE_DEFAULT);
	GF_STR_PrintColor(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,TRAINER2_ID_DATA_X,TRAINER2_ID_DATA_Y,MSG_NO_PUT,COL_NORMAL,NULL);

	if(dfp->dsp.trainer2_id){
		{
			MSGDATA_MANAGER	*man;

			man=MSGMAN_Create(MSGMAN_TYPE_DIRECT,ARC_MSG,NARC_msg_trtype_dat,HEAPID_DEBUG_APPLICATION);

			trtype=TT_TrainerDataParaGet(dfp->dsp.trainer2_id,ID_TD_tr_type);
			MSGMAN_GetString(man,trtype,dfp->msg_buf);
			GF_STR_PrintSimple(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,TRAINER2_ID_DATA_X+12*2,TRAINER2_ID_DATA_Y,MSG_NO_PUT,NULL);
			MSGMAN_Delete(man);

			man=MSGMAN_Create(MSGMAN_TYPE_DIRECT,ARC_MSG,NARC_msg_trname_dat,HEAPID_DEBUG_APPLICATION);
			MSGMAN_GetString(man,dfp->dsp.trainer2_id,dfp->msg_buf);
			GF_STR_PrintSimple(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,TRAINER2_ID_DATA_X+12*11,TRAINER2_ID_DATA_Y,MSG_NO_PUT,NULL);
			MSGMAN_Delete(man);
		}
	}
...
static	void	DebugFightBT_Trainer1IDPut(DEBUG_FIGHT_PARAM *dfp)
{
	MSGMAN_GetString(dfp->man_msg,DF_BT_TRAINER1_ID,dfp->msg_buf);
	GF_STR_PrintSimple(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,BT_TRAINER1_ID_X,BT_TRAINER1_ID_Y,MSG_NO_PUT,NULL);
	dfp->put_req=1;
}
...
static	void	DebugFightBT_Trainer1IDDataPut(DEBUG_FIGHT_PARAM *dfp)
{
	...
	GF_BGL_BmpWinFill(dfp->win_m,0x0f,BT_TRAINER1_ID_DATA_X,BT_TRAINER1_ID_DATA_Y,256-BT_TRAINER1_ID_DATA_X,16);
	STRBUF_SetNumber(dfp->msg_buf,dfp->dsp.bt_trainer1_id,3,NUMBER_DISPTYPE_ZERO,NUMBER_CODETYPE_DEFAULT);
	GF_STR_PrintColor(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,BT_TRAINER1_ID_DATA_X,BT_TRAINER1_ID_DATA_Y,MSG_NO_PUT,COL_NORMAL,NULL);

	if(dfp->dsp.bt_trainer1_id){
		{
			B_TOWER_TRAINER_ROM_DATA	*trd;
			MSGDATA_MANAGER	*man;

			trd=ArchiveDataLoadMalloc(ARC_BTD_TR,dfp->dsp.bt_trainer1_id-1,HEAPID_DEBUG_APPLICATION);

			man=MSGMAN_Create(MSGMAN_TYPE_DIRECT,ARC_MSG,NARC_msg_trtype_dat,HEAPID_DEBUG_APPLICATION);

			MSGMAN_GetString(man,trd->tr_type,dfp->msg_buf);
			GF_STR_PrintSimple(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,BT_TRAINER1_ID_DATA_X+12*2,BT_TRAINER1_ID_DATA_Y,MSG_NO_PUT,NULL);
			MSGMAN_Delete(man);

			man=MSGMAN_Create(MSGMAN_TYPE_DIRECT,ARC_MSG,NARC_msg_btdtrname_dat,HEAPID_DEBUG_APPLICATION);
			MSGMAN_GetString(man,dfp->dsp.bt_trainer1_id-1,dfp->msg_buf);
			GF_STR_PrintSimple(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,BT_TRAINER1_ID_DATA_X+12*11,BT_TRAINER1_ID_DATA_Y,MSG_NO_PUT,NULL);
			MSGMAN_Delete(man);
...
static	void	DebugFightBT_Trainer2IDPut(DEBUG_FIGHT_PARAM *dfp)
{
	MSGMAN_GetString(dfp->man_msg,DF_BT_TRAINER2_ID,dfp->msg_buf);
	GF_STR_PrintSimple(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,BT_TRAINER2_ID_X,BT_TRAINER2_ID_Y,MSG_NO_PUT,NULL);
	dfp->put_req=1;
}
...
static	void	DebugFightBT_Trainer2IDDataPut(DEBUG_FIGHT_PARAM *dfp)
{
	...
	GF_BGL_BmpWinFill(dfp->win_m,0x0f,BT_TRAINER2_ID_DATA_X,BT_TRAINER2_ID_DATA_Y,256-BT_TRAINER2_ID_DATA_X,16);
	STRBUF_SetNumber(dfp->msg_buf,dfp->dsp.bt_trainer2_id,3,NUMBER_DISPTYPE_ZERO,NUMBER_CODETYPE_DEFAULT);
	GF_STR_PrintColor(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,BT_TRAINER2_ID_DATA_X,BT_TRAINER2_ID_DATA_Y,MSG_NO_PUT,COL_NORMAL,NULL);

	if(dfp->dsp.bt_trainer2_id){
		{
			B_TOWER_TRAINER_ROM_DATA	*trd;
			MSGDATA_MANAGER	*man;

			trd=ArchiveDataLoadMalloc(ARC_BTD_TR,dfp->dsp.bt_trainer2_id-1,HEAPID_DEBUG_APPLICATION);

			man=MSGMAN_Create(MSGMAN_TYPE_DIRECT,ARC_MSG,NARC_msg_trtype_dat,HEAPID_DEBUG_APPLICATION);

			MSGMAN_GetString(man,trd->tr_type,dfp->msg_buf);
			GF_STR_PrintSimple(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,BT_TRAINER2_ID_DATA_X+12*2,BT_TRAINER2_ID_DATA_Y,MSG_NO_PUT,NULL);
			MSGMAN_Delete(man);

			man=MSGMAN_Create(MSGMAN_TYPE_DIRECT,ARC_MSG,NARC_msg_btdtrname_dat,HEAPID_DEBUG_APPLICATION);
			MSGMAN_GetString(man,dfp->dsp.bt_trainer2_id-1,dfp->msg_buf);
			GF_STR_PrintSimple(dfp->win_m,FONT_SYSTEM,dfp->msg_buf,BT_TRAINER2_ID_DATA_X+12*11,BT_TRAINER2_ID_DATA_Y,MSG_NO_PUT,NULL);
			MSGMAN_Delete(man);
...
BOOL	DebugFightStart(DEBUG_FIGHT_PARAM *dfp)
{
	...
	DEBUG_POKEMON_PARAM	*dpp_m1,*dpp_m2,*dpp_e1,*dpp_e2;

	cnt=0;
	bit=1;

	for(no=0;no<CLIENT_MAX;no++){
		for(pos=0;pos<TEMOTI_POKEMAX;pos++){
			if((dfp->dsp.dpp[no][pos].mons_no)&&(dfp->dsp.dpp[no][pos].default_hp)){
				cnt|=bit;
			}
			bit=bit<<1;
		}
	...
	if(dfp->dsp.trainer1_id){
		dfp->param->trainer_id[CLIENT_NO_ENEMY]=dfp->dsp.trainer1_id;
		TT_EncountTrainerDataMake(dfp->param,dfp->sv,HEAPID_DEBUG_APPLICATION);
	}
	else if(dfp->dsp.bt_trainer1_id){
		{
			MSGDATA_MANAGER				*man;
			B_TOWER_TRAINER_ROM_DATA	*trd;
			STRBUF						*name;
			...
			dfp->param->trainer_id[CLIENT_NO_ENEMY]=dfp->dsp.bt_trainer1_id-1;
			dfp->param->trainer_data[CLIENT_NO_ENEMY].tr_type=trd->tr_type;
			name=MSGMAN_AllocString(man,dfp->dsp.bt_trainer1_id-1);
	...
	if(dfp->dsp.trainer2_id){
		dfp->param->trainer_id[CLIENT_NO_ENEMY2]=dfp->dsp.trainer2_id;
		TT_EncountTrainerDataMake(dfp->param,dfp->sv,HEAPID_DEBUG_APPLICATION);
	}
	else if(dfp->dsp.bt_trainer2_id){
		{
			MSGDATA_MANAGER				*man;
			B_TOWER_TRAINER_ROM_DATA	*trd;
			STRBUF						*name;
			...
			dfp->param->trainer_id[CLIENT_NO_ENEMY2]=dfp->dsp.bt_trainer2_id-1;
			dfp->param->trainer_data[CLIENT_NO_ENEMY2].tr_type=trd->tr_type;
			name=MSGMAN_AllocString(man,dfp->dsp.bt_trainer2_id-1);
			STRBUF_GetStringCode(name,&dfp->param->trainer_data[CLIENT_NO_ENEMY2].name[0],BUFLEN_PERSON_NAME);
			STRBUF_Delete(name);
			sys_FreeMemoryEz(trd);
			MSGMAN_Delete(man);
		}
	...
	if((dfp->dsp.trainer1_id==0)&&
	   (dfp->dsp.trainer2_id==0)&&
	   (dfp->dsp.bt_trainer1_id==0)&&
	   (dfp->dsp.bt_trainer2_id==0)){
		do{
			i=gf_rand()%843+1;
	...
	if(dfp->dsp.trainer1_id){
		dfp->param->trainer_data[CLIENT_NO_ENEMY].aibit=TT_TrainerDataParaGet(dfp->dsp.trainer1_id,ID_TD_aibit);
	}
	if(dfp->dsp.trainer2_id){
		dfp->param->trainer_data[CLIENT_NO_ENEMY2].aibit=TT_TrainerDataParaGet(dfp->dsp.trainer2_id,ID_TD_aibit);
	}
	if(dfp->dsp.bt_trainer1_id){
		dfp->param->trainer_data[CLIENT_NO_ENEMY].aibit=0x07;
	}
	if(dfp->dsp.bt_trainer2_id){
		dfp->param->trainer_data[CLIENT_NO_ENEMY2].aibit=0x07;
	}

	for(i=0;i<6;i++){
		dpp_m1=DebugFightDPPGet(dfp,0,0,i);
		dpp_m2=DebugFightDPPGet(dfp,1,0,i);
		dpp_e1=DebugFightDPPGet(dfp,0,0,i+6);
		dpp_e2=DebugFightDPPGet(dfp,0,1,i+6);
		if(dpp_m1->mons_no){
			DebugFightPokemonParamMake(dfp,dpp_m1,dfp->param->poke_party[0]);
		}
		if((dpp_e1->mons_no)&&(dfp->dsp.trainer1_id==0)){
			DebugFightPokemonParamMake(dfp,dpp_e1,dfp->param->poke_party[1]);
		}
		if(dpp_m2->mons_no){
			DebugFightPokemonParamMake(dfp,dpp_m2,dfp->param->poke_party[2]);
		}
		if((dpp_e2->mons_no)&&(dfp->dsp.trainer2_id==0)){
			DebugFightPokemonParamMake(dfp,dpp_e2,dfp->param->poke_party[3]);
		}
	}
	...
	dfp->param->battle_status_flag|=(dfp->dsp.battle_status_flag<<24);

	FontProc_UnloadBitData(FONT_SYSTEM);
	sys_VBlankFuncChange(NULL,NULL);

	if(sys.cont&PAD_KEY_LEFT){
		dfp->param->battle_status_flag|=BATTLE_STATUS_FLAG_REC_BATTLE;
	}
	if(sys.cont&PAD_KEY_RIGHT){
		dfp->param->battle_status_flag|=BATTLE_STATUS_FLAG_HAKAI_GIRA_BATTLE|BATTLE_STATUS_FLAG_YABURETA_SEKAI;
	}
...
static	void	DebugFightPokemonParamMake(DEBUG_FIGHT_PARAM *dfp,DEBUG_POKEMON_PARAM *dpp,POKEPARTY *party)
{
	...
	rnd=PokeRndGet(dpp->mons_no,dpp->sex,dpp->chr,dpp->rare);
	PokeParaSet(dfp->pp,dpp->mons_no,dpp->level,0,RND_SET,rnd,ID_SET,0);

	for(i=0;i<4;i++){
		PokeWazaSetPos(dfp->pp,dpp->waza[i],i);
		PokeParaPut(dfp->pp,ID_PARA_pp1+i,&dpp->pp[i]);
		PokeParaPut(dfp->pp,ID_PARA_pp_count1+i,&dpp->pp_count[i]);
	}
	for(i=STATUS_HP;i<STATUS_MAX;i++){
		PokeParaPut(dfp->pp,ID_PARA_hp_rnd+i,&dpp->status_rnd[i]);
		PokeParaPut(dfp->pp,ID_PARA_hp_exp+i,&dpp->status_exp[i]);
	...
	if(dpp->speabino_set){
		PokeParaPut(dfp->pp,ID_PARA_speabino,&dpp->speabino_set);
	}
	else{
		PokeParaPut(dfp->pp,ID_PARA_speabino,&dpp->speabino);
	}
	PokeParaPut(dfp->pp,ID_PARA_item,&dpp->item);
	PokeParaPut(dfp->pp,ID_PARA_friend,&dpp->friend);
	PokeParaPut(dfp->pp,ID_PARA_condition,&cond_table[dpp->condition]);
	PokeParaPut(dfp->pp,ID_PARA_exp,&dpp->exp);
	if(dpp->mons_no==MONSNO_AUSU){
		form_no=AusuTypeGet(ItemParamGet(dpp->item,ITEM_PRM_EQUIP,HEAPID_BASE_SYSTEM));
		PokeParaPut(dfp->pp,ID_PARA_form_no,&form_no);
	}
	else{
		PokeParaPut(dfp->pp,ID_PARA_form_no,&dpp->form_no);
	}
	PokeParaPut(dfp->pp,ID_PARA_tamago_flag,&dpp->egg_flag);
	PokeParaGetInfoSet(dfp->pp,dfp->param->my_status[0],dpp->get_ball,NULL,NULL,HEAPID_DEBUG_APPLICATION);

	PokeParaCalc(dfp->pp);

	PokeParaPut(dfp->pp,ID_PARA_hp,&dpp->default_hp);
	PokeParaPut(dfp->pp,ID_PARA_id_no,&dpp->order);

	PokeParty_Add(party,dfp->pp);
}
		...
		if(sys.repeat&PAD_KEY_UP){
			...
			if((dfp->dsp.fight_type==FT_2vs2)||
			   ...
			   (dfp->dsp.fight_type==FT_2vs2_SIO_C)){
				if(dfp->dsp.dpp[0][1].mons_no==0){
					DebugFightDefaultPokeSet(dfp,DebugFightDPPGet(dfp,dfp->mine,dfp->enemy,1));
				}
				if(dfp->dsp.dpp[1][1].mons_no==0){
					DebugFightDefaultPokeSet(dfp,DebugFightDPPGet(dfp,dfp->mine,dfp->enemy,1+6));
				}
			}
			else if(dfp->dsp.fight_type==FT_TAG){
				if(dfp->dsp.dpp[0][1].mons_no==0){
					DebugFightDefaultPokeSet(dfp,DebugFightDPPGet(dfp,dfp->mine,dfp->enemy,1));
				}
			}
...
{
	DebugFightStart(dfp);
	BattleRec_BattleParamRec(dfp->param);
...
void	DebugFightBRDSaveActMain(DEBUG_FIGHT_PARAM *dfp)
{
	int			rec_mode;
	SAVE_RESULT	ret;

	switch(fight_type_table[dfp->dsp.fight_type][0]){
	case FIGHT_TYPE_1vs1_YASEI:
	case FIGHT_TYPE_1vs1_TRAINER:
	case FIGHT_TYPE_1vs1_SIO:
	case FIGHT_TYPE_BATTLE_TOWER_1vs1:
		rec_mode = RECMODE_COLOSSEUM_SINGLE;
		break;
	case FIGHT_TYPE_2vs2_YASEI:
	case FIGHT_TYPE_2vs2_SIO:
	case FIGHT_TYPE_BATTLE_TOWER_2vs2:
		rec_mode = RECMODE_COLOSSEUM_DOUBLE;
		break;
	case FIGHT_TYPE_2vs2_TRAINER:
	case FIGHT_TYPE_MULTI_SIO:
	case FIGHT_TYPE_AI_MULTI:
	case FIGHT_TYPE_BATTLE_TOWER_AI_MULTI:
	case FIGHT_TYPE_BATTLE_TOWER_SIO_MULTI:
	case FIGHT_TYPE_TAG_BATTLE:
	...
	do{
		ret=BattleRec_Save(dfp->sv, rec_mode, 1, LOADDATA_MYREC, &dfp->save_seq0, &dfp->save_seq1);
	}while((ret!=SAVE_RESULT_OK)&&(ret!=SAVE_RESULT_NG));
}
...
static	BOOL	DebugFightGroundAct(DEBUG_FIGHT_PARAM *dfp,DEBUG_POKEMON_PARAM *dpp)
{
	...
	case 1:
		...
		else if((sys.repeat&PAD_KEY_DOWN)&&(dfp->dsp.ground<GROUND_ID_STAGE)){
			dfp->dsp.ground++;
			DebugFightGroundDataPut(dfp,COL_ACTIVE);
		}
		else if(sys.trg&PAD_BUTTON_A){
...
static	BOOL	DebugFightBGAct(DEBUG_FIGHT_PARAM *dfp,DEBUG_POKEMON_PARAM *dpp)
{
	...
	case 1:
		...
		else if((sys.repeat&PAD_KEY_DOWN)&&(dfp->dsp.bg<BG_ID_STAGE)){
			dfp->dsp.bg++;
			DebugFightBGDataPut(dfp,COL_ACTIVE);
		}
		else if(sys.trg&PAD_BUTTON_A){
...
void	DebugFightDefaultPokeSet(DEBUG_FIGHT_PARAM *dfp,DEBUG_POKEMON_PARAM *dpp)
{
	...
	PokeParaSetSexChr(dfp->pp,
					  ...
					  dpp->chr,
					  0);
	dpp->default_hp	=PokeParaGet(dfp->pp,ID_PARA_hpmax,NULL);
	dpp->hp_max		=PokeParaGet(dfp->pp,ID_PARA_hpmax,NULL);
	dpp->speabino	=PokeFormNoPersonalParaGet(dpp->mons_no,dpp->form_no,ID_PER_speabi1);
	dpp->get_ball	=PokeParaGet(dfp->pp,ID_PARA_get_ball,NULL);
}
...
static	int	DebugFightFormNoMaxGet(DEBUG_POKEMON_PARAM *dpp)
{
	...
	case MONSNO_ANNOON:
		...
		break;
	case MONSNO_EURISU:
		max=FORMNO_SHEIMI_MAX;		///<シェイミの別フォルムMAX
		break;
	case MONSNO_PURAZUMA:
		max=FORMNO_ROTOMU_MAX;		///<ロトムの別フォルムMAX
		break;
	case MONSNO_KIMAIRAN:
		max=FORMNO_GIRATINA_MAX;		///<ロトムの別フォルムMAX
		break;
	default:
		max=0;
		break;
