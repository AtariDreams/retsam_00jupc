...
static	const TCATS_OAM_INIT TgtTcats = {
...
static	const TCATS_CHAR_MANAGER_MAKE TgtCcmm = {
	BATTLE_CHAR_MAX,
...
static	const TCATS_RESOURCE_NUM_LIST TgtResourceList = {
...
static	void	TrGraInit(TR_GRA_TEST *tgt)
{
	...
	tgt->pfd=PaletteFadeInit(HEAPID_BATTLE);
	PaletteTrans_AutoSet(tgt->pfd, TRUE);
	PaletteFadeWorkAllocSet(tgt->pfd,FADE_MAIN_OBJ,BATTLE_MAIN_OBJPAL_SIZE,HEAPID_BATTLE);

	initVramTransferManagerHeap(BATTLE_VRAM_TRANSFER_TASK_NUM,HEAPID_BATTLE);

	tgt->csp=CATS_AllocMemory(HEAPID_BATTLE);
	CATS_SystemInit(tgt->csp,&TgtTcats,&TgtCcmm,BATTLE_OAM_PLTT_MAX);
	tgt->crp=CATS_ResourceCreate(tgt->csp);
	CATS_ClactSetInit(tgt->csp,tgt->crp,BATTLE_ACTOR_MAX);
	CATS_ResourceManagerInit(tgt->csp,tgt->crp,&TgtResourceList);
	CLACT_U_SetSubSurfaceMatrix(CATS_EasyRenderGet(tgt->csp),0,BATTLE_SUB_ACTOR_DISTANCE);

	tgt->tr_type=TRTYPE_BOY;

	tgt->cap=TrCLACTSet(tgt->csp,tgt->crp,tgt->pfd,APPEAR_X_TYPE_BB,APPEAR_Y_TYPE_BB,tgt->tr_type,PARA_FRONT,0,HEAPID_BATTLE);
...
static	void	TrGraTestMain(TR_GRA_TEST * tgt)
{
	...
	if ( sys.trg & PAD_BUTTON_START ){
		int max = CLACT_GetAnmSeqNum( tgt->cap->act );		
		if ( max != 1 ){
			CATS_ObjectAnimeSeqSetCap( tgt->cap, 1 );
		}
	}	
	if( old != tgt->tr_type ){
	...
	if(sys.trg & PAD_BUTTON_SELECT){
		...
		PaletteFadeWorkAllocFree(tgt->pfd,FADE_MAIN_OBJ);
		PaletteFadeFree(tgt->pfd);
		CATS_ResourceDestructor_S(tgt->csp,tgt->crp);
		CATS_FreeMemory(tgt->csp);
		DellVramTransferManager();
		sys_FreeMemoryEz(tgt->bgl);
		tgt->end_flag = TRUE;
		return;
	}
	CATS_ObjectUpdateCap( tgt->cap );
	CATS_Draw(tgt->crp);
	CATS_UpdateTransfer();
}
...
static	void	TrGraTestDebugPrint(TR_GRA_TEST *tgt)
{
	...
	CATS_ActorPointerDelete_S(tgt->cap);

	CATS_FreeResourceChar(tgt->crp,CHARID_ENCOUNT_TRAINER1);
	CATS_FreeResourcePltt(tgt->crp,PLTTID_ENCOUNT_TRAINER1);
	CATS_FreeResourceCell(tgt->crp,CELLID_ENCOUNT_TRAINER1);
	CATS_FreeResourceCellAnm(tgt->crp,CELLANMID_ENCOUNT_TRAINER1);
