...
#define	BATTLE_RESTART_KEY	(PAD_BUTTON_A|PAD_BUTTON_B|PAD_BUTTON_L|PAD_BUTTON_R)
...
#define	ADD_STATUS_MAX		(8-1)	//追加効果100%系のページの項目数（0オリジンなので、-1する）
...
#define	BOOT_AI_X		(16)
#define	BOOT_AI_Y		(0)
#define	BOOT_AI_DATA_X	(16)
#define	BOOT_AI_DATA_Y	(16)
#define	BOOT_AI_DATA_X2	(16+14*8)
...
BOOL	DebugBattleMain(BATTLE_WORK *bw,BOOL *flag)
{
	...
	case 1:
		...
		if(sys.cont == BATTLE_RESTART_KEY){
			dbp->seq_no=3;
		}
		break;
	case 3:
...
BOOL	DebugBattlePokemonReshuffle(BATTLE_WORK *bw,int client_no,int *pos)
{
	...
	switch(dbp->reshuffle_seq_no[client_no]){
	case 0:
		if(((bw->server_param->client_status[client_no]&CLIENT_STATUS_POKE_RESHUFFLE)==0)||
		   ((dbp->debug_flag&DEBUG_FLAG_RESHUFFLE_CONTROL)==0)){
			return TRUE;
		}
		if(dbp->reshuffle_mode){
			break;
		}
		DebugBattleInit(bw);
		dbp->reshuffle_sel_mons[client_no]=*pos;
		DB_ClientTypePut(bw,dbp,client_no);
		DB_PokemonPut(bw,dbp,client_no,*pos);
		dbp->reshuffle_seq_no[client_no]=1;
		dbp->reshuffle_mode=1;
		break;
	case 1:
		if(sys.trg & PAD_BUTTON_A){
			dbp->reshuffle_sel_mons[client_no]=DebugBattleSelMonsNoCalc(bw,dbp,client_no,dbp->reshuffle_sel_mons[client_no],1);
			DB_PokemonPut(bw,dbp,client_no,dbp->reshuffle_sel_mons[client_no]);
		}
		...
		if(sys.trg & PAD_BUTTON_START){
			dbp->reshuffle_seq_no[client_no]=2;
		}
		break;
	case 2:
		BmpMenuWinClear(dbp->win,WINDOW_TRANS_ON);
		...
		*pos=dbp->reshuffle_sel_mons[client_no];
		dbp->reshuffle_seq_no[client_no]=0;
		dbp->reshuffle_mode=0;
...
//		DB_RemainHeapPut(bw,dbp);
		...
		DB_RandomSeedPut(bw,dbp,MODE_NORMAL);
		break;
	case SEQ_STATUS:
...
static	void	DebugBattleSequence(BATTLE_WORK *bw,DEBUG_BATTLE_PARAM *dbp)
{
	...
	case SEQ_YUBIHURU_DEBUG:
		...
		break;
	case SEQ_BOOT_AI:
		DebugBattleBootAISequence(bw,dbp);
		break;
	}
}
...
static	void	DebugBattleAddStatusSequence(BATTLE_WORK *bw,DEBUG_BATTLE_PARAM *dbp)
{
	...
	case 0:
		...
		break;
	case 1:
		DB_RandomSeedPut(bw,dbp,MODE_ACTIVE);
		dbp->sub_seq_no++;
	case 2:
		switch(DebugBattleHexNumCalc(dbp,&bw->rand_temp,8,0,0xffffffff,MODE_LOOP)){
		case NUM_CALC_NONE:
			break;
...
static	void	DebugBattleCommandSequence(BATTLE_WORK *bw,DEBUG_BATTLE_PARAM *dbp)
{
	...
	case 0:
		if(sys.trg & PAD_BUTTON_ALL){
			if(bw->fight_type&FIGHT_TYPE_AI){
				do{
					dbp->client_no++;
				...
				if((ST_ServerReshuffleCheck(bw,sp,dbp->client_no)==TRUE)&&
				   (ST_EscapeNGCheck(bw,sp,dbp->client_no,NULL)==FALSE)){
					...
					sp->client_act_work[dbp->client_no][ACT_PARA_WORK]=
						DebugBattleSelMonsNoCalc(bw,dbp,dbp->client_no,6,NULL);
...
static	void	DebugBattleBootAISequence(BATTLE_WORK *bw,DEBUG_BATTLE_PARAM *dbp)
{
	if(sys.trg & (PAD_BUTTON_A|PAD_BUTTON_X)){
		dbp->client_no++;
		if(BattleWorkClientSetMaxGet(bw)<=dbp->client_no){
			dbp->client_no=0;
		}
		DB_BootAIPut(bw,dbp);
	}
	if(sys.trg & (PAD_BUTTON_B|PAD_BUTTON_Y)){
		dbp->client_no--;
		if(dbp->client_no<0){
			dbp->client_no=BattleWorkClientSetMaxGet(bw)-1;
		}
		DB_BootAIPut(bw,dbp);
	}
}
...
static	void	DebugBattlePokemonSelect(BATTLE_WORK *bw,DEBUG_BATTLE_PARAM *dbp)
{
	...
	if(sys.trg & PAD_BUTTON_A){
		...
		sp->client_act_work[dbp->client_no][ACT_PARA_WORK]=DebugBattleSelMonsNoCalc(bw,dbp,dbp->client_no,pos,1);
		sp->reshuffle_sel_mons_no[dbp->client_no]=sp->client_act_work[dbp->client_no][ACT_PARA_WORK];
		DB_PokemonPut(bw,dbp,dbp->client_no,sp->reshuffle_sel_mons_no[dbp->client_no]);
	}
	if(sys.trg & PAD_BUTTON_B){
		pos=sp->client_act_work[dbp->client_no][ACT_PARA_WORK];
		sp->client_act_work[dbp->client_no][ACT_PARA_WORK]=DebugBattleSelMonsNoCalc(bw,dbp,dbp->client_no,pos,-1);
...
static	void	DB_RandomSeedPut(BATTLE_WORK *bw,DEBUG_BATTLE_PARAM *dbp,int mode)
{
	...
	DebugBattleHexNumPut(dbp,REMAIN_HEAP_X+12*8,REMAIN_HEAP_Y,bw->rand_temp,8,dbp->cur_pos_keta,mode);

	MSGMAN_GetString(dbp->msg_m,DB_RAND_SEED,dbp->msg_buf);
	GF_STR_PrintColor(dbp->win,FONT_SYSTEM,dbp->msg_buf,REMAIN_HEAP_X,REMAIN_HEAP_Y,0,COL_NORMAL,NULL);
}
 ...
 * @param	bw			戦闘用パラメータ構造体
 * @param	dbp			戦闘デバッグ用パラメータ構造体
 * @param	client_no	ClientTypeを表示するクライアントナンバー
 */
//--------------------------------------------------------------
static	void	DB_ClientTypePut(BATTLE_WORK *bw,DEBUG_BATTLE_PARAM *dbp,int client_no)
{
	...
	//ClientType
	MSGMAN_GetString(dbp->msg_m,DB_CLIENT_TYPE_AA+BattleWorkClientTypeGet(bw,client_no),dbp->msg_buf);
...
static	void	DB_CommandDataPut(BATTLE_WORK *bw,DEBUG_BATTLE_PARAM *dbp)
{
	...
	case SERVER_FIGHT_COMMAND_NO:
		...
		break;
	case SERVER_POKEMON_COMMAND_NO:
		DB_PokemonPut(bw,dbp,dbp->client_no,sp->client_act_work[dbp->client_no][ACT_PARA_WORK]);
		break;
	}
}
 ...
 * @param	bw			戦闘用パラメータ構造体
 * @param	dbp			戦闘デバッグ用パラメータ構造体
 * @param	client_no	入れ替え対象クライアントナンバー
 * @param	pos			入れ替え対象位置
 */
//--------------------------------------------------------------
static	void	DB_PokemonPut(BATTLE_WORK *bw,DEBUG_BATTLE_PARAM *dbp,int client_no,int pos)
{
	...
	pp=BattleWorkPokemonParamGet(bw,client_no,pos);
...
static	void	DB_BootAIPut(BATTLE_WORK *bw,DEBUG_BATTLE_PARAM *dbp)
{
	...
	GF_BGL_BmpWinFill(dbp->win,0x0f,BOOT_AI_X,BOOT_AI_Y,28*8,16*8);

	DB_ClientTypePut(bw,dbp,dbp->client_no);

	MSGMAN_GetString(dbp->msg_m,DB_BOOT_AI,dbp->msg_buf);
	GF_STR_PrintColor(dbp->win,FONT_SYSTEM,dbp->msg_buf,BOOT_AI_X+2*8,BOOT_AI_Y,0,COL_NORMAL,NULL);

	msg_m=MSGMAN_Create(MSGMAN_TYPE_DIRECT,ARC_MSG,NARC_msg_debug_fight_dat,HEAPID_BATTLE);
	ai_bit=bw->trainer_data[dbp->client_no].aibit&0x000007ff;
	ai_no=0;
	cnt=0;

	while(ai_bit){
		if(ai_bit&1){
			MSGMAN_GetString(msg_m,DF_BASIC_AI+ai_no,dbp->msg_buf);
			if(cnt<7){
				GF_STR_PrintColor(dbp->win,FONT_SYSTEM,dbp->msg_buf,BOOT_AI_DATA_X,BOOT_AI_DATA_Y+16*cnt,0,COL_NORMAL,NULL);
			}
			else{
				GF_STR_PrintColor(dbp->win,FONT_SYSTEM,dbp->msg_buf,BOOT_AI_DATA_X2,BOOT_AI_DATA_Y2+16*(cnt-7),0,COL_NORMAL,NULL);
			}
			cnt++;
		}
		ai_bit=ai_bit>>1;
...
static	void	DebugBattleHexNumPut(DEBUG_BATTLE_PARAM *dbp,int x,int y,int param,int keta,int pos,int mode)
{
	...
	ofs=0;
	for(i=0;i<keta;i++){
		data=(param&0xf0000000)>>28;
		param=param<<4;
		STRBUF_SetHexNumber(dbp->msg_buf,data,1,NUMBER_DISPTYPE_ZERO,NUMBER_CODETYPE_DEFAULT);
		if(mode==MODE_NORMAL){
			GF_STR_PrintColor(dbp->win,FONT_SYSTEM,dbp->msg_buf,x+ofs,y,MSG_NO_PUT,COL_NORMAL,NULL);
		}
		else{
