...
void PokeStatus_SoftSpriteSet( PST_WORK * wk )
{
	...
	if( wk->dat->ppt == PST_PP_TYPE_POKEPASO ){
		PokeGraArcDataGetPPP( &ssa, pp, PARA_FRONT, FALSE );
	}else{
...
//	PokeAnmDataSet( wk->p3d.ssanm, wk->pp.mons );
	...
	PokeEditData_AnimeSet( wk->handle, wk->p3d.ssanm, wk->pp.mons, CLIENT_TYPE_BB );	

//	wk->p3d.flip = PokePersonalParaGet( wk->pp.mons, ID_PER_reverse ) ^ 1;
	wk->p3d.flip = PokeFormNoPersonalParaGet( wk->pp.mons, wk->pp.form, ID_PER_reverse ) ^ 1;
...
//		PokePrgAnmDataSet( wk->p3d.poke_anm_sys, wk->p3d.ss, 0, PARA_FRONT, 0, wk->p3d.flip, 0 );
		...
		PokeEditData_PrgAnimeSet( wk->handle,
								  wk->p3d.poke_anm_sys,
								  wk->p3d.ss,
								  0,
								  PARA_FRONT,
