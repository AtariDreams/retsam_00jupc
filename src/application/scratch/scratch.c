...
static void Scratch_InitTouchSub( SCRATCH_WORK* wk )
{
	...
	//cp_param.char_offs	= (1024 - 64);			//キャラクタ転送オフセット((キャラクタ単位 1/32byte)
	cp_param.char_offs	= (1024 - 128);			//キャラクタ転送オフセット((キャラクタ単位 1/32byte)
	cp_param.pltt_offs	= SCRATCH_TOUCH_SUB_PAL;//パレット転送オフセット(パレット1本分単位 1/32byte)
	cp_param.x			= 24;					//x座標(キャラクタ単位)
	cp_param.y			= 8;					//y座標(キャラクタ単位)
...
static u8 ScratchWriteMsg_ov111_21D2424( SCRATCH_WORK* wk, GF_BGL_BMPWIN* win, int msg_id, u32 x, u32 y, u32 wait, u8 f_col, u8 s_col, u8 b_col, u8 font )
{
    GF_BGL_BmpWinDataFill(win, b_col);
    MSGMAN_GetString(wk->msgman, msg_id, wk->tmp_buf);
    WORDSET_ExpandStr(wk->wordset, wk->msg_buf, wk->tmp_buf);
    x -= (FontProc_GetPrintStrWidth(font, wk->msg_buf, 0) + 1) / 2;
    return GF_STR_PrintColor( win, font, wk->msg_buf,
                              x, y, wait,
                              GF_PRINTCOLOR_MAKE(f_col,s_col,b_col), NULL );
}
