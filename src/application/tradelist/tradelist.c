...
// ダメタマゴチェックの結果に使う
enum{
TRADE_NO_DAMETAMAGO=0,		// ダメタマゴは存在しない
TRADE_DAMETAMAGO_MINE,		// 自分がダメタマゴを持っている
TRADE_DAMETAMAGO_FRIEND,	// 相手がダメタマゴを持っている
};
...
static const u8 movepostbl[][4][6]={
									//   0 の位置からの4方向の行きつく先の優先度テーブル(上下左右の順)
									//   少ない数字の方が優先度が高い
...
static int DameTamagoCheck( TRADELIST_WORK *wk );
...
static void _trade_ok_command( TRADELIST_WORK *wk );
static void _trade_print( TRADELIST_WORK *wk, int winno, int msgno );
...
//	}
	...
	if( GF_RTC_IsNightTime() == FALSE ){			//FALSE=昼、TRUE=夜
		...
		Snd_VChatVolSetBySeqNo( SEQ_PC_01 );		//ボイスチャット中の音量操作(08.05.01)
	}else{
		Snd_DataSetByScene( SND_SCENE_FIELD, SEQ_PC_02, 1 );
		Snd_VChatVolSetBySeqNo( SEQ_PC_02 );		//ボイスチャット中の音量操作(08.05.01)
	}
...
PROC_RESULT TradeListProc_Main( PROC * proc, int * seq )
{
	...
	case SEQ_MAIN:
#ifdef PM_DEBUG
		// デバッグボタンを押すとダメタマゴになります
		if(sys.trg&PAD_BUTTON_DEBUG){
			POKEMON_PARAM *pp;
			int *tmp;
			pp = PokeParty_GetMemberPointer( wk->MyPokeParty, 0 );
			tmp = (int*)pp;
			tmp[20] = 0xffffffff;
			tmp[21] = 0xffffffff;
		}
...
static void PokemonDataExchange(POKEPARTY *my, POKEPARTY *your, int mysel, int yoursel, TRADELIST_PARAM *param)
{
	...
	if(PokeParaGet(pp2, ID_PARA_monsno, NULL) == MONSNO_AUSU){
		if(PokeParaGet(pp2, ID_PARA_event_get_flag, NULL) || 
				...
				&& PokeParaGet(pp2, ID_PARA_event_get_flag, NULL) == 0)){
			EVENTWORK* ev = SaveData_GetEventWork(param->savedata);
...
//------------------------------------------------------------------
/**
 * @brief   自分と相手の手持ちにダメタマゴが無いか？
 *
 * @param   wk		
 *
 * @retval  int		TRADE_DAMETAMAGO_FRIEND 
 *					TRADE_DAMETAMAGO_MINE 
 * 					TRADE_NO_DAMETAMAGO
 */
//------------------------------------------------------------------
static int DameTamagoCheck( TRADELIST_WORK *wk )
{
	...
	for(i=0;i<pokenum;i++){
		...
		if(PokeParaGet( pp, ID_PARA_fusei_tamago_flag, NULL)){
			return TRADE_DAMETAMAGO_MINE;
		}
	}
	
	
	// 相手の手持ちポケモンのダメタマゴフラグをチェック
	pokenum = PokeParty_GetPokeCount( wk->FriendPokeParty );
	for(i=0;i<pokenum;i++){
		pp = PokeParty_GetMemberPointer( wk->FriendPokeParty, i );
		if(PokeParaGet( pp, ID_PARA_fusei_tamago_flag, NULL)){
			return TRADE_DAMETAMAGO_FRIEND;
		}
	}
	
	// OK
	return TRADE_NO_DAMETAMAGO;
}
