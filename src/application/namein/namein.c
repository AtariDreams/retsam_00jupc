...
static void BgGraphicSet( NAMEIN_WORK * wk, ARCHANDLE* p_handle );
...
static void InitCellActor(NAMEIN_WORK *wk, ARCHANDLE* p_handle);
...
static int NameInputMainMode( NAMEIN_WORK *wk, int seq )
{
			...
			if( sys.trg & PAD_BUTTON_SELECT){
				if ( CLACT_GetDrawFlag( wk->clActWork[8] ) == 0 ){	///< goto
...
#define RIVAL_NAME_NO2			( msg_opening_rival_pearl_01 )
...
static int DecideMainButton(NAMEIN_WORK *wk,u16 code, int pad)
{
	...
	if ( CLACT_GetDrawFlag( wk->clActWork[8] ) == 0  && sys.tp_trg == FALSE ){	///< goto
		CLACT_SetDrawFlag( wk->clActWork[8], 1);
		return SEQ_MAIN;
	}
