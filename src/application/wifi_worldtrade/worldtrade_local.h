...
typedef struct _WORLDTRADE_WORK{
	...
	MSGDATA_MANAGER *CountryNameManager;					// 国名メッセージデータマネージャー
	...
	GF_BGL_BMPWIN			InfoWin[11+2];	//プラチナで2個追加
...
extern void WorldTrade_PokeInfoPrint( 	MSGDATA_MANAGER *MsgManager,
							...
							Dpw_Tr_PokemonDataSimple *post );
extern void WorldTrade_PokeInfoPrint2( MSGDATA_MANAGER *MsgManager, GF_BGL_BMPWIN win[], STRCODE *name, POKEMON_PARAM *pp, GF_BGL_BMPWIN oya_win[] );
...
extern BMPLIST_WORK *WorldTrade_LevelListMake(BMP_MENULIST_DATA **menulist, GF_BGL_BMPWIN *win, MSGDATA_MANAGER *MsgManager, int tbl_select  );
...
extern BMPLIST_WORK *WorldTrade_CountryListMake(BMP_MENULIST_DATA **menulist, GF_BGL_BMPWIN *win, MSGDATA_MANAGER *CountryMsgManager, MSGDATA_MANAGER *NormalMsgManager);

extern void WorldTrade_PokeNamePrint( GF_BGL_BMPWIN *win, MSGDATA_MANAGER *nameman, int monsno, int flag, int y, GF_PRINTCOLOR color );
