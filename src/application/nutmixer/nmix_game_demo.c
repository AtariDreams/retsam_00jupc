 ...
 */
//-----------------------------------------------------------------------------
void NMixGameDemo_SubWin_GraphicLoad( NMIX_DEMO_SUB_WIN* p_work, GF_BGL_INI* p_bgl, u32 heapID, ARCHANDLE* p_handle )
{
	GF_ASSERT( p_work->res_load == FALSE );
...
static void NMixMainWin_BackGroundInit( NMIX_DEMO_MAIN_WIN* p_work, u32 heapID, ARCHANDLE* p_handle )
{
	...
	ArcUtil_HDL_PalSet( p_handle, NARC_nutmixer_cook_s_window_NCLR, 
			...
			1*32, heapID );
	ArcUtil_HDL_BgCharSet( p_handle, NARC_nutmixer_cook_s_window_NCGR,
...
{
	if( p_work->p_timewait != NULL ){
		TimeWaitIconDel( p_work->p_timewait );
		p_work->p_timewait = NULL;
	}
}
...
static void NMixGameDemo_MainWin_ResultSeen1( NMIX_DEMO_MAIN_WIN* p_work )
{
	...
	// 回数によって単位の単複を出し分け
	{
		...
		WORDSET_ExpandStr( p_work->p_msg_data->wset, p_work->p_msg_data->tmp, p_work->p_msg_data->res[msg_idx] );	// 展開
	}
	// ----------------------------------------------------------------------------
...
void NMixGameDemo_SoopEffect_Add( NMIX_DEMO_SOOP_EFFECT* p_work, int soop_type, int eff_type )
{
	...
	if( (soop_type*3+eff_type) >= EFF_SOOP_DATA_NUM ){
		return ;
	}
