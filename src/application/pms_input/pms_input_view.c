...
static void setup_obj_graphic( COMMAND_WORK* cwk, ARCHANDLE* p_handle )
{
	...
	ArcUtil_HDL_PalSysLoad(p_handle, NARC_pmsi_obj_main_nclr, NNS_G2D_VRAM_TYPE_2DMAIN, 0, 
			HEAPID_PMS_INPUT_VIEW, &(vwk->obj_pltt_proxy[PMSIV_LCD_MAIN]));


	vwk->cell_load_ptr[PMSIV_LCD_MAIN] = ArcUtil_HDL_CellBankDataGet(p_handle, NARC_pmsi_obj_main_lz_ncer, TRUE,
				&(vwk->cellbank[PMSIV_LCD_MAIN]), HEAPID_PMS_INPUT_VIEW );

	vwk->anm_load_ptr[PMSIV_LCD_MAIN] = ArcUtil_HDL_AnimBankDataGet(p_handle, NARC_pmsi_obj_main_lz_nanr, TRUE,
