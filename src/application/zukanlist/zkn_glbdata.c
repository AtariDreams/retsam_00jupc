...
{
	return ZukanWork_GetPokeForm( cp_glb->cp_zukansave_data, monsno, 0 );
#if 0
	switch( monsno ){
	case MONSNO_ANNOON:
...
{
	return ZukanWork_GetPokeForm( cp_glb->cp_zukansave_data, monsno, count );
}

//----------------------------------------------------------------------------
/**
 *	@brief	ポケモンの見たフォルム数取得
 *
 *	@param	cp_glb		グローバルデータ
 *	@param	monsno		モンスターナンバー
 *
...
{
	return ZukanWork_GetPokeFormNum( cp_glb->cp_zukansave_data, monsno );
}
...
{
	u16* p_buff0;	// 検索元データ用テーブルバッファ
	int count0;
	u16* p_buff1;	// 検索結果受け取り用テーブルバッファ
	int count1;
	u16* p_all_list_data;	// ソート前全ポケモンリストデータ
 ...
 *	@retval ZKN_TCHCUSO_CURSOR_FAST		カーソル開始
 */
//-----------------------------------------------------------------------------
 ...
 *
 *	@return	データバッファ		要素数はp_arry_numに格納
 */
//-----------------------------------------------------------------------------
static u16* ZKN_SORTDATA_Get( int heap, int idx, int* p_arry_num )
{
...
//----------------------------------------------------------------------------
/**
 *	@brief	ポケモンリスト内の見つけた数　発見した数をカウント
 *
 *	@param	p_poke_list		ポケモンリストテーブル
