...
enum{
	...
	ZKN_ZUKANGRAVER_DRAW_TYPE_FLG_SYEIMI_POKE,// シェイミ描画タイプ
	ZKN_ZUKANGRAVER_DRAW_TYPE_FLG_GIRATYINA_POKE,// ギラティナ描画タイプ
	ZKN_ZUKANGRAVER_DRAW_TYPE_FLG_ROTOM_POKE,// ロトム描画タイプ
...
#define ZKN_ZUKANGRAVER_GRA_DRAW_FRONT_X	( 76 )
#define ZKN_ZUKANGRAVER_GRA_DRAW_BACK_X_OFS	( 104 )
#define ZKN_ZUKANGRAVER_GRA_DRAW_FRONT_NEXT_X	( ZKN_ZUKANGRAVER_GRA_DRAW_FRONT_X + 256 )

// FONTOAM
#define ZKN_GRAPHIC_BUTTONFONT_NUM			( 2 )
#define ZKN_GRAPHIC_BUTTONFONT_BMP_SIZE_CX ( 16 )
...
static void ZknZukanGraverPokeGraDrawOnTypeRotom( ZKN_ZUKANGRAVER_DRAW* p_draw, ZKN_ZUKANGRAVER_DRAWGLB* p_drawglb, const ZKN_ZUKANGRAVER_GLB* cp_glb, int monsno, int draw_type, int s_idx );
...
static void ZknZukanGraverPoke_GraSet( ZKN_ZUKANGRAVER_DRAW* p_draw, ZKN_ZUKANGRAVER_DRAWGLB* p_drawglb, const ZKN_ZUKANGRAVER_GLB* cp_glb, int monsno, int draw_type_flag, int draw_type, int idx )
{
	...
	case ZKN_ZUKANGRAVER_DRAW_TYPE_FLG_SYEIMI_POKE:// シェイミ描画タイプ
		ZknZukanGraverPokeGraDrawOnTypeSheimi( p_draw, p_drawglb, cp_glb, monsno, draw_type, idx );
		break;
		
	case ZKN_ZUKANGRAVER_DRAW_TYPE_FLG_GIRATYINA_POKE:// ギラティナ描画タイプ
		ZknZukanGraverPokeGraDrawOnTypeGiratyina( p_draw, p_drawglb, cp_glb, monsno, draw_type, idx );
		break;
		
	case ZKN_ZUKANGRAVER_DRAW_TYPE_FLG_ROTOM_POKE:// ロトム描画タイプ
...
static int ZknZukanGraverPokeDrawTypeFlagGet( int monsno, const ZKN_ZUKANGRAVER_GLB* cp_glb )
{
	...
	case MONSNO_EURISU:
		return ZKN_ZUKANGRAVER_DRAW_TYPE_FLG_SYEIMI_POKE;

	case MONSNO_KIMAIRAN:
		return ZKN_ZUKANGRAVER_DRAW_TYPE_FLG_GIRATYINA_POKE;

	case MONSNO_PURAZUMA:
		return ZKN_ZUKANGRAVER_DRAW_TYPE_FLG_ROTOM_POKE;
...
static int ZknZukanGraverPokeDrawTypeNumGet( int monsno, const ZKN_ZUKANGRAVER_GLB* cp_glb )
{
	...
	case MONSNO_KIMAIRAN:
	case MONSNO_PURAZUMA:
		return ZKN_GLBDATA_SaveData_FormNumGet( cp_glb->p_glb, monsno );
...
static void ZknZukanGraverFontSetUpGraphicType( ZKN_ZUKANGRAVER_DRAW* p_draw, ZKN_ZUKANGRAVER_DRAWGLB* p_drawglb, const ZKN_ZUKANGRAVER_GLB* cp_glb, int heap, int draw_type_flag, int draw_type, int parent_idx, int fontoam_idx )
{
	...
	// comment
	ZKN_FONTOAM_PrintBmpStrCenter( p_glb_draw->fontoam_sys, win,
			NARC_msg_zkn_dat, comment );
...
{
	int comment;
	int form = ZKN_GLBDATA_SaveData_FormGet( cp_glb->p_glb, MONSNO_EURISU, draw_type );
	comment = ZKN_FORMNAME_00 + form;
	return comment;
}
...
{
	int comment;
	int form = ZKN_GLBDATA_SaveData_FormGet( cp_glb->p_glb, MONSNO_KIMAIRAN, draw_type );
	comment = ZKN_FORMNAME_02 + form;
	return comment;
}
...
{
	int comment;
	int form = ZKN_GLBDATA_SaveData_FormGet( cp_glb->p_glb, MONSNO_PURAZUMA, draw_type );
	comment = ZKN_FORMNAME_04 + form;
	return comment;
}
