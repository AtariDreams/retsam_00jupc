...
typedef struct _ZKN_FIGHT_TEXT_DRAW{
	...
	ZKN_FIGHT_TYPE poke_type;	// ポケモンタイプワーク
...
static void ZknFight_BackGroundSetUp( GF_BGL_INI* p_bg, int heap, ARCHANDLE* p_handle );
static void ZknFight_BackUtilSetUp( GF_BGL_INI* p_bg, int heap, ARCHANDLE* p_handle );
...
ZKN_FIGHT_TEXT_DRAW* ZKN_FIGHT_TextDrawReq( const ZKN_FIGHT_TEXT_PARAM* cp_param )
{
	...
	// ギラティナは絶対にFORMNo0
	{
		...
		monsno	= PokeParaGet( cp_param->p_pp, ID_PARA_monsno, NULL ); 
		form	= PokeParaGet( cp_param->p_pp, ID_PARA_form_no, NULL );
...
//*/
	...
	{
		int foot_monsno;
		// ギラティナオリジンフォルムのときだけ、ダミーとして足跡のないポケモンの足跡を出す
		if((p_zftd->monsno == MONSNO_KIMAIRAN) && (form_no > 0)){
			foot_monsno = MONSNO_TORANSERU;
		}else{
			foot_monsno = p_zftd->monsno;
		}
	
		ZknFight_PokeFootResLoad( &p_zftd->pokefoot, p_zftd->res_manager, foot_monsno, cp_param->heap );
...
/*		GF_Disp_GX_VisibleControl( GX_PLANEMASK_BG0, VISIBLE_ON );
	...
	case ZKN_FIGHT_SND_PLY:
		check = Snd_PMVoicePlayEx( PV_ZUKAN_NORMAL, p_zftd->monsno, PV_PTN_PARAM_NONE, 
								   PV_PTN_PARAM_NONE, PV_PTN_PARAM_NONE, 0 );	//ぺラップ(default)
		GF_ASSERT( check );
		p_zftd->seq++;
		break;
...
static void ZknFight_TransPalAnm( ZKN_PALANM_WORK* p_data, int frm )
{
	DC_FlushRange( (void *)p_data->p_plt[ frm ]->pRawData, ZKN_FIGHT_PALANN_PAL_NUM*32 );
	GX_LoadBGPltt(
