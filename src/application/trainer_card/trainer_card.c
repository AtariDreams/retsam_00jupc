...
#define TRCLOR_PLTT_OVERWRITE_TRANS	( UNI_TRAINER_PLTT_NO*32 )	// 上書きパレット転送先
#define TRCLOR_PLTT_OVERWRITE_SIZE	( 2*32 )	// 上書きパレットサイズ
...
static void SetTrCardBgGraphic( TR_CARD_WORK * wk, ARCHANDLE* p_handle )
{
	...
	if (wk->TrCardData->UnionTrNo == UNION_TR_NONE){
		...
		case VERSION_PEARL:
			...
			nsc_data = NARC_trainer_case_card_trainer01_dp_NSCR;
		...
		default:
			...
			ncl_data = TRCLOR_PLTT_OVERWRITE_NONE;
			break;
		}
