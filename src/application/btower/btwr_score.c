...
static int BTowerApp_KeyInLeader(BTOWER_APP_WORK* wk)	
{
	...
	if(sys.trg & PAD_BUTTON_DECIDE){
		...
		if(sys.repeat & PAD_KEY_UP){
			...
			wk->side = (wk->side+2)%3;
			update = 1;
			if(wk->line != LINE_RET){					//無条件でupdate = 1になるため
				Snd_SePlay( SEQ_SE_DP_SELECT );
			}
...
static void BTowerApp_MsgWriteScore(BTOWER_APP_WORK* wk)
{
	...
	case 2:	//wifi
		...
		tapp_ScoreRenshouWrite(wk,&wk->win[WINS_REC02],4,FRID_TOWER_WIFI_DL_RENSHOU,3);
		break;
	}
}
