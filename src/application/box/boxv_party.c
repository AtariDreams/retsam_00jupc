 ...
 */
//------------------------------------------------------------------
BOOL BoxAppView_PartyInit( PARTY_VIEW_WORK* wk, BOXAPP_VIEW_WORK* vwk, const BOXAPP_VPARAM* vpara, GF_BGL_INI* bgl, CLACT_SET_PTR actsys, ARCHANDLE* p_boxgra_handle )
{
	wk->vwk = vwk;
	wk->vpara = vpara;
	wk->actsys = actsys;
	...
	wk->scrnBuf = ArcUtil_HDL_ScrnDataGet( p_boxgra_handle, NARC_box_bg1_party_lz_nscr, TRUE, &(wk->scrnData), HEAPID_BOX_VIEW );
