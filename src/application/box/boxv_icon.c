...
//--------------------------------------------------------------
/**
 * @brief   アイコンの再描画を行う(実質ギラティナからハッキンダマを取り上げた時とシェイミを掴んでトレイに移動して来た時専用の処理)
 *
 * @param   iconSys		
 * @param   icon		
 */
...
void BoxAppView_PokeIconRewrite_VramTrans(BOX_ICON_SYS* iconSys, BOX_ICON_WORK *icon, BOOL a2)
{
	...
	//所持アイテムとフォルムが一致しない場合のみ書き換え処理を行う
	...
	dest_vram = icon->imgProxy.vramLocation.baseAddrOfVram[NNS_G2D_VRAM_TYPE_2DMAIN] 
		+ (u32)G2_GetOBJCharPtr();
	
	ArchiveDataLoadOfsByHandle( iconSys->arcHandle, icon->imgArcIdx, 0, BOXICON_CHARBUF_SIZE, iconSys->charbuf );
	NNS_G2dGetUnpackedCharacterData( iconSys->charbuf, &charData );
	MI_CpuCopy32(charData->pRawData, (void*)dest_vram, BOXICON_CHAR_SIZE*0x20);
}
