...
enum {
	...
	TRAYNAME_WIN_HEIGHT = 17, // MatchComment: 16 -> 17
...
static void setup_actors( REEL_VIEW_WORK* wk, ARCHANDLE* p_boxgra_handle );
static void load_actor_datas( REEL_VIEW_WORK* wk, ARCHANDLE* p_boxgra_handle );
...
static void setup_blend_alpha_table( REEL_VIEW_WORK* wk )
{
	...
	{
		s1 += sval;
		wk->blendAlphaEv1[i] = (_Sin360(s1>>FX32_SHIFT) * 16) >>FX32_SHIFT;
 ...
 */
//======================================================================================
void BoxAppView_Reel_Setup( REEL_VIEW_WORK* wk, ARCHANDLE* p_boxgra_handle )
{
	if( wk )
	{
		ArcUtil_HDL_BgCharSet(p_boxgra_handle, NARC_box_sub_bg_lz_ncgr, wk->bgl, FRM_SUB_BACK, 0, 0, TRUE, HEAPID_BOX_VIEW);
		ArcUtil_HDL_ScrnSet(p_boxgra_handle, NARC_box_sub_bg3_lz_nscr, wk->bgl, FRM_SUB_BACK, 0, 0, TRUE, HEAPID_BOX_VIEW);
		ArcUtil_HDL_ScrnSet(p_boxgra_handle, NARC_box_sub_bg2_lz_nscr, wk->bgl, FRM_SUB_ICONLINE, 0, 0, TRUE, HEAPID_BOX_VIEW);
		ArcUtil_HDL_ScrnSet(p_boxgra_handle, NARC_box_sub_bg1_lz_nscr, wk->bgl, FRM_SUB_REEL, 0, 0, TRUE, HEAPID_BOX_VIEW);
		...
		ArcUtil_HDL_PalSet( p_boxgra_handle, NARC_box_sub_bg_nclr, PALTYPE_SUB_BG, 0, 0x20*USE_PAL_NUMS, HEAPID_BOX_VIEW );

		setup_actors( wk, p_boxgra_handle );
	}
}
...
static void setup_actors( REEL_VIEW_WORK* wk, ARCHANDLE* p_boxgra_handle )
{
	...
	load_actor_datas( wk, p_boxgra_handle );

	NNS_G2dInitImageProxy( &imgProxy );
	ArcUtil_HDL_CharSysLoad( p_boxgra_handle, NARC_box_sub_button_lz_ncgr, TRUE, CHAR_MAP_1D, 0, NNS_G2D_VRAM_TYPE_2DSUB,
...
static void load_actor_datas( REEL_VIEW_WORK* wk, ARCHANDLE* p_boxgra_handle )
{
	wk->buttonCellBuf = ArcUtil_HDL_CellBankDataGet( p_boxgra_handle, NARC_box_sub_button_lz_ncer, TRUE,
									&(wk->buttonCellData), HEAPID_BOX_VIEW );

	wk->buttonAnimBuf = ArcUtil_HDL_AnimBankDataGet( p_boxgra_handle, NARC_box_sub_button_lz_nanr, TRUE,
									&(wk->buttonAnimData), HEAPID_BOX_VIEW );

	wk->iconCellBuf = ArcUtil_HDL_CellBankDataGet( p_boxgra_handle, NARC_box_sub_icon_lz_ncer, TRUE,
									&(wk->iconCellData), HEAPID_BOX_VIEW );

	wk->iconAnimBuf = ArcUtil_HDL_AnimBankDataGet( p_boxgra_handle, NARC_box_sub_icon_lz_nanr, TRUE,
									&(wk->iconAnimData), HEAPID_BOX_VIEW );

//	ArcUtil_HDL_ObjCharSet( p_boxgra_handle, NARC_box_sub_button_lz_ncgr, OBJTYPE_SUB, OBJCHAR_SUB_BUTTON_POS*0x20,
//							0, TRUE, HEAPID_BOX_VIEW );

	NNS_G2dInitImagePaletteProxy( &(wk->objPlttProxy) );
	ArcUtil_HDL_PalSysLoad( p_boxgra_handle, NARC_box_sub_obj_nclr, NNS_G2D_VRAM_TYPE_2DSUB, 0, 
