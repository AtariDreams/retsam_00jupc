...
BOOL BoxAppView_CursorInit( CURSOR_VIEW_WORK* wk, BOXAPP_VIEW_WORK* vwk, const BOXAPP_VPARAM* vpara, CLACT_SET_PTR actsys, ARCHANDLE* p_boxgra_handle )
{
	...
	wk->cellBuf = ArcUtil_HDL_CellBankDataGet(p_boxgra_handle, NARC_box_m_hand_lz_ncer, TRUE, &(wk->cellBank), HEAPID_BOX_VIEW);
	wk->anmBuf = ArcUtil_HDL_AnimBankDataGet(p_boxgra_handle, NARC_box_m_hand_lz_nanr, TRUE, &(wk->anmBank), HEAPID_BOX_VIEW);
 ...
 */
//--------------------------------------------------------------------------------------
void BoxAppView_CursorActSet( CURSOR_VIEW_WORK* wk, ARCHANDLE* p_boxgra_handle )
{
	CLACT_HEADER  header;
	u32 area, mode;

	NNS_G2dInitImageProxy( &(wk->imgProxy) );
	ArcUtil_HDL_CharSysLoad( p_boxgra_handle, NARC_box_m_hand_lz_ncgr, TRUE, CHAR_MAP_1D, 0,
 ...
 */
//--------------------------------------------------------------
BOX_ICON_WORK * BoxAppView_CursorCatchIconPtrGet( CURSOR_VIEW_WORK* wk, int pos )
{
	return &wk->catch_icon[pos];
}
