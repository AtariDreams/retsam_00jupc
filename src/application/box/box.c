...
static BOOL CursorPointPokeUpdate( BOXAPP_WORK* wk );
...
static void SubSeq_Menu_Item( BOXAPP_WORK* wk, u32* seq )
{
	...
	case SEQ_DECIDE_MENU:
		...
		case BOXMENU_ITEM_SET:		///< アイテム「もたせる」
			...
			else if( BoxAppVPara_GetCatchItemNumber( &wk->vpara ) != ITEM_DUMMY_ID )
			{
...
static void SubSeq_AreaSelect( BOXAPP_WORK* wk, u32* seq )
{
    ...
    case SEQ_TRAYCHANGE_WAIT:
        if( BoxAppView_WaitCommand( wk->vwk, CMD_TRAY_CHANGE ) )
        {
            if( !(BoxAppVPara_GetCursorCatchPokeFlag( &wk->vpara ) & CUROSR_CATCH_STATUS_HOLD_MASK) )
            {
...
static void SubSeq_Connect_Bag( BOXAPP_WORK* wk, u32 *seq )
{
	...
	case 4:
		if( BoxAppView_WaitCommand( wk->vwk, CMD_FADEIN ) )
		{
			...
			else if(select_itemno == ITEM_HAKKINDAMA && PokePasoParaGet( wk->vpara.statusPoke.poke_data, ID_PARA_monsno, NULL ) != MONSNO_KIMAIRAN){
...
static void VParaSet_PokePut( BOXAPP_WORK* wk, BOXAPP_VPARAM* vpara )
{
	...
	BOXAPP_CURSOR* cursor = &vpara->cursor;
	POKEMON_PASO_PARAM* ppp;
	int sheimi_form_change = 0, form_no;
	BOOL update_ret;
	
	if( BoxAppVPara_GetCursorArea( vpara ) == CURSOR_AREA_TRAY )
	{
		form_no = PokePasoParaGet(catchPoke->ppp, ID_PARA_form_no, NULL);
		...
		if(PokePasoParaGet(ppp, ID_PARA_monsno, NULL) == MONSNO_EURISU && form_no == FORMNO_SHEIMI_FLOWER){
			sheimi_form_change = TRUE;
		}
	}
	else
	{
...
//		ppp = catchPoke->ppp;
	...
	update_ret = CursorPointPokeUpdate( wk );
	if(update_ret == FALSE && sheimi_form_change == TRUE){
...
static void VParaSet_StatusPokeItemNumber( BOXAPP_VPARAM* vpara, u16 itemno, BOXAPP_WORK* wk )
{
...

	{
		...
		if( monsno == MONSNO_AUSU )
		{
			...
			stpoke->type1 = PokePasoParaGet( stpoke->poke_data, ID_PARA_type1, NULL );
