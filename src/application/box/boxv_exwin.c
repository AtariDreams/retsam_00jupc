...
BOOL BoxAppView_ExWinInit( EXWIN_VIEW_WORK* wk, BOXAPP_VIEW_WORK* vwk, const BOXAPP_VPARAM* vpara, GF_BGL_INI* bgl, CLACT_SET_PTR actsys, ARCHANDLE* p_boxgra_handle )
{
	...
	ArcUtil_HDL_BgCharSet( p_boxgra_handle, NARC_box_bg0_lz_ncgr, bgl, FRM_MAIN_MESSAGE, 0, 0, TRUE, HEAPID_BOX_VIEW );

	{
		NNSG2dPaletteData* palData;
		void* allocBuf;

		allocBuf = ArcUtil_HDL_PalDataGet(p_boxgra_handle, NARC_box_wp_image_nclr, &palData, HEAPID_BOX_VIEW);
		MI_CpuCopy16(palData->pRawData, &(wk->trayIconPalBuf[0][0]), 0x40);
		sys_FreeMemoryEz(allocBuf);

		allocBuf = ArcUtil_HDL_PalDataGet(p_boxgra_handle, NARC_box_tray_icon_nclr, &palData, HEAPID_BOX_VIEW);
	...
	wk->iconCharBuf = ArcUtil_HDL_CharDataGet( p_boxgra_handle, NARC_box_tray_icon_lz_ncgr, TRUE, &(wk->iconCharData), HEAPID_BOX_VIEW );
	wk->iconCellBuf = ArcUtil_HDL_CellBankDataGet( p_boxgra_handle, NARC_box_tray_icon_lz_ncer, TRUE, &(wk->iconCellData), HEAPID_BOX_VIEW );
	wk->itemwinScrnBuf = ArcUtil_HDL_ScrnDataGet( p_boxgra_handle, NARC_box_bg0a_lz_nscr, TRUE, &(wk->itemwinScrnData), HEAPID_BOX_VIEW );
	wk->jumpwinScrnBuf = ArcUtil_HDL_ScrnDataGet( p_boxgra_handle, NARC_box_bg0b_lz_nscr, TRUE, &(wk->jumpwinScrnData), HEAPID_BOX_VIEW );
...
// 壁紙イメージ色で塗りつぶす
	...
	color = OBJPAL_TRAYICON_WP*0x10 + wall_no;
