...
typedef struct{
    ...
    WMBssDesc sIRQBssDesc;  ///< 親機の情報を記憶している構造体 割り込みコピー用
...
static void _whInitialize(BOOL blink);
 ...
 * @param   blink		FALSE:LEDを点滅させない(受信専用モード)
 * @retval  none
 */
//==============================================================================

void CommMPInitialize(MYSTATUS* pMyStatus, BOOL blink)
{
    int i;
    ...
    _pCommMP->gameInfoOrg = (u32)sys_AllocMemory(HEAPID_COMMUNICATION, WM_SIZE_USER_GAMEINFO+32);
    _pCommMP->gameInfoBuff = (u16*)(32 - (_pCommMP->gameInfoOrg % 32) + _pCommMP->gameInfoOrg);   //32byteアライメント
...
//            DEBUG_MACDISP("デバッグが異なる場合は拾わないx\n",bssdesc);
    ...
    MI_CpuCopy8( bssdesc, &_pCommMP->sIRQBssDesc, sizeof(WMBssDesc));
    _pCommMP->bCatchBeacon = TRUE;

}

//==============================================================================
/**
 * ビーコンを移し変える関数 常に回っている
 * @param   bssdesc   グループ情報
 * @retval  none
 */
//==============================================================================

static void beaconMainLoop(void)
{
    WMBssDesc *bssdesc = &_pCommMP->sIRQBssDesc;
    int i;
    
    if(!_pCommMP->bCatchBeacon){
        return;
    }
    _pCommMP->bCatchBeacon = FALSE;
...
static void _whInitialize(BOOL blink)
{
    ...
    // 無線初期化
    {
        ...
        (void)WH_Initialize((void*)addr, blink);
#if T1657_060818_FIX
...
static void _commEnd(void)
{
    ...
    sys_FreeMemoryEz((void*)_pCommMP->gameInfoOrg);
...
//==============================================================================
/**
 * 自分のBCONのレギュレーションデータを得る
 * @param   none
 * @retval  pRegulation  レギュレーションデータ構造体のポインタ
 */
//==============================================================================

void* CommMPGetMyRegulation(void)
{
    return _pCommMP->pRegulation;
}
