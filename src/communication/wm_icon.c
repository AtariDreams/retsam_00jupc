...
static void IconOamTrans( VINTR_WIRELESS_ICON* work );
 ...
 */
//==============================================================================
VINTR_WIRELESS_ICON *AddWirelessIconOAM(u32 objVRAM, u32 HeapId, int x, int y, BOOL bWifi, const VOamAnm *tbl[], int vramType)
{
...
/**
 * アイコンパレットリソース取得
 * @param	heap_id	パレットリソースを確保するヒープID
 * @retval	void*	パレットデータ。 内容はアイコン用NCLRです。使用後は開放して下さい。
 */
//--------------------------------------------------------------
void * WirelessIconPlttResGet( u32 heap_id )
{
	void *pal = sys_AllocMemoryLo( heap_id, WM_ICON_PALFILE_SIZE );
...
//	        sys_LoadFileNoMem( "data/wifi.NCGR", &ncg );
	        sys_LoadFileNoMem( "data/pl_wifi.NCGR", &ncg );
	    }
	    else
	    {
//	        sys_LoadFileNoMem( "data/wm.NCGR", &ncg );
		...
		NNS_G2dGetUnpackedBGCharacterData( ncg, &ppCharData );   // 展開
		...
		case GX_OBJVRAMMODE_CHAR_1D_128K:
			...
			else if( objBank==GX_VRAM_OBJ_64_E )
			{
 ...
 */
//--------------------------------------------------------------
void WirelessIconOAM_Trans( void )
{
	VINTR_WIRELESS_ICON *vwi = VintrWirelessIconPtr;
...
void WirelessIconEasy(void)
{
    ...
    WirelessIconEasyXY(240, 0, bWifi, NNS_G2D_VRAM_TYPE_2DMAIN);
}

//==============================================================================
/**
 * $brief   超絶らくちん通信アイコン(ディスプレイ選択版)
 *
 * @param   vramtype 		NNS_G2D_VRAM_TYPE_2DMAIN or NNS_G2D_VRAM_TYPE_2DSUB
 *
 * @retval  none		
 */
//==============================================================================
void WirelessIconEasyDisp(int vramtype)
{
    BOOL bWifi = FALSE;
    ...
    WirelessIconEasyXY(240, 0, bWifi, vramtype);
}
...
void WirelessIconEasyXY(int x,int y, BOOL bWifi, int vramtype)
{
	...
	VintrWirelessIconPtr = AddWirelessIconOAM(0,HEAPID_COMMICON,x,y, bWifi, WM_IconAnimTbl, vramtype);
}
