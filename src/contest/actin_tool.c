...
void AT_SoftSpriteHensinPop(ACTIN_SYSTEM_PARAM *sys, int breeder_no)
{
	...
	//反転セット
	{
		...
		form_no = PokeParaGet(sys->c_game->pp[breeder_no], ID_PARA_form_no, NULL);
		if(ContestPokeFlipCheck(monsno, form_no) == TRUE){
...
void AT_SoftSpriteAddAll(ACTIN_SYSTEM_PARAM *sys)
{
	...
	for(i = 0; i < BREEDER_MAX; i++){
		...
		//反転セット
		{
			...
			form_no = PokeParaGet(sys->c_game->pp[i], ID_PARA_form_no, NULL);
			if(ContestPokeFlipCheck(monsno, form_no) == TRUE){
