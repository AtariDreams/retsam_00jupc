...
//		if( IsFinishedBrightnessChg(MASK_DOUBLE_DISPLAY) ){
	...
	case SEQ_MAIN:
		if(ConTool_ServerMineCheck(apw->consys) == TRUE){
			...
			else if(ret == ASRET_END && apw->main_end == TRUE){
				*seq = SEQ_OUT;
				MI_CpuClear8(&apw->local, sizeof(ACTIN_LOCAL_WORK));
		...
		if(apw->main_end == TRUE && CO_TransmitFlagCheck(&apw->cow) == FALSE 
				&& ConTool_ServerMineCheck(apw->consys) == FALSE){
			*seq = SEQ_OUT;
			MI_CpuClear8(&apw->local, sizeof(ACTIN_LOCAL_WORK));
			WIPE_SYS_Start(WIPE_PATTERN_FSAM, WIPE_TYPE_SCREWOUT, WIPE_TYPE_SCREWOUT, 
				WIPE_FADE_BLACK, WIPE_DEF_DIV, WIPE_DEF_SYNC, HEAPID_ACTIN);
...
static void ActinDefaultOBJSet(ACTIN_PROC_WORK *apw, ARCHANDLE *hdl_obj)
{
	...
	AT_NextIconResourceLoad(apw->sys.csp, apw->sys.crp, hdl_obj);
	
	AT_BreederPanelFontOamCreateAll(apw);
	AT_JudgeAddAll(&apw->sys, apw->consys, hdl_obj);
	AT_SpecialHeartSet(&apw->sys, apw->consys->c_game.special_judge_no, hdl_obj);
	
	AT_JudgeReactionIconResourceLoad(apw->sys.pfd, apw->sys.csp, apw->sys.crp, hdl_obj);
	AT_JudgeReactionIconActorAdd(&apw->sys);

	AT_AppealPointResourceLoad(apw->sys.csp, apw->sys.crp, hdl_obj);
	AT_VoltagePointResourceLoad(apw->sys.csp, apw->sys.crp, hdl_obj);
}
...
static void ActinDefaultOBJSet_Sub(ACTIN_PROC_WORK *apw, ARCHANDLE *hdl_obj)
{
	...
	ACIT_AppHeartResourceLoad(apw->sys.csp, apw->sys.crp, hdl_obj);
	ACIT_SubHeartJudgeResourceSet(apw->sys.csp, apw->sys.crp, hdl_obj);
