...
void RT_Result_OBJSet(CONRES_PROC_WORK *rpw)
{
	...
	RT_ConfettiResouceLoad(rpw, hdl_obj);

	RT_PokeIconSetAll(rpw);
	RT_RankingIconSetAll(rpw, hdl_obj);
	RT_MeterResouceLoad(rpw, hdl_obj);
...
void RT_ConfettiResouceLoad(CONRES_PROC_WORK *rpw, ARCHANDLE *hdl_obj)
{
	CATS_LoadResourceCharArcH(rpw->sys.csp, rpw->sys.crp, hdl_obj, CONFETTI8X8_NCGR_BIN, 1, 
		NNS_G2D_VRAM_TYPE_2DMAIN, R_CHARID_CONFETTI);
	CATS_LoadResourceCellArcH(rpw->sys.csp, rpw->sys.crp, hdl_obj, 
		CONFETTI8X8_NCER_BIN, 1, R_CELLID_CONFETTI);
	CATS_LoadResourceCellAnmArcH(rpw->sys.csp, rpw->sys.crp, hdl_obj, 
		CONFETTI8X8_NANR_BIN, 1, R_CELLANMID_CONFETTI);
}
...
static void RT_ConfettiActorAdd(CONRES_PROC_WORK *rpw)
{
	...
	CATS_ObjectAnimeSeqSetCap(confetti->cap, contest_rand(rpw->consys) % CONFETTI_ANM_NUM);
...
void RT_RankingIconSetAll(CONRES_PROC_WORK *rpw, ARCHANDLE *hdl_obj)
{
	CATS_LoadResourceCharArcH(rpw->sys.csp, rpw->sys.crp, hdl_obj, RANKING_NCGR_BIN, 1, 
...
{
	CATS_LoadResourcePlttWorkArcH(rpw->sys.pfd, FADE_MAIN_OBJ, rpw->sys.csp, rpw->sys.crp, 
		hdl_obj, CONTEST_KEKKA_METER_NCLR, 0, 
		1, NNS_G2D_VRAM_TYPE_2DMAIN, R_PLTTID_METER);
	CATS_LoadResourceCharArcH(rpw->sys.csp, rpw->sys.crp, hdl_obj, 
