...
static BOOL DanceData_DataRhythmGet(DANCE_PROC_WORK *dpw, int *data_no, int *rhythm_no);
...
PROC_RESULT DanceProc_Init( PROC * proc, int * seq )
{
	...
	OS_TPrintf("consys size = %d\n", sizeof(CONTEST_SYSTEM));
...
//		if( IsFinishedBrightnessChg(MASK_DOUBLE_DISPLAY) ){
	...
	case SEQ_MAIN:
		if(ConTool_ServerMineCheck(dpw->consys) == TRUE){
			...
			else if(ret == DSRET_END && dpw->main_end == TRUE){
				*seq = SEQ_OUT;
				MI_CpuClear8(&dpw->local, sizeof(DANCE_LOCAL_WORK));
		...
		if(dpw->main_end == TRUE && CO_TransmitFlagCheck(&dpw->cow) == FALSE 
				&& ConTool_ServerMineCheck(dpw->consys) == FALSE){
			*seq = SEQ_OUT;
			MI_CpuClear8(&dpw->local, sizeof(DANCE_LOCAL_WORK));
			WIPE_SYS_Start(WIPE_PATTERN_FSAM, WIPE_TYPE_BLINDOUT_H, WIPE_TYPE_BLINDOUT_H, 
				WIPE_FADE_BLACK, WIPE_DEF_DIV, WIPE_DEF_SYNC, HEAPID_DANCE);
...
//--------------------------------------------------------------
/**
 * @brief   ダンスデータからデータ番号とリズム番号を取得
 *
 * @param   dpw		
 * @param   data_no			データ番号代入先
 * @param   rhythm_no		リズム番号代入先
 *
 * @retval  TRUE:DPが混ざっている
 * @retval  FALSE:DPは混ざっていない
 */
//--------------------------------------------------------------
static BOOL DanceData_DataRhythmGet(DANCE_PROC_WORK *dpw, int *data_no, int *rhythm_no)
{
	if(dpw->consys->sio_flag == TRUE && dpw->consys->sio_dp_num > 0){
		dpw->dp_sio = TRUE;
	}
	else{
		dpw->dp_sio = 0;
	...
	if(dpw->dp_sio == TRUE){	//DPが混ざった通信対戦
...
static void DanceDefaultBGSet(DANCE_PROC_WORK *dpw, ARCHANDLE *hdl_bg)
{
	...
	//スクリーン
	{
		...
		DanceData_DataRhythmGet(dpw, &data_no, &rhythm_no);
		scrn_index = RhythmDataGet(rhythm_no, RHYTHM_IDX_BG_SCRN_INDEX);
...
static int DanceSeq_Init(DANCE_PROC_WORK *dpw, DANCE_LOCAL_WORK *local)
{
	...
	switch(local->seq){
	case 0:
		if(dpw->consys->sio_dp_num > 0){
			if(local->work < CON_DP_SIO_WAIT){
				local->work++;
				break;
			}
		}
...
//		if(CO_AnswerCountCheck(&dpw->cow, dpw->consys->c_game.player_num, 
//				dpw->consys->c_game.my_breeder_no, dpw->consys->c_game.server_no) == TRUE){
		//通信BTS#0467対処の為、プラチナ子機のみ終了同期を取る
		if(CO_AnswerCountCheck(&dpw->cow, 
				dpw->consys->c_game.player_num - dpw->consys->sio_dp_num - 1, 
				dpw->consys->c_game.my_breeder_no, dpw->consys->c_game.server_no) == TRUE){
			return DSRET_END;
		}
		break;
