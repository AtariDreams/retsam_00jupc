...
//		if( IsFinishedBrightnessChg(MASK_DOUBLE_DISPLAY) ){
	...
	case SEQ_MAIN:
		if(ConTool_ServerMineCheck(vpw->consys) == TRUE){
			...
			else if(ret == VSRET_END && vpw->main_end == TRUE){
				*seq = SEQ_OUT;
				MI_CpuClear8(&vpw->local, sizeof(VISUAL_LOCAL_WORK));
		...
		if(vpw->main_end == TRUE && CO_TransmitFlagCheck(&vpw->cow) == FALSE 
				&& ConTool_ServerMineCheck(vpw->consys) == FALSE){
			*seq = SEQ_OUT;
			MI_CpuClear8(&vpw->local, sizeof(VISUAL_LOCAL_WORK));
			WIPE_SYS_Start(WIPE_PATTERN_FSAM, WIPE_TYPE_HOLEOUT, WIPE_TYPE_TUNNELOUT_TOP,
				WIPE_FADE_BLACK, WIPE_DEF_DIV, WIPE_DEF_SYNC, HEAPID_VISUAL);
...
static void VisualDefaultOBJSet(VISUAL_PROC_WORK *vpw, ARCHANDLE *hdl_obj)
{
	...
	VT_AudienceHeartResourceLoad(vpw->sys.csp, vpw->sys.crp, hdl_obj);
