...
//		dpw->sys.exchr_param[i].exchr_buffer = sys_AllocMemory(HEAPID_DANCE, EXCHR_BUFFER_SIZE);
	...
	for(i = 0; i < BREEDER_MAX; i++){
		...
		height = PokeParaHeightGet_IMC(dpw->consys->c_game.pp[breeder_no], PARA_FRONT);
	#if 0
...
void DT_ShadowActorSetAll(DANCE_SYSTEM_PARAM *sys, CATS_SYS_PTR csp, CATS_RES_PTR crp)
{
	...
	for(i = 0; i < BREEDER_MAX; i++){
		...
		sys->shadow[i].height = PokeParaHeightGet_IMC(sys->c_game->pp[i], PARA_FRONT);
...
void DT_SpotLightActorSetAll(DANCE_SYSTEM_PARAM *sys, CATS_SYS_PTR csp, CATS_RES_PTR crp)
{
	...
	for(i = 0; i < BREEDER_MAX; i++){
		...
		sys->spotlight[i].height = PokeParaHeightGet_IMC(sys->c_game->pp[i], PARA_FRONT);
...
{
	u32 len, one_len, one_len_offset;
	int x, y, one_percent;
	int bar_len, x_offset;
	...
	if(rhythm_no == RHYTHM_NO_4){
		...
		one_len = (bar_len * ONE_TEMPO_CALC_DECIMAL) / 8;	//3拍子はそもそもイリーガルで突っ込んだのでベタ対応
	}
