...
static void Recieve_VisualExit(CONTEST_ORDER_WORK *cow, void *syswork, const CORDER_HEAD *req_head, void *data)
{
	...
	//返事を返す	バッファは特に渡すものがないのでNULL指定
	if(ConTool_ServerMineCheck(vpw->consys) == FALSE){
		CO_ANSWER_TransmitBufferSet(cow, req_head, NULL, 0);
	}
}
...
static void Recieve_PokeAppear(CONTEST_ORDER_WORK *cow, void *syswork, const CORDER_HEAD *req_head, void *data)
{
	...
	height = PokeParaHeightGet_IMC(vpw->sys.c_game->pp[vap->now_breeder], PARA_FRONT);
...
static void VOTCB_PokeAppear(TCB_PTR tcb, void *work)
{
	...
	case 1:
		...
		height = PokeParaHeightGet_IMC(pa->vpw->sys.c_game->pp[pa->breeder_no], PARA_FRONT);
...
static void VOTCB_Advance(TCB_PTR tcb, void *work)
{
	...
	case 0:
		...
		break;
	case 1:
		height = PokeParaHeightGet_IMC(ad->vpw->sys.c_game->pp[ad->breeder_no], PARA_FRONT);
...
static void VOTCB_PokeOut(TCB_PTR tcb, void *work)
{
	...
	height = PokeParaHeightGet_IMC(po->vpw->sys.c_game->pp[po->breeder_no], PARA_FRONT);
	switch(po->seq){
	case 0:
...
//	SOFT_SPRITE *ss;
	...
	height = PokeParaHeightGet_IMC(poke_eff->vpw->sys.c_game->pp[poke_eff->breeder_no], PARA_FRONT);
