...
void DancdAI_MainDancerCalc(DANCE_PROC_WORK *dpw, DANCE_AI_WORK *aiwork, u32 all_tempo_frame, u32 one_tempo_frame)
{
	...
	if(dpw->d_game.rhythm_no == RHYTHM_NO_4){
		...
		beat_max_zure_frame = 
			(one_tempo_frame / 6 + ONE_TEMPO_CALC_DECIMAL/2) / ONE_TEMPO_CALC_DECIMAL;
	}
...
static int BackDancer_AlphaGet(int con_rank, int con_type, int character, int beat, int ahead_beat, int rhythm_no, int dp_sio)
{
	...
	if(ahead_beat != -1){
		//半テンポずらされる
		if(rhythm_no == RHYTHM_NO_4){	//3拍子は裏拍が多いのでこの判定を省く
	...
	case CONRANK_HYPER:
		if(dp_sio == TRUE && (con_type == CONTYPE_CUTE || con_type == CONTYPE_CLEVER)){
			alpha = alpha*3;	//この２つは難しいとの意見があったのでalphaを調整
		}
		else{
			alpha += alpha/2;
		}
	...
	default:
		if(dp_sio == TRUE && (con_type == CONTYPE_CUTE || con_type == CONTYPE_CLEVER)){
			alpha = alpha*2 + alpha/2;	//この２つは難しいとの意見があったのでalphaを調整
		}
		break;
	}
	...
	int main_step, int ahead_step, int dp_sio)
{
	...
	case CONRANK_HYPER:
		if(dp_sio == TRUE && (con_type == CONTYPE_CUTE || con_type == CONTYPE_CLEVER)){
			beta *= 2;	//この２つは難しいとの意見があったのでbetaを調整
		}
		else{
			beta += beta/2;
		}
	...
	default:
		if(dp_sio == TRUE && (con_type == CONTYPE_CUTE || con_type == CONTYPE_CLEVER)){
			beta += beta/2;	//この２つは難しいとの意見があったのでbetaを調整
		}
		break;
	}
	...
	DANCING_SEND_WORK *sendwork)
{
	...
	if(aiwork->back.occ == FALSE){
		...
		alpha = BackDancer_AlphaGet(dpw->consys->c_game.rank, dpw->consys->c_game.type, 
			aiwork->character, main_param->hit_beat, ahead_beat, dpw->d_game.rhythm_no,
			dpw->dp_sio);
		beta = BackDancer_BetaGet(dpw->consys->c_game.rank, dpw->consys->c_game.type,
			aiwork->character, main_param->hit_beat, ahead_beat, 
			main_param->dance_step, ahead_step, dpw->dp_sio);
