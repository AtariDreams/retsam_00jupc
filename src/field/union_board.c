...
struct UNION_BOARD_WORK{
	...
	BOOL					scrollbarYCont;	// ScrollControl関数内で
											// scrollbarのY座標をコントロールするか
	u16						scrllbttn_push[2];	// スクロールボタン押している判定
...
//		UnionBoard_ChatDataDelete( ubw->UnionChatRingBuf );
        ...
        // 止まらないASSERTを止まるように修正
		GF_ASSERT_MSG(0,"NotInit\n"); // レーダー初期化が終わらないうちに終了呼び出しされました");
	}
}
...
/* 欧州版で文字が上に切れるバグがあったので、１ドットだけ上げる修正が組み込まれた */
	...
	// 'g' や 'y' などの下端が切れるので、表示位置を1ピクセル上に移動
	// ※ 描画領域を下端まで使い切った文字があるとさらに切れるので注意
...
static int TouchTrgFunc( UNION_BOARD_WORK *ubw )
{
	int button,i,listpos, repeat;
	WMBssDesc         *beacon;
	_GF_BSS_DATA_INFO *gfinfo;

	button=GF_TP_RectHitCont( sub_button_hittbl );
	repeat=UnionBoardTouchRepeatGet( ubw );
...
//			MyStatus_Copy((MYSTATUS*)gfinfo->myStatusBuff, status);
			...
			if(UnionChatRegisterCheck(ubw, pms, gfinfo->pokeID)){
				// 一致しなかったので登録
				UnionBoard_AddChatSub( ubw, CommMPGetBconMyStatus(i), pms, gfinfo->pokeID );	// 簡易会話登録
			}
		}
	}
...
static void UnionBoardTouchRepeatInit( UNION_BOARD_WORK *ubw )
{
	// gflib/systemのkeyrepeatと一緒の処理で行う
	// タッチなし状態から初タッチ時はWAITの間隔でタッチを立てて
	// 初タッチからはSPEEDの間隔でタッチフラグを立てる
	ubw->touchRepeat = 0;
	ubw->touchRepeatWait = SYS_KEYREPEAT_WAIT_DEF;
	ubw->touchRepeatSpeed = SYS_KEYREPEAT_SPEED_DEF;
	ubw->touchRepeatCount = ubw->touchRepeatWait;
}
	...
	if( sys.tp_trg ){
		...
		if( sys.tp_cont ){
			...
			if( ubw->touchRepeatCount < 0 ){
				ubw->touchRepeat = TRUE;
				ubw->touchRepeatCount = ubw->touchRepeatSpeed;
			}
		}else{
			ubw->touchRepeatCount = ubw->touchRepeatWait;
		}
 ...
 *
 *	@retval	TRUE		タッチしている
 *	@retval FALSE		タッチしていない
 */
//-----------------------------------------------------------------------------
static BOOL UnionBoardTouchRepeatGet( const UNION_BOARD_WORK * cp_ubw )
{
	return cp_ubw->touchRepeat;
}

//----------------------------------------------------------------------------
/**
 *	@brief	スクロールバーのY座標を管理するかのフラグ
 *
 *	@param	ubw		ワーク
 *	@param	flag	フラグ
 */
...
static void UnionBoardScrollbarYContFlagSet( UNION_BOARD_WORK *ubw, BOOL flag )
{
	ubw->scrollbarYCont  = flag;
}

//----------------------------------------------------------------------------
/**
 *	@brief	スクロールバーのY座標を管理するかフラグを取得する
 *
 *	@param	cp_ubw	ワーク
 *
 *	@retval	TRUE	管理する
...
static BOOL UnionBoardScrollbarYContFlagGet( const UNION_BOARD_WORK *cp_ubw )
{
	return cp_ubw->scrollbarYCont;
}


//----------------------------------------------------------------------------
/**
 *	@brief	スクロールボタンアニメ		押しているフラグ設定
 *
 *	@param	ubw			ワーク
 *	@param	button_no	ボタンナンバー
 *
 *	リピート動作のときにボタンが押しっぱなしになるように調整
 */
//-----------------------------------------------------------------------------
static void UnionBoardScrollBttnAnmPush( UNION_BOARD_WORK *ubw, int button_no )
{
	...
	ubw->scrllbttn_push[ button_no ] = TRUE;
} 

//----------------------------------------------------------------------------
/**
 *	@brief	スクロールボタンアニメ　	アニメ管理処理
 *
 *	@param	ubw			ワーク
 *
 *	リピート動作のときにボタンが押しっぱなしになるように調整
...
static void UnionBoardScrollBttnAnmMain( UNION_BOARD_WORK *ubw )
{
	...
	for( i=0; i<2; i++ ){
		...
		if( ubw->scrllbttn_push[i] == TRUE ){
			if( frame >= UNIONBOARD_SCRLLBTTN_PUSH ){
				CLACT_SetAnmFlag(ubw->clActWork[i],0);
			}

			// フラグを下げる次のフレームもおしてあったらTRUEになってこの関数に帰ってくる
			ubw->scrllbttn_push[i] = FALSE;
			
		}else{
			if( CLACT_GetAnmFlag(ubw->clActWork[i]) == 0 ){
				CLACT_SetAnmFlag(ubw->clActWork[i],1);
				CLACT_AnmFrameSet( ubw->clActWork[i], UNIONBOARD_SCRLLBTTN_PUSH+1 );
			}
		}
	}
}
