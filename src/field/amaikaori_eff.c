...
#include "include/field/weather_sys.naix"
#include "zonedata.h"
...
typedef struct {
	...
	u16					palbuf[16];
	AMAIKAORI_MOVE_WORK fade;
	BOOL end;
} AMAIKAORI_PALANM_WORK;
...
#define AMAIKAORI_COLOR				( GX_RGB( 31, 10, 23 ) )

#define AMAIKAORI_FADE_EVY_START	( 0 )
#define AMAIKAORI_FADE_EVY_END		( 8 )
...
{
	AMAIKAORI_PALANM_WORK* p_data = p_work;
	sys_FreeMemoryEz( p_data->p_plttbuff );
}
...
static void FLDMAP_Amaikaori_Komorebi_Main(FLDMAPFUNC_WORK * p_fwk, FIELDSYS_WORK * p_fsys, void * p_work)
{
	...
	result = AmaikaoriMoveMain( &p_data->fade );
	if( result ){
		p_data->end = TRUE;
	}
...
FLDMAPFUNC_WORK* FLDMAP_Amaikaori_Komorebi_Start( FIELDSYS_WORK* fsys )
{
	static const FLDMAPFUNC_DATA FLDMAP_AmaikaoriData = {
		AMAIKAORI_TSK_PRI,
		sizeof(AMAIKAORI_PALANM_WORK),
		FLDMAP_Amaikaori_Komorebi_Init,
		FLDMAP_Amaikaori_Komorebi_Delete,
		FLDMAP_Amaikaori_Komorebi_Main,
		FLDMAP_Amaikaori_Draw
	};
	FLDMAPFUNC_WORK* p_fwk;
...
//----------------------------------------------------------------------------
/**
 *	@brief	木漏れ日用	甘い香りエフェクト	エンカウントしないとき
 *
 *	@return	ワーク
 */
//-----------------------------------------------------------------------------
FLDMAPFUNC_WORK* FLDMAP_AmaikaoriEndEff_Komorebi_Start( FIELDSYS_WORK* fsys )
{
	static const FLDMAPFUNC_DATA FLDMAP_AmaikaoriData = {
		AMAIKAORI_TSK_PRI,
		sizeof(AMAIKAORI_PALANM_WORK),
		FLDMAP_AmaikaoriEndEff_Komorebi_Init,
		FLDMAP_Amaikaori_Komorebi_Delete,
		FLDMAP_Amaikaori_Komorebi_Main,
		FLDMAP_Amaikaori_Draw
	};
...
{
	AMAIKAORI_PALANM_WORK* p_data = FLDMAPFUNC_GetFreeWork( p_eff );
	return p_data->end;
}
 ...
 */
//-----------------------------------------------------------------------------
static BOOL FLDMAP_Amaikaori_Komorebi_ChangeCheck( FIELDSYS_WORK* fsys )
{
	static const u32 sc_Komorebi[] = {
		WEATHER_SYS_KOGOREBI,
	};
	u32 weather_no;
	int i;
	...
	for( i=0; i<NELEMS( sc_Komorebi ); i++ ){
		if( weather_no == sc_Komorebi[i] ){
			return TRUE;
		}
	}
	return FALSE;
...
BOOL FLDMAP_AmaikaoriEvent( GMEVENT_CONTROL * event )
{
	...
	case FLDMAP_AMAIKAORI_START_0:	// エフェクト開始
		if( p_work->komorebi == FALSE ){
			...
			p_work->p_fwk = FLDMAP_Amaikaori_Komorebi_Start( fsys );		
		}
	...
	case FLDMAP_AMAIKAORI_END_0:		// 終了チェック
		if( p_work->komorebi == FALSE ){
			...
			result = FLDMAP_Amaikaori_Komorebi_EndCheck( p_work->p_fwk );
		}
		if( result == TRUE ){
	...
	case FLDMAP_AMAIKAORI_START_1:	// エンカウントしないときの　エフェクト開始
		...
		if( p_work->komorebi == FALSE ){
			...
			p_work->p_fwk = FLDMAP_AmaikaoriEndEff_Komorebi_Start( fsys );
		}
