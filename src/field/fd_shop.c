...
static void ShopBuyWinAdd( SHOP_WORK * wk )
{
	...
	for( i=0; i<WIN_IDX_MAX; i++ ){
		...
		else if ( wk->shop_type == SHOP_TYPE_BP && i == WIN_IDX_GOLD ){
...
*/
	...
	wk->cm_cnt = 0;
	MI_CpuClear32((void*)HW_BG_VRAM, HW_BG_VRAM_SIZE);	///< VRAM CLEAR
...
static u8 ShopBuyCmpSeq( SHOP_WORK * wk )
{
	...
	if( wk->shop_type == SHOP_TYPE_BP ){
		RECORD_Add( wk->rec, RECID_USE_BP, wk->sel_price*wk->sel_num );
	}else{
