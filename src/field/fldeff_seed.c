...
static void SG_GraphicInit( FE_SEEDGROUND_PTR sg )
{
	...
	do{
		FE_FROMdl_ResSetArcLoadTexTrans(
			sg->fes, &sg->rmdl[i], 0, DATA_SeedGroundMdlArcIDTbl[i], ALLOC_FR );
		FRO_OBJ_InitInMdl( &sg->robj[i], &sg->rmdl[i] );
		i++;
...
static void EoaSeedGround_Draw( EOA_PTR eoa, void *wk )
{
	...
	if( work->vanish_sw != TRUE ){
		...
		pos.z += SG_DRAW_Z_OFFSET;
		FRO_OBJ_DrawPos( &work->head.sg->robj[work->dry], &pos );
	}
}
