...
#if (CRC_LOADCHECK && CRCLOADCHECK_GMDATA_ID_POKETCH_DATA)
	SVLD_SetCrc(GMDATA_ID_POKETCH_DATA);
#endif //CRC_LOADCHECK
}
//------------------------------------------------------------------
/**
...
void PoketchData_PokeHistory_Add( POKETCH_DATA* poketch_data, const POKEMON_PASO_PARAM* ppp )
{
	...
	poketch_data->poke_history[max].formno = PokePasoParaGet((POKEMON_PASO_PARAM*)ppp, ID_PARA_form_no, NULL);
#if (CRC_LOADCHECK && CRCLOADCHECK_GMDATA_ID_POKETCH_DATA)
	SVLD_SetCrc(GMDATA_ID_POKETCH_DATA);
#endif //CRC_LOADCHECK
}
 ...
 */
//------------------------------------------------------------------
u32 PoketchData_PokeHistory_GetFormNo( const POKETCH_DATA* poketch_data, int index )
{
	GF_ASSERT(index<POKETCH_POKE_HISTORY_COUNT_MAX);
	GF_ASSERT(poketch_data->poke_history[index].monsno);
	return(poketch_data->poke_history[index].formno);
}
...
#if (CRC_LOADCHECK && CRCLOADCHECK_GMDATA_ID_POKETCH_DATA)
	SVLD_CheckCrc(GMDATA_ID_POKETCH_DATA);
#endif //CRC_LOADCHECK
	return pk;
}
