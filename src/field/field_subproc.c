...
void* WazaOshiePokeStatusEvent_SetProc( int heap, FIELDSYS_WORK* fsys, u16 pos ,u16 waza )
{
	...
	psd->ribbon		= SaveData_GetSpRibbon( fsys->savedata );
	psd->zukan_mode = PMNumber_GetMode( fsys->savedata );
	psd->ev_contest	= SysFlag_PSTContestCheck( SaveData_GetEventWork(fsys->savedata) );
	//psd->ev_contest = PokeStatus_ContestFlagGet( fsys->savedata );
	psd->perap		= NULL;
...
//-----------------------------------------------------------------------------
/**
 * @brief	破れた世界突入デモ画面呼び出し
 * @param	fsys	フィールド制御ワークへのポインタ
 */
//-----------------------------------------------------------------------------
void Field_HakaiWarp_SetProc(FIELDSYS_WORK * fsys)
{
	GameSystem_StartSubProc(fsys, &HakaiWarp_proc_data, fsys->savedata);
}
