...
static int FldOBJ_MoveStartCheck( CONST_FIELD_OBJ_PTR fldobj )
{
	...
	{	//移動禁止フラグ相殺チェック
		u32 st = FieldOBJ_StatusBit_Get( fldobj );
		
		//高さ取得しない場合
		if( (st&FLDOBJ_STA_BIT_HEIGHT_GET_ERROR) &&
			(st&FLDOBJ_STA_BIT_HEIGHT_GET_OFF) == 0 ){
			return( FALSE );
		}
		
		//アトリビュート取得しない場合
		if( (st&FLDOBJ_STA_BIT_ATTR_GET_ERROR) &&	
			FieldOBJ_MoveBitCheck_AttrGetOFF(fldobj) == FALSE ){
			return( FALSE );
		}
	}
		...
		FIELD_OBJ_PTR fldobj, MATR now, MATR old, const OBJCODE_STATE *state )
{
	...
	if( MATR_IsShadowOnSnow(old) == TRUE ){
		if( state->type_footmark == FLDOBJ_FOOTMARK_NORMAL ){
			FE_FldOBJFootMarkShadowSnow_Add( fldobj );
		}else if( state->type_footmark == FLDOBJ_FOOTMARK_CYCLE ){
			FE_FldOBJFootMarkShadowSnowCycle_Add( fldobj );
		}
	}
	...
	
	{
		if( FieldOBJ_StatusBitCheck_Reflect(fldobj) == FALSE ){
			...
			if( MATR_IsReflect(now) == TRUE ){
				...
				MATR next = FieldOBJ_NextDirAttrGet( fldobj, DIR_DOWN );
			...
			if( hit != MATR_IsNotAttrGet() ){	//映り込みヒット
				...
				FE_FldOBJReflect_Add( fldobj, type );
			}
		}
	}	
}
 ...
 */
//--------------------------------------------------------------
int FieldOBJ_MoveHitCheckAttr(
	CONST_FIELD_OBJ_PTR fldobj, int x, int z, int dir )
{
	if( FieldOBJ_MoveBitCheck_AttrGetOFF(fldobj) == FALSE ){
		...
		if( next_attr == MATR_IsNotAttrGet() ){
			OS_Printf( "Next Attr ERROR !!\n" );
			OS_Printf( "OBJ ID = 0x%x,", FieldOBJ_OBJIDGet(fldobj) );
			OS_Printf( "OBJ CODE = 0x%x,", FieldOBJ_OBJCodeGet(fldobj) );
			OS_Printf( "MOVE CODE = 0x%x", FieldOBJ_MoveCodeGet(fldobj) );
			OS_Printf( "GX = 0x%x, GZ = 0x%x\n",
				FieldOBJ_NowPosGX_Get(fldobj),
				FieldOBJ_NowPosGZ_Get(fldobj) );
		}
		#endif
 ...
 */
//--------------------------------------------------------------
void FieldOBJ_DrawInitAfterMoveProcCall( FIELD_OBJ_PTR fldobj )
{
