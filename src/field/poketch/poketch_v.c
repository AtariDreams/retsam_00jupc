...
enum {
	...
	SCRN_BUTTON_HEIGHT_SAME_SCRN_START = 4,
	SCRN_BUTTON_HEIGHT_SAME_SCRN_END = 6,
	SCRN_BUTTON_HEIGHT_SAME_SCRN_START_DOWN = 2,
	SCRN_BUTTON_HEIGHT_SAME_SCRN_END_DOWN = 4,
};
...
static void MakeButtonScrn( u16 *buf, u32 charno_top, int same_y_start, int same_y_end );
	...
	*wk_ptr = sys_AllocMemory(POKETCH_MAIN_VIEW_HEAPID, sizeof(POKETCH_VIEW_WORK));
	...
	if(*wk_ptr != NULL)
	{
		...
		MakeButtonScrn( (*wk_ptr)->scrn_button_free, BOARD_CHAROFS+CHARNO_BUTTON_FREETOP,
			SCRN_BUTTON_HEIGHT_SAME_SCRN_START, SCRN_BUTTON_HEIGHT_SAME_SCRN_END );
		...
		MakeButtonScrn( (*wk_ptr)->scrn_button_down_free, 
			...
			SCRN_BUTTON_HEIGHT_SAME_SCRN_START_DOWN, SCRN_BUTTON_HEIGHT_SAME_SCRN_END_DOWN );
...
static void MakeButtonScrn( u16 *buf, u32 charno_top, int same_y_start, int same_y_end )
{
	...
	for(y = 0; y < SCRN_BUTTON_HEIGHT; y++){
		...
		if(y >= same_y_start && y < same_y_end){
			;
		}
		else{
