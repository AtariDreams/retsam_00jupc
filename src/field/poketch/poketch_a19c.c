...
static BOOL SetupWork( APPWORK *wk, CONTACT_WORK* conwk, GF_BGL_INI* bgl, u32 param )
{
	...
	for(i=0; i<wk->vpara.poke_count; i++)
	{
		...
		if ( PokeParaGet(pp, ID_PARA_tamago_flag, NULL) ){ continue; }
		
		wk->vpara.pokeImgArcIndex[ count ] = PokeIconCgxArcIndexGet((const POKEMON_PASO_PARAM*)pp);
		wk->vpara.pokeMonsNo[ count ] = PokeParaGet(pp, ID_PARA_monsno, NULL);
		wk->vpara.pokeForm[ count ] = PokeParaGet(pp, ID_PARA_form_no, NULL);
		wk->ppp[ count ] = (POKEMON_PASO_PARAM*)pp;
