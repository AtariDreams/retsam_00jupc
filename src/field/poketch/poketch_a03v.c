...
static BOOL BmpWinSetup( VIEWWORK* wk )
{
	...
	if( wk->bmpwin )
	{
		...
		if( PoketchWork_LoadFast(wk->vpara->appNumber,
			wk->bmpwin->chrbuf,BMPWIN_CGX_BYTESIZE ) == FALSE)
		{
...
//add pl 080704 kaga キャッシュ更新 2x2ドット描画に対応
static void CmdPixelCharLoad(
	u8 *char_buf_p, int dot_x, int dot_y, int size_x, int size_y )
{
	int char_no,char_x,char_y,char_size_x,char_size_y;
	
	size_x--;	//0org
	size_y--;
	
	char_x = dot_x / 8;
	char_size_x = ((dot_x+size_x) / 8) - char_x + 1;
	char_y = dot_y / 8;
	char_size_y = ((dot_y+size_y) / 8) - char_y + 1;
	
	char_no = (char_y * BMPWIN_WIDTH) + char_x;
	
	while( char_size_y-- ){
		DC_FlushRange( char_buf_p+(char_no*0x20), char_size_x*0x20 );
		GXS_LoadBG3Char( char_buf_p+(char_no*0x20),
			(BMPWIN_CHARTOP+char_no)*0x20, char_size_x*0x20 );
		char_no += BMPWIN_WIDTH;
	}
}
...
static void CmdPixelSet( TCB_PTR tcb, void* wk_ptr )
{
	...
	if( vwk->vpara->touch_mode == TOUCHMODE_ERASER )
	{
		...
		CmdPixelCharLoad( (u8*)vwk->bmpwin->chrbuf, x, y, rangeX, rangeY );
	}
	else
	{
		...
		CmdPixelCharLoad( (u8*)vwk->bmpwin->chrbuf, x, y, 2, 2 );
	}
...
static void CmdPixelSet( TCB_PTR tcb, void* wk_ptr )
{
	...
	if( vwk->vpara->touch_mode == TOUCHMODE_ERASER )
	{
		...
		{
			GXS_LoadBG3Char(
				(u8*)(vwk->bmpwin->chrbuf) + (charno*0x20),
				(BMPWIN_CHARTOP+charno)*0x20, charsizeX*0x20 ); 
			charno += BMPWIN_WIDTH;
		}
	...
	else
	{
		...
		GXS_LoadBG3Char(
			(u8*)(vwk->bmpwin->chrbuf) + (charno*0x20),
			(BMPWIN_CHARTOP+charno)*0x20, 0x20 ); 
	}
