...
struct _POKETCH_WORK {
	...
	//プラチナから追加
	int button_dir;			///<上下どちらのボタンが押されたかを判定
};
...
static void SeqAppMain(POKETCH_WORK *wk)
{
	...
	case 1:
		...
		{
			if(wk->button_dir == BUTTON_DIR_UP){
				wk->vpara.app_pos = PoketchData_SetApplicationPosPrev( wk->poketch_savedata );
			}
			else{
...
static const u32 ButtonViewCmd[] = {
	...
	CMD_MAIN_BUTTONFREE_DOWN,
	CMD_MAIN_BUTTONLOCK_DOWN,
	CMD_MAIN_BUTTONPUSH_DOWN,
};
...
enum {
	BUTTON_ID_MAIN_UP,			///< メインのボタン(上)
	BUTTON_ID_MAIN_DOWN,		///< メインのボタン(下)
...
static void BmnCallBack( u32 idx, u32 event, u32 touch_event, void* wk_adrs )
{
	...
	else
	{
		...
		switch(touch_event){
		case BMN_TOUCH_OFF:
			v_cmd = (idx == BUTTON_ID_MAIN_UP) ? CMD_MAIN_BUTTONFREE : CMD_MAIN_BUTTONFREE_DOWN;
			PoketchProcPrintf( "@@BTN  off\n");
			break;
		case BMN_TOUCH_ON:
			if( PoketchIsSleepMode(wk) || wk->appchange_button_disable_flag )
			{
				v_cmd = (idx == BUTTON_ID_MAIN_UP) ? CMD_MAIN_BUTTONLOCK:CMD_MAIN_BUTTONLOCK_DOWN;
				event = BMN_EVENT_NULL;
				PoketchProcPrintf( "@@BTN  on, sleep\n");
			}
			else
			{
				v_cmd = (idx == BUTTON_ID_MAIN_UP) ? CMD_MAIN_BUTTONPUSH:CMD_MAIN_BUTTONPUSH_DOWN;
		...
		if(v_cmd != CMD_NULL)
		{
			...
			if( ButtonViewReq( wk->view_wk, v_cmd ) )
			{
				if( v_cmd == CMD_MAIN_BUTTONLOCK || v_cmd == CMD_MAIN_BUTTONPUSH 
						|| v_cmd == CMD_MAIN_BUTTONLOCK_DOWN || v_cmd == CMD_MAIN_BUTTONPUSH_DOWN)
				{
		...
		wk->button_dir = (idx == BUTTON_ID_MAIN_UP) ? BUTTON_DIR_UP : BUTTON_DIR_DOWN;
		OS_TPrintf("idx = %d, button_dir = %d\n", idx, wk->button_dir);
	}
}
//------------------------------------------------------------------
/**
...
void PoketchVoicePlay( u32 pokeno, u32 formno )
{
	POKETCH_WORK *wk = GetMainWorkPtr();
