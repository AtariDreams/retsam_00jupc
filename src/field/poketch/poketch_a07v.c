...
static void SetupPokeActor( VIEWWORK* vwk, const VIEWPARAM* vpara )
{
	...
	if( arcHandle )
	{
		...
		for(i=0; i<vpara->poke_count; i++)
		{
			...
			vwk->icon[i].reverse_NG = PokeFormNoPersonalParaGet( vpara->poke_param[i].monsno, vpara->poke_param[i].form, ID_PER_reverse );
...
//			PoketchVoicePlay( vpara->poke_param[ vwk->touch_icon ].monsno );
			PoketchVoicePlay( vpara->poke_param[ vwk->touch_icon ].monsno, vpara->poke_param[ vwk->touch_icon ].form );
		}
		vwk->prev_touch_icon = vwk->touch_icon;
	}
	else
