...
{
	//add pl 080702 ピクセル情報クリーンが無い為、更新されない
	memset( wk->vpara.pixel, 0, sizeof(u8)*(PIXEL_H_MAX*PIXEL_V_MAX) );
	...
	if( Poketch_A03_SetViewWork( &(wk->vwk), &(wk->vpara), bgl ) )
	{
		...
		wk->button_man = PBTN_Create(
			ButtonRectData, NELEMS(ButtonRectData),
...
static void TouchPixelEraser( APPWORK *wk, u32 tx, u32 ty )
{
	int x,y,sx,sy,ex,ey;
	
	sx = tx - (ERASE_RANGE_PIXEL/2);
	if( sx < 0 ){ sx = 0; }
	sy = ty - (ERASE_RANGE_PIXEL/2);
	if( sy < 0 ){ sy = 0; }
	
	ex = tx + (ERASE_RANGE_PIXEL/2);
	if( ex >= PIXEL_H_MAX ){ ex = PIXEL_H_MAX; }
	ey = ty + (ERASE_RANGE_PIXEL/2);
	if( ey >= PIXEL_V_MAX ){ ey = PIXEL_V_MAX; }
	
	for( x = sx; x < ex; x++ ){
		for( y = sy; y < ey; y++ ){
			wk->vpara.pixel[x][y] = TOUCHMODE_ERASER;
		}
	}
}
...
static void InterpolateCommandSet( APPWORK* wk, u32 sx, u32 sy, u32 ex, u32 ey )
{
	...
	if( diffX > diffY )
	{
		...
		while( sx != ex )
		{
			...
			{
				if( wk->vpara.touch_mode == TOUCHMODE_ERASER ||
					wk->vpara.pixel[sx][intVal] != wk->vpara.touch_mode )
				{
					if( wk->vpara.touch_mode == TOUCHMODE_ERASER )
					{
						TouchPixelEraser( wk, sx, intVal );
					}
