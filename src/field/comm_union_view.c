...
COMM_UNIONROOM_VIEW *Comm_UnionRoomViewInit(COMM_UNIONROOM_WORK *cuw)
{
	...
	view->jiki			 = view->fsys->player;	//tomoya 080701	ここで設定後、MapChangeが動いてしまうので、playerのポインタが変わる可能性がある
...
static void commUnionViewTask(TCB_PTR tcb, void* work)
{
	...
	if(!FieldEvent_Check(view->fsys)){
		...
		//tomoya 080701	
		//ここでplayerのポインタを設定
		//以前の場所だとMapChangeが動いて、playerのポインタがかわる可能性あり
...
//			}
			if(beacon==NULL){
				...
				if( view->objwork[i].trainerId!=gfbssdata->pokeID ){
					UnionObjRequest( view->objwork, i, OBJ_REQ_RISE );
				}
			}
