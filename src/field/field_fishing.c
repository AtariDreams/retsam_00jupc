...
//定義でギョエーアクター即削除。ただし、表示したまま
//VRAM情報を削除する為タイミングによってはゴミが表示される
		...
		FISHWORK *work, PLAYER_STATE_PTR jiki, FIELD_OBJ_PTR fldobj )
{
	...
	work->eoa_gyoe = FE_FldOBJGyoe_Add( fldobj, GYOE_GYOE, TRUE, FALSE );
...
static int FishMove_JikiFishONHit(
		FISHWORK *work, PLAYER_STATE_PTR jiki, FIELD_OBJ_PTR fldobj )
{
	#ifdef GYOE_EOA_DEL_QUICK
	EOA_Delete( work->eoa_gyoe );
	work->eoa_gyoe = NULL;
	#else
	FE_Gyoe_Vanish( work->eoa_gyoe );
	#endif
