...
static void ItemUse_CheckWorkMakeTWorld(
...
void ItemUse_CheckWorkMake( FIELDSYS_WORK * fsys, ITEMCHECK_WORK * icwk )
{
	...
	//やぶれたせかい特殊地形用の処理へ add pl 080626 kaga
	...
	FIELDSYS_WORK * fsys, ITEMCHECK_WORK * icwk )
{
	...
	OS_Printf( "メニュー ItemCheck やぶれたせかい用\n" );
	#endif
...
static u32 ItemUseCheck_Tankenset( const ITEMCHECK_WORK * icwk )
{
	...
	// 探検セットが使えないアトリビュート add pl 080626 kaga
...
static u32 ItemUseCheck_FishingRod( const ITEMCHECK_WORK * icwk )
{
	...
	if( icwk->zone_id == ZONE_ID_D34R0101 ||
		...
		icwk->zone_id == ZONE_ID_D34R0104 ||
		icwk->zone_id == ZONE_ID_D34R0105 ||
		icwk->zone_id == ZONE_ID_D34R0106 ||
		icwk->zone_id == ZONE_ID_D34R0107 ||
		icwk->zone_id == ZONE_ID_D34R0108 ||
		icwk->zone_id == ZONE_ID_D34R0109 ||
		icwk->zone_id == ZONE_ID_D34R0110 ||
		icwk->zone_id == ZONE_ID_D34R0111 ){
