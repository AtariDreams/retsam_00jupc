...
#define _SELECTMENU_CGX		( FLD_MSG_WIN_CGX - _TRAPMENU_SIZE_X * _SELECTMENU_SIZE_Y )
#define	_SHOPMENU_CGX		( _SELECTMENU_CGX  - _TRAPMENU_SIZE_X * _SHOPMENU_SIZE_Y   )
...
static void _cursorChangeDisp(BMPLIST_WORK * wk,u32 param,u8 mode)
{
    ...
    GF_STR_PrintSimple(&pShop->winSub, FONT_SYSTEM,
                       pShop->pStrBuf1, 1, 1, MSG_NO_PUT, NULL);
    ...
    if(param != _MENURETURN_EXIT){ 
        ...
        GF_STR_PrintSimple(&pShop->winSub, FONT_SYSTEM, pShop->pStrBuf2, 1, 17, MSG_NO_PUT, NULL);
    }
    else{
//        STRBUF_Clear(pShop->pStrBuf1);
//        GF_STR_PrintSimple(&pShop->winSub, FONT_SYSTEM, pShop->pStrBuf1, 1, 17, 0, NULL);
    }
    GF_BGL_BmpWinOnVReq( &pShop->winSub );
...
//        GF_BGL_BmpWinDataFill( &pShop->winSub, FBMP_COL_WHITE );
    ...
    if(param == BMPLIST_CANCEL){
        ...
        GF_STR_PrintSimple(&pShop->winSub, FONT_SYSTEM, pShop->pStrBuf1, 1, 17, MSG_NO_PUT, NULL);
    }
    GF_BGL_BmpWinOnVReq( &pShop->winSub );
...
void UgShopMiniInfoDispWindow(_START_WORK* pShop, int msg)
{
    ...
    GF_STR_PrintSimple(&pShop->winSub2, FONT_SYSTEM, pShop->pStrBuf1, 1, 1, MSG_NO_PUT, NULL);
    GF_BGL_BmpWinOnVReq( &pShop->winSub2 );
...
void UgShopMiniInfoDispWindowDelOn(_START_WORK* pShop)
{
    if(GF_BGL_BmpWinAddCheck(&pShop->winSub2)){
        BmpMenuWinClear(&pShop->winSub2, WINDOW_TRANS_ON );
        GF_BGL_BmpWinDel(&pShop->winSub2);
    }
}
