...
#if (AFTERMASTER_070410_UNDERGROUND_N23_EUR_FIX|AFTER_MASTER_070122_FOSSILMSG_FIX)
#define _MSGWAIT_TIME (60)
#endif
...
void CommFossilRecvDigEndReq(int netID, int size, void* pData, void* pWork)
{
    ...
    _pCommFossilWork->radarIntervalTimer[netID] = 0;  //化石堀後はレーダー更新
#endif
}
...
//    simple_3DBGInit(HEAPID_DIGFOSSIL);
    ...
    // 下地 32+108+1
    {
		...
		hdl = ArchiveDataHandleOpen( ARC_UG_FOSSIL_GRA, HEAPID_DIGFOSSIL );
			    
	    ArcUtil_HDL_PalSet( hdl, NARC_ug_fossil_kaseki_board_NCLR,
	                    PALTYPE_MAIN_BG, 0, 32*3,  HEAPID_DIGFOSSIL);
	    charByte = ArcUtil_HDL_BgCharSet( hdl, NARC_ug_fossil_kaseki_board_NCGR,
	                                  bgl, GF_BGL_FRAME0_M, 0, 0, 0, HEAPID_DIGFOSSIL);
	    ArcUtil_HDL_ScrnSet( hdl, NARC_ug_fossil_kaseki_board_NSCR,
...
static void _commFossilGameFinalize(FossilEventTask* pFET)
{
    ...
    CommMsgSetBgl(CommUnderGetMsgUnderWorld(),_pCommFossilWork->pFSys->bgl, COMM_TALK_WIN_CGX_NUM, COMM_MSG_WIN_CGX);
}
...
// ----------------------------------------------------------------------------
      ...
      case _SEQ_RESULT_MSG:
        ...
        if(_fossilGetMessage(pFET)){
            ...
            pFET->msgtimer = _MSGWAIT_TIME;
#endif   //AFTERMASTER_070410_UNDERGROUND_N23_EUR_FIX
        }
        else{
...
static int _fossilRandGet(FossilParts* pParts)
{
    ...
    BOOL bEvent = MyStatus_GetID(SaveData_GetMyStatus(pSave)) % 2;
...
#else  //#if ( PM_VERSION == VERSION_PERL || PM_VERSION == VERSION_PLATINUM )
...
static int _depositBGInitParts(int idx, GF_BGL_INI* bgl, int charOffset, FossilEventTask* pFET)
{
    ...
    if(idx >= pFET->_PARTS_TREASURE_NUM){   // 硬い石の制限
        ...
        pFET->pPalBuff[idx] = ArcUtil_HDL_PalDataGet(
            hdl, pFPD->pParts->ncl, &pFET->palData[idx], HEAPID_DIGFOSSIL );
        DC_FlushRange( pFET->palData[idx]->pRawData, 32 );
        GX_LoadBGPltt( pFET->palData[idx]->pRawData, (pidx + _PALLET_INDEX)*32, 32 );
    }

    sendByte = ArcUtil_HDL_BgCharSet( hdl, pFPD->pParts->ncg, bgl,
...
/*	REND_OAMInit(
	...
	hdl = ArchiveDataHandleOpen( ARC_UG_ANIM_GRA, HEAPID_DIGFOSSIL );
	...
	_setResObjTbl(
        CLACT_U_ResManagerResAddArcKindCell_ArcHandle(_pCommFossilWork->resMan[CLACT_U_CELL_RES],
                                            hdl, NARC_ug_anim_kaseki_obj_NCER,
                                            0, _CLACT_HUMMER, CLACT_U_CELL_RES,HEAPID_DIGFOSSIL));
    _setResObjTbl(
        CLACT_U_ResManagerResAddArcKindCell_ArcHandle(_pCommFossilWork->resMan[CLACT_U_CELLANM_RES],
                                            hdl, NARC_ug_anim_kaseki_obj_NANR,
    ...
    _setResObjTbl(
        CLACT_U_ResManagerResAddArcKindCell_ArcHandle(_pCommFossilWork->resMan[CLACT_U_CELL_RES],
							hdl, NARC_ug_anim_kaseki_hibi_NCER, 0, _CLACT_HIBI, CLACT_U_CELL_RES,HEAPID_DIGFOSSIL));
    _setResObjTbl(
        CLACT_U_ResManagerResAddArcKindCell_ArcHandle(_pCommFossilWork->resMan[CLACT_U_CELLANM_RES],
                                            hdl, NARC_ug_anim_kaseki_hibi_NANR, 0, _CLACT_HIBI, CLACT_U_CELLANM_RES,HEAPID_DIGFOSSIL));
...
#if AFTER_MASTER_070410_UNDERGROUND_N14_EUR_FIX
        else{
            _RESULT_RADAR tr;
            tr.netID = i + ((_pCommFossilWork->radarIntervalTimer[i] - 20 + 1) << 4);
            tr.xpos = 0;
...
void UgDigFossilDeleteLog(void)
{
    if(_pCommFossilWork){
        MI_CpuClear8(_pCommFossilWork->logMsgFossil, COMM_MACHINE_MAX);
    }
}
...
void UgDigFossilResetPlayer(int netID)
{
    if(_pCommFossilWork){
        _pCommFossilWork->bEntryFossil[netID]=FALSE;
#if AFTER_MASTER_070410_UNDERGROUND_N13_EUR_FIX
        _pCommFossilWork->radarIntervalTimer[netID] = 0;
#endif
    }
}
