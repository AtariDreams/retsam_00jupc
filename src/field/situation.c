...
#if (CRC_LOADCHECK && CRCLOADCHECK_GMDATA_ID_FIELDOBJSV)
	SVLD_SetCrc(GMDATA_ID_FIELDOBJSV);
#endif //CRC_LOADCHECK
}
...
#if (CRC_LOADCHECK && CRCLOADCHECK_GMDATA_ID_SITUATION)
	SVLD_SetCrc(GMDATA_ID_SITUATION);
#endif //CRC_LOADCHECK
}
...
SITUATION * SaveData_GetSituation(SAVEDATA * sv)
{
		...
		return st;
#if (CRC_LOADCHECK && CRCLOADCHECK_GMDATA_ID_SITUATION)
	SVLD_CheckCrc(GMDATA_ID_SITUATION);
#endif //CRC_LOADCHECK
}
...
#if (CRC_LOADCHECK && CRCLOADCHECK_GMDATA_ID_FIELDOBJSV)
	SVLD_CheckCrc(GMDATA_ID_FIELDOBJSV);
#endif //CRC_LOADCHECK
	return fld;
}
