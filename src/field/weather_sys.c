...
#endif
#define WEATEHR_KOGOREBI_COUNT_ONE		(8)
#define WEATEHR_KOGOREBI_COUNT_NUM		(64)
#define WEATHER_KOGOREBI_MOVE_DIS		(1)
static u8 WEATHER_KOGOREBI_MOVEData[ WEATEHR_KOGOREBI_COUNT_NUM ] = {
	-WEATHER_KOGOREBI_MOVE_DIS,
	-WEATHER_KOGOREBI_MOVE_DIS,
	-WEATHER_KOGOREBI_MOVE_DIS*2,
	-WEATHER_KOGOREBI_MOVE_DIS*2,
	-WEATHER_KOGOREBI_MOVE_DIS,
	-WEATHER_KOGOREBI_MOVE_DIS,
	0,
...
static void scrollObj_Fx(WEATHER_SYS_WORK* pWork, fx32* x, fx32* y);			// 全オブジェクトの座標をscrollにあわせる
...
#if	PL_S0798_080712_FIX
		if( pWork->ObjArray[i].clact_w ){
...
static void scrollObj(WEATHER_SYS_WORK* pWork,int* x, int* y)
{
	...
	scrollObj_Fx( pWork, &fx_x, &fx_y );

	if(x != NULL){
		*x = fx_x >> FX32_SHIFT;
		if( *x < 0 ){
			*x += FX32_ONE;	// マイナスのとき-1分の誤差が出るため
		}
	}
	if(y != NULL){
		*y = fx_y >> FX32_SHIFT;
...
static void addWeatherSnow(WEATHER_SYS_WORK* pWork, int num)
{
	...
	for(i=0;i<num;i++){
		...
		obj_w[9] = WEATHER_SNOW_OBJ_AUTODEST_TIMING_MIN + (rand % WEATHER_SNOW_OBJ_AUTODEST_TIMING_MAX);		// 自滅カウンタ
...
static void addWeatherVolcano(WEATHER_SYS_WORK* pWork, int num)
{
	...
	for(i=0;i<num;i++){
		...
		obj_w[7] = WEATEHR_VOLCANO_DEL_COUNT_MIN + (gf_mtRand() % WEATEHR_VOLCANO_DEL_COUNT_MAX);					// 破棄カウンタ
...
static void contWeatherKogorebi(TCB_PTR tcb, void* work)
{
	...
	if((sys_work->Sequence != WEATHER_SEQ_DEST) &&
		...
		GF_BGL_ScrollReq( sys_work->pWSysCont->fsys->bgl, GF_BGL_FRAME2_M, GF_BGL_SCROLL_X_SET, (sys_w->x>>FX32_SHIFT)+add_x );
		GF_BGL_ScrollReq( sys_work->pWSysCont->fsys->bgl, GF_BGL_FRAME2_M, GF_BGL_SCROLL_Y_SET, (sys_w->y>>FX32_SHIFT) );
	}
}
