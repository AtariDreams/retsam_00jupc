...
#define DEBUG_FLDOBJ_PRINTF			//OS_Printf()有効
#endif

#define DEBUG_FLDOBJ_PRINTF_FORCE	//全環境で出すPrintf()有効
...
static BLACT_WORK_PTR FldOBJ_BlActAdd( BLACT_SET_PTR set,
	const BLACT_HEADER *head, const VecFx32 *vec, u32 fog_on );
...
//	FldOBJ_BlActVTransWorkInit( cont, max, 10 ); //old DP
	FldOBJ_BlActVTransWorkInit( cont, max, frm_trans_max ); //new PL
	
	#ifdef DEBUG_FLDOBJ_PRINTF
	OS_Printf( "FldOBJ BlActFrameTransMax %d\n", frm_trans_max );
	#endif
...
BLACT_WORK_PTR FieldOBJ_BlActAddCode( FIELD_OBJ_PTR fldobj, int code )
{
	...
	work = FldOBJ_BlActAdd( set, head, &vec,
			FldOBJ_BlAcOBJCodeFogEnableFlag(code) );
...
//--------------------------------------------------------------
/**
 * OBJコードからFog有効、無効フラグを取得
 * @param	code	HERO等
 * @retval	u32		TRUE=有効 FALSE=無効
 * @retval	char*	ヘッダーファイルパス
 */
//--------------------------------------------------------------
static u32 FldOBJ_BlAcOBJCodeFogEnableFlag( int code )
{
	...
	const int *tbl = DATA_FieldOBJ_BlActFogEnableOFFTbl;
 ...
 * @param	vec			座標
 * @param	fog_on		TRUE=フォグ有効,FALSE=無効
...
typedef struct
{
	...
	void *dummy_delete_call_proc_work;	//add pl
	void (*dummy_delete_call_proc)(void*,CONST_BLACT_WORK_PTR); //add pl
}BLACTDMY_WORK;

static BLACT_WORK_PTR FldOBJ_BlActDummyDrawResetSet(
		FIELD_OBJ_PTR fldobj, int code,
 ...
 * @param	dmy_del_call_proc ダミーアクターを削除する際に呼び出す関数
 * @param	dmy_del_call_proc_work dmy_del_call_procに指定する引数
 * @retval	BLACT_WORK_PTR 追加されたダミーアクター*。
 * return NULL=即変更によりダミーアクター追加無し。
 * dmy_del_call_proc 引数CONST_BLACT_WORK_PTRはダミーアクターが指定される。
 * NULL指定の場合はコールしない。
 * また即変更時はコールせず終わる。
 * dmy_del_call_proc内でのダミーアクター削除は禁止。
 */
//--------------------------------------------------------------
		...
		void *dmy_del_call_proc_work )
{
	...
	return( FldOBJ_BlActDummyDrawResetSet(
		fldobj,code,dmy_del_call_proc,dmy_del_call_proc_work) );
}
	...
	void *dmy_del_call_proc_work )
{
	...
	
	{
		...
		vec = *actvec;
//		vec.z += GRID_FX32*2;
		work->act = FldOBJ_BlActAdd(
			set, &work->head, &vec, FldOBJ_BlAcOBJCodeFogEnableFlag(old) );
		GF_ASSERT( work->act != NULL && "ERROR" );
		ret_act = work->act;
	}
	
#if 0	//FldOBJ_BlActAdd()にて対応
	{	//pl add FogEnable継承
		NNSG3dResMdl *mdl = BLACT_MdlResGet( oldact );
		if( mdl != NULL ){
			if( NNS_G3dMdlGetMdlFogEnableFlag(mdl,0) ){
				mdl = BLACT_MdlResGet( work->act );
	...
	BLACT_pRotateSet( work->act, BLACT_pRotateGet(oldact) ); //pl add
	...
	{	//映り込みアリ
		if( FieldOBJ_StatusBitCheck_Reflect(fldobj) == TRUE ){
			...
			if( MATR_IsMirrorReflect(attr) == TRUE ){ 	//鏡床
				type = REFTYPE_MIRROR;
			}else if( MATR_IsPoolCheck(attr) == TRUE ){	//水溜まり
				type = REFTYPE_POOL;
			}else{
				type = REFTYPE_POND;		//池
			}
	...
	
	{
		...
		BlActDummyActDeleteTCB( tcb, work ); //即追加の状況を考慮し即コール
	}
	
	if( BLACT_GetState(work->act) == BLACT_MOVE_NONE ){ //即追加により削除
		ret_act = NULL;
	}else{
		work->dummy_delete_call_proc_work = dmy_del_call_proc_work;
		work->dummy_delete_call_proc = dmy_del_call_proc;
	}
...
static void BlActDummyActDeleteTCB( TCB_PTR tcb, void *wk )
{
	...
	case 0:
		if( FieldOBJ_StatusBitCheck_Use(fldobj) == FALSE 	||
			...
			if( work->dummy_delete_call_proc != NULL ){
				work->dummy_delete_call_proc(
					work->dummy_delete_call_proc_work, work->act );
			}
