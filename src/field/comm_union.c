...
#define COMM_UNION_DEBUG_STATE_CHANGE	// ステートの変化をデバック出力する	080608 tomoya
#endif


//-------------------------------------
///	ステートの変化をデバック出力する	080608 tomoya
//=====================================
...
#else
#define _changeState_func(cuw, state, time)	_changeState((cuw), (state), (time));
#endif
...
static void _UnionStart(COMM_UNIONROOM_WORK *cuw)
{
    ...
    if(cuw->CommConnectStartFlag!=0){
        ...
        _changeState_func(cuw, _underChildConnecting, _CHILD_P_SEARCH_TIME);
		return;
	}
}
...
static void _underParentConnect(COMM_UNIONROOM_WORK *cuw)
{
    if(CommStateGetErrorCheck() && (0 == CommIsChildsConnecting())){
        // エラー検査してほしい状態の時に切断したらエラーに通信が行うので、ここは処理しない  BTS0405
        return;
    }
...
//	status      = CommMPGetBconMyStatus( objno );  //参照してないのでコメント 080619
	gfbssdata   = (_GF_BSS_DATA_INFO *)cuw->UnionBeaconTbl[objno]->gameInfo.userGameInfo;
	beaconState = (UNION_BEACON_STATE *)gfbssdata->regulationBuff;
	debug_beacon = beaconState;

	OS_Printf( "Union_ConnectStart() 接続開始" );
	OS_Printf( "ビーコン = %d", beaconState->mode );
	OS_Printf( "タイプ = %d\n", type );
	...
	case UNION_PARENT_MODE_OEKAKI_FREE:
		...
		return UNION_PARENT_CONNECT_OK;
		break;
	//ぐるぐる交換募集中の場合はデータシェアリング通信
	case UNION_PARENT_MODE_GURU2_FREE:
	case UNION_PARENT_MODE_GURUGURU_FREE:
		if(type!=UNION_PARENT_GURUGURU_OK){
			OS_Printf( "ぐるぐる交換接続失敗================================\n" );
			return UNION_PARENT_CONNECT_NG;
		}
		
		OS_Printf( "ぐるぐる交換接続成功！！=======================================\n" );
...
//		_changeState_func(fsys->union_work, _UnionRestart, 0);
	}
...
int Union_GetTalkNumber( COMM_UNIONROOM_WORK *cuw, int no, int mode, WORDSET *wordset )
{
    ...
    if(status==NULL){
        ...
        return 0;   //080619 NULLアクセス修正 エラー扱いにします
    }
	sex    = MyStatus_GetMySex(status);
...
u8 Union_GetRomCode( void )
{
	...
	code = MyStatus_GetRomCode( status );
...
//	GF_ASSERT(status!=NULL && "取得したMYSTATUSはNULL");
	...
	if(status==NULL){
        MSGMAN_Delete( msgman );  // 開放忘れ修正 08/06/10
		return;
	}
