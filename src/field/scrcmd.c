...
static BOOL EvCmdTwCameraDefaultSet( VM_MACHINE * core );
...
static BOOL EvCmdHakaiWarpSetProc( VM_MACHINE * core );

static BOOL EvCmdTwGirasSet( VM_MACHINE * core );
static BOOL EvCmdTwGirasDelete( VM_MACHINE * core );

static BOOL EvCmdWflbyCounterTime( VM_MACHINE * core );
static BOOL EvCmdGirathinaFormUpdate( VM_MACHINE * core );
static BOOL EvCmdD35HaihuReziCheck( VM_MACHINE * core );
...
{
	FIELDSYS_WORK *fsys = core->fsys;
	FldTornWorld_CameraDefaultSet( fsys );
	return 0;
}
...
static BOOL EvCmdHakaiWarpSetProc( VM_MACHINE * core )
{
	Field_HakaiWarp_SetProc( core->fsys );
	VM_SetWait( core, EvCmdWaitSubProcEnd );
	return 1;
}
