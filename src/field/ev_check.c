...
#include "field_villa.h"	//別荘　家具話しかけ
...
int CheckRequest(const EV_REQUEST * req, FIELDSYS_WORK * repw)
{
	...
	//トレーナー視線チェック
	{
		if( req->DebugKeyPush == FALSE ){
			...
			#else	//PL GhostGym kaga
			if( EvTrainerEyeCheck(repw,flag) == TRUE ||
				(GYM_GimmickCodeCheck(repw, FLD_GIMMICK_GHOST_GYM) == TRUE &&
				 GYM_EventCheckPLGhostGymLightTrainer(repw) == TRUE) ){
				Player_MoveStateClear( repw->player );
				FieldOBJSys_MovePauseAll( repw->fldobjsys );
				return TRUE;
			}
	...
	if( req->DebugKeyPush == FALSE ){
		...
		if( GYM_GimmickCodeCheck(repw,FLD_GIMMICK_TORNWORLD) == TRUE ){
			if( FldTornWorld_TornWorldCheck(repw) == TRUE ){
				evbit |= HERO_EVBIT_TORNWORLD;
			}
		}
	...
	if(req->StepCheck ){
		...
		if( GYM_GimmickCodeCheck(repw,FLD_GIMMICK_TORNWORLD) == TRUE ){
			...
			if( FldTornWorld_StepEventCheck(repw,dir) == TRUE ){
				return TRUE;
			}
		}
	}
	...
	if( req->TalkCheck ){
		...
		{	// OBJとの会話チェック
			...
			if( TalkObjEventCheck( repw, &obj ) == TRUE ){
				...
				if( Player_MoveForceStopCheck( repw->player ) == TRUE ){
					Player_MoveForceStop(
						repw->player, Player_DirGet(repw->player) );
				}
		...
		{	// OBJとの会話チェック
			...
			if( GYM_GimmickCodeCheck(repw,FLD_GIMMICK_TORNWORLD) == TRUE ){
				ret = TwTalkObjEventCheck( repw, &obj );
			}else{
				ret = TalkObjEventCheck( repw, &obj );
			}
		...
		{	// BGの話しかけチェック
			...
			if( twtype == HEROTWTYPE_NON || twtype == HEROTWTYPE_ON ){
				int	id;
				id = TalkBgEventCheck(
						repw,
						(void *)EventData_GetNowBgTalkData( repw ),
			...
			if( twtype == HEROTWTYPE_NON ){	// 蜜塗り木チェック
				int id;
				if (HTE_CheckHoneyTree(repw,&id)){
					EventSet_Script( repw, id, NULL );
					return TRUE;
		...
		#else	//破れた世界を考慮
		{
			...
			int id = EvCheck_TalkMapAttr( repw, attr );
			if (id != EVENT_ID_NONE) {
				EventSet_Script(repw, id, NULL);
				return TRUE;
			}
		...
		{	//別荘　家具話しかけ
			...
			if( FldVilla_TalkCheck(repw,x,z,dir) == TRUE ){
				return TRUE;
			}
		}
	}
...
#else
			int dir = req->PushSite;
			if(GYM_GimmickCodeCheck(fsys, FLD_GIMMICK_GHOST_GYM) == TRUE){
				GYM_PlGhostGymHintMissDoorCheck( fsys, x, z, &dir );
			}
			MJUMP_ChangeMap(fsys, cnct.zone_id, cnct.door_id,
							0, 0, dir, M_JUMP_DOOR);
#endif
...
u16 EvCheck_TalkMapAttr(FIELDSYS_WORK * repw, MATR matr)
{
	...
	if( Player_FormGet(repw->player) != HERO_FORM_SWIM ){
		...
		if( Player_EventAttrCheck_Naminori(repw->player,natr,matr)
				//&& MyStatus_GetBadgeFlag(my, BADGE_ID_C05)){
				&& MyStatus_GetBadgeFlag(my, BADGE_ID_C06)){			//プラチナ変更
			if( EvPoke_CheckWaza(SaveData_GetTemotiPokemon(repw->savedata),
						WAZANO_NAMINORI) != 0xff) {
				return SCRID_HIDEN_NAMINORI;
			}
...
//--------------------------------------------------------------
/**
 * 破れた世界専用　OBJ話しかけチェック。SXY標準のカウンターチェックは行わない
 * @param	fsys	FIELDSYS_WORK 
 * @param	fldobj	話しかけ対象のFIELD_OBJ_PTR格納先
 * @retval	BOLL	TRUE=起動 FALSE=相手が居ない
 */
//--------------------------------------------------------------
static BOOL TwTalkObjEventCheck( FIELDSYS_WORK *fsys, FIELD_OBJ_PTR *fldobj )
{
	int no = 0;
	int gx,gy,gz,talk_gx,talk_gy,talk_gz;
	
	Player_TwThinkFrontPosGet( fsys->player, &gx, &gy, &gz );
	
	#ifdef PM_DEBUG
	OS_Printf( "破れた世界 話しかけ GX=%d,GY=%d,GZ=%d\n", gx, gy, gz );
	#endif
	
	while( FieldOBJSys_FieldOBJSearch(
		fsys->fldobjsys,fldobj,&no,FLDOBJ_STA_BIT_USE) ){
		talk_gx = FieldOBJ_NowPosGX_Get( *fldobj );
		talk_gy = H_GRID_G_GRID( FieldOBJ_NowPosGY_Get(*fldobj) );
		talk_gz = FieldOBJ_NowPosGZ_Get( *fldobj );
		
		if( gy == talk_gy && gx == talk_gx && gz == talk_gz ){
			return( TRUE );
		}
	}
