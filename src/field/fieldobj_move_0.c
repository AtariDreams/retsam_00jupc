...
static void MvRt4WorkInit( FIELD_OBJ_PTR fldobj, int check_no, int check_type, int tbl_id );
int (* const DATA_MvRt4MoveTbl[])( FIELD_OBJ_PTR fldobj, MV_RT4_WORK *work );
 ...
 */
//--------------------------------------------------------------
void FieldOBJ_MoveRndHLim_Init( FIELD_OBJ_PTR fldobj )
{
	MvRndWorkInit( fldobj, AC_WALK_U_8F, DIRID_MV_RND_H_DirTbl, MV_RND_MOVE_CHECK_LIMIT_ONLY );
}
...
void FieldOBJ_MvRnd_Move( FIELD_OBJ_PTR fldobj )
{
	...
	case 3:
		...
		{
			u32 hit = FieldOBJ_MoveHitCheckDir( fldobj, ret );
			
			if( hit != FLDOBJ_MOVE_HIT_BIT_NON ){
				if( work->move_check_type == MV_RND_MOVE_CHECK_LIMIT_ONLY ){
					if( (hit&FLDOBJ_MOVE_HIT_BIT_LIM) ){ //移動制限のみ見る
						work->seq_no = 0;
						break;
					}
				}else{
...
static void MvRt4WorkInit( FIELD_OBJ_PTR fldobj, int check_no, int check_type, int tbl_id )
{
	...
	work = FieldOBJ_MoveProcWorkInit( fldobj, MV_RT4_WORK_SIZE );
	work->turn_check_no = check_no;
	work->turn_check_type = check_type;
	work->tbl_id = tbl_id;
...
{
	MvRt4WorkInit( fldobj, 2, MV_RT3_CHECK_TYPE_Z, DIRID_MvRtUL_DirTbl );
}
...
{
	MvRt4WorkInit( fldobj, 2, MV_RT3_CHECK_TYPE_Z, DIRID_MvRtDR_DirTbl );
}
...
{
	MvRt4WorkInit( fldobj, 2, MV_RT3_CHECK_TYPE_X, DIRID_MvRtLD_DirTbl );
}
...
{
	MvRt4WorkInit( fldobj, 2, MV_RT3_CHECK_TYPE_X, DIRID_MvRtRU_DirTbl );
}
...
{
	MvRt4WorkInit( fldobj, 2, MV_RT3_CHECK_TYPE_Z, DIRID_MvRtUR_DirTbl );
}
...
{
	MvRt4WorkInit( fldobj, 2, MV_RT3_CHECK_TYPE_Z, DIRID_MvRtDL_DirTbl );
}
...
{
	MvRt4WorkInit( fldobj, 2, MV_RT3_CHECK_TYPE_X, DIRID_MvRtLU_DirTbl );
}
...
{
	MvRt4WorkInit( fldobj, 2, MV_RT3_CHECK_TYPE_X, DIRID_MvRtRD_DirTbl );
}
 ...
 */
//--------------------------------------------------------------
void FieldOBJ_MoveRoute4_Move( FIELD_OBJ_PTR fldobj )
{
	MV_RT4_WORK *work = FieldOBJ_MoveProcWorkGet( fldobj );
	while( DATA_MvRt4MoveTbl[work->seq_no](fldobj,work) == TRUE ){};
}
...
{
	if( FieldOBJ_EventTypeGet(fldobj) == EV_TYPE_TRAINER_ESCAPE ){
		FIELDSYS_WORK *fsys = FieldOBJ_FieldSysWorkGet( fldobj );
		PLAYER_STATE_PTR jiki = Player_FieldSysWorkPlayerGet( fsys );
		int eye_dir = FieldOBJ_DirDispGet( fldobj );
		int eye_range = FieldOBJ_ParamGet( fldobj, FLDOBJ_PARAM_0 );
		int ret = EvTrainer_EyeRangeHitCheck( fldobj, jiki, eye_dir, eye_range );
		
		if( ret != EYE_CHECK_NOHIT ){
			int ac = FieldOBJ_AcmdCodeDirChange( eye_dir, AC_STAY_JUMP_U_8F );
			FieldOBJ_CmdSet( fldobj, ret );
			FieldOBJ_StatusBitON_Move( fldobj );
