...
static void DirTrap_GraphicInit( FE_DIRTRAP_PTR sd )
{
	FE_FROMdl_ResSetArcLoadTexTrans(
		sd->fes, &sd->rmdl, 0, NARC_fldeff_t_yaji_nsbmd, ALLOC_FR );
...
static void EoaDirTrap_Draw( EOA_PTR eoa, void *wk )
{
	...
	
    {
		...
		FRO_OBJ *robj = &work->head.dirtrap->robj;
		
		EOA_MatrixGet( eoa, &pos );
		pos.x += -(FX32_ONE * 1) / 2;
		pos.y += -(FX32_ONE * 1);
		pos.z += DIRTRAP_DRAW_Z_OFFSET;
		
        switch(work->dir){
          case DIR_UP:		y = 270; break;
          case DIR_DOWN:	y = 90; break;
          case DIR_RIGHT:	y = 180; break;
          case DIR_LEFT:
          default:			break;
        }
		
		Rot360_to_MtxFx33( &rot, 0, y, 0 );
		FRO_OBJ_Draw1Mat1Shape( robj, &pos, &scale, &rot );
	}
}
