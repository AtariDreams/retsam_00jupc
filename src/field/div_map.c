...
typedef struct BLOCK_NODE_tag{
	u16 Attribute[32*32];				//アトリビュート
	NNSG3dRenderObj		FloorData;		//地形データレンダーＯＢＪ
	NNSG3dResFileHeader	*FloorResFile;	//地形データリソースヘッダー
	u8				*HeightMem;			//高さデータメモリポインタ
	MHI_PTR			MapHeightInfo;		//高さデータ情報へのポインタ
	int BlockIndex;						//ブロックインデックス
	BOOL DrawOKFlg;						//描画許可フラグ
	M3DOL_PTR	ObjDataList;			//配置モデルＯＢＪリスト
}BLOCK_NODE;

typedef struct MAP_LOAD_BLOCK_ST_tag
{
	BLOCK_NODE *Node[2];	//読み込み対象ブロックデータへのポインタ
	
	TCB_PTR	RandTask;		//地形ロードタスクポインタ
	TCB_PTR	HeightTask;		//高さロードタスクポインタ
	MAP_LOAD_TASK_COUNTER	TaskCnter;	//地形ロードタスク、高さロードタスクのカウンタ
	int Block[2];		//読み込み対象ブロックインデックス
	u8 Index[2];		//読み込み対象ローカルブロックインデックス

	u8 TotalCount;		//読み込み終了カウント数0〜2
	BOOL BlockLoadOK;	//読み込みが完了したかのフラグ
 ...
 * @param	inMapW			マップ幅
 *
 * @return	BOOL			TRUE:縦チェックＯＫ		FALSE:縦チェックＮＧ
 */
//--------------------------------------------------------------------------------------------
 ...
 * @param	inMapW			マップ幅
 *
 * @return	BOOL			TRUE:横チェックＯＫ		FALSE:横チェックＮＧ
 */
//--------------------------------------------------------------------------------------------
								...
								int *outBlockTbl)
{
	...
	if( BlockMode == BLOCKMODE_2x2 ){		//2x2ブロック限定モード
		int i;
 ...
 * ブロックデータノードの解放
 *
 * @param	inFreeNo1		解放ブロックノードナンバー１つ目
 * @param	inFreeNo2		解放ブロックノードナンバー２つ目
 * @param	inMoveNo1		新規読み込みブロックノードナンバー１つ目
 * @param	inMoveNo2		新規読み込みブロックノードナンバー２つ目
 * @param	ioDivMapCont	分割マップ構造体ポインタ
 *
 * @return	none
 */
 ...
 * @param	inMapH			マップ縦幅
 * @param	inMapGridW		マップグリッド横幅
 * @param	inNowGridNo		現在グリッドインデックス
 *
 * @return	u32				グリッドナンバー
 */
//--------------------------------------------------------------------------------------------
 ...
 *
 * @param	inDirection		方向
 * @param	inNowPosInBlock	現在ブロック内のどこにいるか（0〜3）
 *
 ...
 * @param	inDirection			更新方向
 * @param	inNowPosInBlock		現在ブロック内のどこにいるか（0〜3）
 * @param	outNowLocalIndex	ローカルインデックス格納バッファ
 *
 * @return	none
 */
//--------------------------------------------------------------------------------------------
	...
	DMC_PTR outDivMapCont )
{
	...
	if( outDivMapCont->BlockMode == BLOCKMODE_2x2 ){	//0,1,2,3固定
 ...
 * @param	mapresource		マップリソース
 * @param	inFld3DAnmPtr	フィールド3Ｄアニメポインタ
 * @param	inMode			読み込みモード
 * 
 * @retval  DMC_PTR			分割マップ構造体ポインタ		
 */
//==============================================================================
 ...
 *
 * @param	inDivMapCont	分割マップ構造体ポインタ
 * @param	glst_data		グローバルステートデータポインタ
 *
 * @return	none
 */
//==============================================================================
 ...
 * @param	inDivMapCont	分割マップ構造体ポインタ
 *
 * @return	BOOL		TRUE:解放終了		FALSE:解放待ち
 */
//==============================================================================
 ...
 *
 * @param	inIdx			ローカルインデックス
 * @param   inDivMapCont	分割マップ構造体
 * @param	outObjList		OBJリストポインタ
 *
 * @return	none
 */
//==============================================================================
...
//==============================================================================
/**
 * 現在のローカルブロックインデックスを取得
 *
 * @param   inDivMapCont　分割マップ構造体
 *
 * @return	u8		現在のローカルブロックインデックス
 */
//==============================================================================
 ...
 * @param	inGridX			グリッドＸ座標
 * @param	inGridZ			グリッドＺ座標
 * @param	outLocalIndex	ローカルインデックス格納バッファ
 *
 * @return	BOOL			TRUE:内側　 FALSE:外側
 */
//==============================================================================
...
//==============================================================================
/**
 * ブロックロード後のコールバック関数のセット
 *
 * @param   outDivMapCont	分割マップ構造体
 * @param	call_back		コールバック
 * @param	waork			コールバック用ワーク
 *
 * @return	none
 */
//==============================================================================
 ...
 * @param	inSize			データサイズ
 * @param	object_p		レンダーＯＢＪポインタ
 * @param	resFile			リソースファイル
 * @param	pTexture		テクスチャ
 * @param	ioDrawOKFlg		描画許可フラグ
 * @param	inEndFlg		タスク終了フラグ
 *
 * @return	TCB_PTR			タスクポインタ
 */
//==============================================================================
		...
		ARCHANDLE **handle, u32 *size )
{
	...
	#ifdef PM_DEBUG
	if( data.MapSize > 0xf000 ){
		OS_Printf("arc%d:地形サイズオーバー:%d\n",arc_index,data.MapSize);
		GF_ASSERT(0);
	}
	#endif
	
	seek = (0x800) + data.ObjSize;		//0x800=Attribute
	ArchiveDataSeekByHandleCur( DivMapCont->ArcHandle, seek );	//3DMdl位置
...
//--------------------------------------------------------------
/**
 * ダミーDMC_PTR初期化。地形モデルのみを展開するDMC_PTR
 * @param	world	WORLD_MAP_PTR
 * @param	MAP_RESOURCE_PTR mapresource
 * @param	ARCHANDLE * NULL=新規に作成
 ...
 * @param	list	0-3
 * @param	block_no	LoadBlockListSetUpForDummyで作成したリスト
 * @retval	nothing
 */
//--------------------------------------------------------------
									...
									DMC_PTR outDivMapCont)
{
	...
	seek = (0x800) + data.ObjSize;		//0x800=Attribute 3DMdl位置
