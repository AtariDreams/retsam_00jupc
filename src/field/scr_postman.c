...
static const MP_ACTION MPActionTable[13];
...
{
    return FUSHIGIDATA_GetSlotType(FUSHIGIDATA_GetSlotData());
}
...
{
    return FUSHIGIDATA_GetSlotPtr(FUSHIGIDATA_GetSlotData());
}
//-----------------------------------------------------------------------------
/**
...
{
	FUSHIGIDATA_RemoveSlot(FUSHIGIDATA_GetSlotData());
}
...
static void Pokemon_ReceiveFunc(FIELDSYS_WORK * fsys, void * slot)
{
    ...
    if(PokeSexGet(pPP) != PokeParaGet(pPP, ID_PARA_sex, NULL))
      OS_TPrintf("■■性別が食い違っています！！\n");
#endif
    // 固体乱数に基づき性別を設定
    sum = PokeSexGet(pPP);
    PokeParaPut(pPP, ID_PARA_sex, (u8 *)&sum);
    // 特性をセット(これは適用しない事になりました。任天堂吉原さんより)
...
#if 1	//08.04.10 プラチナ追加
    if( PokeParaGet(pPP,ID_PARA_monsno,NULL) == MONSNO_AUSU ){
	
		//アルセウスは、DPの波乗りバグの影響がないため必要なし！(08.04.22)
		//配布する時は必ず配布フラグが立っているので必要！(08.05.28)
		if( PokeParaGet(pPP,ID_PARA_event_get_flag,NULL) == 1 ){

			//まだアルセウスイベント起動していなかったら
...
static BOOL Key_ReceiveCheckFunc(FIELDSYS_WORK * fsys, void * slot)
{
    ...
    return MyItem_AddCheck(myitem, ITEM_HIMITUNOKAGI, 1, HEAPID_EVENT);
}

//-----------------------------------------------------------------------------
static void Key_ReceiveFunc(FIELDSYS_WORK * fsys, void * slot)
{
    MYITEM* myitem = SaveData_GetMyItem(fsys->savedata);
    GIFT_PRESENT* pGift = MP_GetSlotWork(fsys);
	EVENTWORK * ev = SaveData_GetEventWork(fsys->savedata);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
static void Key_OKMsgFunc(MSG_WORK * msgwk, u16 * arc_id, u16 * msg_id)
{
    GIFT_PRESENT* pGift = MP_GetSlotWork(msgwk->fsys);
    u16 item_no =  ITEM_HIMITUNOKAGI;

    *arc_id = NARC_msg_haitatu_dat;
	*msg_id = msg_haitatu_22;			//★ひみつのかぎ　正式な名前なのか確認
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
static void Key_NGMsgFunc(MSG_WORK * msgwk, u16 * arc_id, u16 * msg_id)
{
    MYITEM* myitem = SaveData_GetMyItem(msgwk->fsys->savedata);
    GIFT_PRESENT* pGift = MP_GetSlotWork(msgwk->fsys);
    u16 item_no =  ITEM_HIMITUNOKAGI;
