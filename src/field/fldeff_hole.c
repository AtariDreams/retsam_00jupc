...
static void Hole_GraphicInit( FE_HOLE_PTR sd )
{
	...
	for(i = 0;i < 32;i++){
		FE_FROMdl_ResSetArcLoadTexTrans(
			sd->fes, &sd->rmdl[i], 0, NARC_fldeff_ug_strap_01_nsbmd+i, ALLOC_FR );
		FRO_OBJ_InitInMdl( &sd->robj[i], &sd->rmdl[i] );
	}	
	
	FE_FROMdl_ResSetArcLoadTexTrans(
		sd->fes, &sd->rmdl[i], 0, NARC_fldeff_hole_nsbmd, ALLOC_FR );
	FRO_OBJ_InitInMdl( &sd->robj[i], &sd->rmdl[i] );
}
...
static void EoaHole_Draw( EOA_PTR eoa, void *wk )
{
	...
	
    {
		...
		FRO_OBJ *robj = &work->head.hole->robj[work->modelNo];
