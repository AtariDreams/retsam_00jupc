...
//--------------------------------------------------------------
/**
 * 木の実OBJが使用しているBLACT_WORK_PTRを取得。
 * 木の実限定
 * @param	fldobj	FIELD_OBJ_PTR
 * @retval	blact	BLACT_WORK_PTR
 */
//--------------------------------------------------------------
BLACT_WORK_PTR FieldOBJ_SeedBlActPtrGet( FIELD_OBJ_PTR fldobj )
{
	BLACT_WORK_PTR blact = NULL;
	
	GF_ASSERT( FieldOBJ_OBJCodeSeedCheck(FieldOBJ_OBJCodeGet(fldobj)) );
