...
//--------------------------------------------------------------
/**
 * 使用チェックワーク作成 やぶれたせかい特殊壁面用 波乗り以外の設定は無い
 * @param	fsys	フィールドワーク
 * @param	id		チェックワーク
 * @return	none
 */
		...
		FIELDSYS_WORK *fsys, SKILLCHECK_WORK *scwk )
{
	...
	dir = Player_TwThinkDirGet( fsys->player );
	fattr = Player_TwThinkNextDirAttrGet( fsys->player, dir );
	nattr = Player_TwThinkNowAttrGet( fsys->player );
	
	if( Player_EventAttrCheck_Naminori(fsys->player,nattr,fattr) == TRUE ){
		scwk->enable_skill |= (1 << FLD_SKILL_NAMINORI);
	}
}
...
static int SkillCheck_Naminori( const SKILLCHECK_WORK * scwk)
{
	...
	if( BadgeCheck( scwk, BADGE_ID_C06 ) == FALSE ){		//プラチナで変更
		return FIELDSKILL_NO_BADGE;
	}
...
static int SkillCheck_Kiribarai( const SKILLCHECK_WORK * scwk)
{
	...
	if( BadgeCheck( scwk, BADGE_ID_C05 ) == FALSE ){		//プラチナで変更
		return FIELDSKILL_NO_BADGE;
	}
...
static int SkillCheck_Osyaberi( const SKILLCHECK_WORK * scwk )
{
	...
	#else	//add pl
	if( MapModeUseChack(scwk) == FALSE ||
		GYM_GimmickCodeCheck(scwk->fsys,FLD_GIMMICK_TORNWORLD) == TRUE ){
		return FIELDSKILL_USE_FALSE;
	}
	#endif
