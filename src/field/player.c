...
#define JIKI_MOVE_BIT_TWORLD			(1<<8)		///<破れた世界動作
#define JIKI_MOVE_BIT_TWORLD_GROUND		(1<<9)
#define JIKI_MOVE_BIT_TWORLD_WLEFT		(1<<10)		///<破れた世界 左壁動作
#define JIKI_MOVE_BIT_TWORLD_WRIGHT		(1<<11)		///<破れた世界 右壁動作
#define JIKI_MOVE_BIT_TWORLD_ROOF		(1<<12)		///<破れた世界 天井動作
...
PLAYER_STATE_PTR Player_Init( CONST_FIELD_OBJ_SYS_PTR fos,
		int x, int z, int dir, int form, int sex, HEROVER ver, PLAYER_SAVE_DATA *save )
{
	...
	
	{
		...
		if( ver == HEROVER_PL ){
			code = Player_FormSexOBJCodeGet( form, sex );
		}else{
			if( sex == PM_MALE ){
				code = DPHERO;
			}else{
 ...
 */
//--------------------------------------------------------------
void Player_DrawInit( PLAYER_STATE_PTR jiki, int g_id )
{
	int code,ret;
	FIELD_OBJ_PTR fldobj;
...
//	FieldOBJ_BlActResmRegularAdd_Anm( fos, ret );
	...
	#elif 0		//破れた世界を考慮
	/* 復帰時はMOVE_BITがクリアされている。
	 * 破れた世界でも復帰直後はHEROTWTYPE_NONの為、
	 * 通常波乗りポケモンを出そうとする為エラーが発生
	 */
	if( Player_FormGet(jiki) == HERO_FORM_SWIM ){
		EOA_PTR eoa;
		int gx = Player_NowGPosXGet( jiki );
		...
		if( tw == HEROTWTYPE_NON ){
			eoa =  FE_FldOBJNamiPokeSet( fldobj, gx, gz, dir, TRUE );
		}else{
			eoa =  FE_FldOBJNamiPokeRSet( jiki, gx, 0, gz, dir, TRUE, tw );
		}
		
		Player_JointEoaSet( jiki, eoa );
	}
	#else
	/*	破れた世界時の波乗りポケモンは破れた世界側でセットしてもらう*/
	if( Player_FormGet(jiki) == HERO_FORM_SWIM ){
...
static void Jiki_WorkInit( PLAYER_STATE_PTR jiki, int form, int sex, PLAYER_SAVE_DATA *save )
{
	...
	Player_AcmdCodeTypeSet( jiki, ACMD_NOT, HEROACTYPE_NON );
...
int Player_TwThinkDirGet( const PLAYER_STATE_PTR jiki )
{
	if( Player_MoveBitCheck_Flat(jiki) == TRUE ){
		return( Player_DirGet(jiki) );
	}
	
	return( Player_DirMoveGet(jiki) );
}
 ...
 */
//--------------------------------------------------------------
void Player_AcmdTypeSet( PLAYER_STATE_PTR jiki, HEROACTYPE type )
{
	jiki->set_ac_type = type;
}
 ...
 * 自機アニメーションコマンドタイプ取得
 * @param	jiki	PLAYER_STATE_PTR
 * @retval	u32		JIKIACTYPE_NON等
 */
//--------------------------------------------------------------
HEROACTYPE Player_AcmdTypeGet( PLAYER_STATE_PTR jiki )
{
	return( jiki->set_ac_type );
}

//--------------------------------------------------------------
/**
 * 自機アニメーションコマンドコード、タイプセット
 * @param	jiki	PLAYER_STATE_PTR
 * @param	ac		アニメコードAC_DIR_U等
 * @retval	nothing
 */
//--------------------------------------------------------------
void Player_AcmdCodeTypeSet( PLAYER_STATE_PTR jiki, u32 ac, HEROACTYPE type )
{
	Player_AcmdCodeSet( jiki, ac );
	Player_AcmdTypeSet( jiki, type );
}
...
int Player_FormSexOBJCodeGet( int form, int sex )
{
	if( sex == PM_MALE ){
		...
		case HERO_DRAWFORM_BS:		return( BSHERO );
		case HERO_DRAWFORM_TW:		return( RTHERO );
		case HERO_DRAWFORM_TWSWIM:	return( SWIMHERORT );
		case HERO_DRAWFORM_TWSAVE:	return( SAVEHERORT );
		case HERO_DRAWFORM_TWPOKETCH:	return( POKEHERORT );
		case HERO_DRAWFORM_GSTGYM:	return( GHOSTHERO );
		case HERO_DRAWFORM_GSTGYM_SAVE:	return( GSTSAVEHERO );
		case HERO_DRAWFORM_GSTGYM_POKETCH: return( GSTPOKEHERO );
		}
	}else{
		switch( form ){
		...
		case HERO_DRAWFORM_BS:		return( BSHEROINE );
		case HERO_DRAWFORM_TW:		return( RTHEROINE );
		case HERO_DRAWFORM_TWSWIM:	return( SWIMHEROINERT );
		case HERO_DRAWFORM_TWSAVE:	return( SAVEHEROINERT );
		case HERO_DRAWFORM_TWPOKETCH:	return( POKEHEROINERT );
		case HERO_DRAWFORM_GSTGYM:	return( GHOSTHEROINE );
		case HERO_DRAWFORM_GSTGYM_SAVE:	return( GSTSAVEHEROINE );
		case HERO_DRAWFORM_GSTGYM_POKETCH: return( GSTPOKEHEROINE );
		}
	}
...
{
	Jiki_MoveBitOFF( jiki, JIKI_MOVE_BIT_TWORLD_ALL );	//一旦消去
	
	switch( type ){
	case HEROTWTYPE_NON:
		return;
	case HEROTWTYPE_ON:
		Jiki_MoveBitON( jiki, JIKI_MOVE_BIT_TWORLD ); return;
	case HEROTWTYPE_GROUND:
		Jiki_MoveBitON( jiki, JIKI_MOVE_BIT_TWORLD_GROUND ); return;
	case HEROTWTYPE_WLEFT:
		Jiki_MoveBitON( jiki, JIKI_MOVE_BIT_TWORLD_WLEFT ); return;
	case HEROTWTYPE_WRIGHT:
		Jiki_MoveBitON( jiki, JIKI_MOVE_BIT_TWORLD_WRIGHT ); return;
	case HEROTWTYPE_ROOF:
		Jiki_MoveBitON( jiki, JIKI_MOVE_BIT_TWORLD_ROOF ); return;
	}
 ...
 */
//--------------------------------------------------------------
HEROTWTYPE Player_MoveBitCheck_TWorld( CONST_PLAYER_STATE_PTR jiki )
{
	HEROTWTYPE type = HEROTWTYPE_NON;
	u32 ret = Jiki_MoveBitCheck( jiki, JIKI_MOVE_BIT_TWORLD_ALL );
	
	switch( ret ){
	case JIKI_MOVE_BIT_TWORLD:
		type = HEROTWTYPE_ON;
		break;
	case JIKI_MOVE_BIT_TWORLD_GROUND:
		type = HEROTWTYPE_GROUND;
		break;
	case JIKI_MOVE_BIT_TWORLD_WLEFT:
		type = HEROTWTYPE_WLEFT;
		break;
	case JIKI_MOVE_BIT_TWORLD_WRIGHT:
		type = HEROTWTYPE_WRIGHT;
		break;
	case JIKI_MOVE_BIT_TWORLD_ROOF:
		type = HEROTWTYPE_ROOF;
		break;
	}
...
//--------------------------------------------------------------
/**
 * 自機動作ビットチェック　破れた世界特殊地形
 * @param	jiki	PLAYER_STATE_PTR
 * @retval	BOOL	破れた世界特殊地形である
 */
//--------------------------------------------------------------
BOOL Player_MoveBitCheck_TWorldSp( CONST_PLAYER_STATE_PTR jiki )
{
	...
	if( type == HEROTWTYPE_NON || type == HEROTWTYPE_ON ){
		return( FALSE );
	}
