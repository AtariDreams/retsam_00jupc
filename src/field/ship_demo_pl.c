...
#include "arc/ship_demo_pl.naix"

#include "ship_demo.h"

#define ANIME_TYPE_NUM	(3)//(4)

#define	ANM_ICA	(0)


typedef struct SHIP_DEMO_WORK_PL_tag
{
...
static ARC_TBL ArcTbl[4] = {
	{
		NARC_ship_demo_pl_g_demo_R_nsbmd,
		{
			NARC_ship_demo_pl_g_demo_R_nsbca,
			NARC_ship_demo_pl_g_demo_R_nsbma,
			NARC_ship_demo_pl_g_demo_R_nsbta,
		}
	},
	{
		NARC_ship_demo_pl_g_demo_L_nsbmd,
		{
			NARC_ship_demo_pl_g_demo_L_nsbca,
			NARC_ship_demo_pl_g_demo_L_nsbma,
			NARC_ship_demo_pl_g_demo_L_nsbta,
		}
	},
	{
		NARC_ship_demo_pl_g_demo_R_nsbmd,
 ...
 */
//--------------------------------------------------------------------------------------------
PROC_RESULT ShipDemoGarinkoProc_Init( PROC * proc, int * seq )
{
	u8 i;
	SHIP_DEMO_WORK *wk;
	SHIPDEMO_DAT *dat;
 ...
 */
//--------------------------------------------------------------------------------------------
PROC_RESULT ShipDemoGarinkoProc_Main( PROC * proc, int * seq )
{
	u8 i;
	BOOL end_flg;
 ...
 */
//--------------------------------------------------------------------------------------------
PROC_RESULT ShipDemoGarinkoProc_End( PROC * proc, int * seq )
{
	u8 i;
	SHIP_DEMO_WORK * wk = PROC_GetWork( proc );
	//アニメ解放
...
static void LoadData(SHIP_DEMO_WORK *wk)
{
	...
	ARCHANDLE* handle;
	//アーカイブハンドルオープン
	handle = ArchiveDataHandleOpen( ARC_SHIP_DEMO_PL, HEAPID_SHIPDEMO );
	//アニメアロケータ
