		...
		PLAYER_STATE_PTR jiki, CONST_FIELD_OBJ_PTR nonobj, EYE_MEET_HITDATA *hit )
{
	...
	while( FieldOBJSys_FieldOBJSearch(fos,&fldobj,&i,FLDOBJ_STA_BIT_USE) ){
		if( nonobj == NULL || (nonobj != fldobj) ){
			range = TrEyeRangeCheck( fldobj, jiki, &dir );
...
{
	return( TrEyeRangeCheck(fldobj,jiki,hit_dir) );
}
	...
	CONST_FIELD_OBJ_PTR fldobj, PLAYER_STATE_PTR jiki, int eye_dir, int eye_range )
{
	...
	int ret = TrEyeLineCheck( fldobj, eye_dir, eye_range, jx, jz, 0 );
	
	if( ret != EYE_CHECK_NOHIT ){
		if( TrEyeLineMoveHitCheck(fldobj,eye_dir,ret) == TRUE ){
			ret = EYE_CHECK_NOHIT;
		}
	}
...
{
	return( TrEyeMeetMoveAddTcb(fsys,fldobj,jiki,dir,range,gyoe,tr_type,work_pos) );
}
...
{
	int dir;
	return( TrEyeRangeCheck(fldobj,jiki,&dir) );
}
