...
//	fes->lres = lres;
	...
	fes->subproc_regdata = sys_AllocMemory( heap_id, SUBPROC_REGDATA_SIZE * reg_max );
...
//	LRES *lres = fes->lres;
	...
	sys_FreeMemoryEz( fes->subproc_regdata );
...
{
	void *alloc;
	if( type == ALLOC_FR ){ alloc = sys_AllocMemory( fes->heap_id, size ); }
	else{ alloc = sys_AllocMemoryLo( fes->heap_id, size ); }
	return( alloc );
}
...
{
	sys_InitAllocator( pAllocator, fes->heap_id, 32 );
}
...
void * FE_EffectWorkGet( FE_SYS *fes, u32 id )
{
	...
	if( subproc == NULL ){
		OS_Printf( "FE_EffectWorkGet() not regist ID %d\n", id );
		GF_ASSERT( 0 );
		return( NULL );
	}
...
//--------------------------------------------------------------
/**
 * フィールドエフェクト用アーカイブハンドル取得
 * @param	fes		FE_SYS *
 * @retval	nothing
 */
//--------------------------------------------------------------
ARCHANDLE * FE_ArcHandleGet( FE_SYS *fes )
{
	return( fes->arc_handle );
}
...
void * FE_ArcDataLoadAlloc( FE_SYS *fes, u32 datid, ALLOCTYPE type )
{
	u32 size = ArchiveDataSizeGetByHandle( fes->arc_handle, datid );
	void *alloc = FE_AllocMemory( fes, size, type );
	ArchiveDataLoadByHandle( fes->arc_handle, datid, alloc );
	return( alloc );
}
...
static void fe_EoaInit( FE_SYS *fes )
{
	fes->eoa_sys = EOA_SysInit( fes->heap_id, fes->eoa_max );
}
...
//--------------------------------------------------------------
/**
 * フィールドエフェクトパラメタ指定 FRO_MDL_ResSetArcLoad()
 * @param	fes		FE_SYS
 * @param	rmdl	FRO_MDL
 * @param	idx	
 * @param	arc_idx
 ...
 */
//--------------------------------------------------------------
void FE_FROMdl_ResSetArcLoad( FE_SYS *fes, FRO_MDL *rmdl,
		u32 idx, u32 arc_idx, ALLOC_TYPE alloc_type )
{
	FRO_MDL_ResSetArcLoad(rmdl, idx, fes->arc_handle, arc_idx, fes->heap_id, alloc_type);
}

//--------------------------------------------------------------
/**
 * フィールドエフェクトパラメタ指定 FRO_MDL_ResSetArcLoad()&FRO_MDL_TexTransBindVTaskAdd()
 * @param	fes		FE_SYS
 * @param	rmdl	FRO_MDL
 * @param	idx	
 * @param	arc_idx
 ...
 */
//--------------------------------------------------------------
void FE_FROMdl_ResSetArcLoadTexTrans( FE_SYS *fes, FRO_MDL *rmdl,
		u32 idx, u32 arc_idx, ALLOC_TYPE alloc_type )
{
	FRO_MDL_ResSetArcLoad(rmdl, idx, fes->arc_handle, arc_idx, fes->heap_id, alloc_type);
	FRO_MDL_TexTransBindVTaskAdd( rmdl );
}

//--------------------------------------------------------------
/**
 * フィールドエフェクトパラメタ指定 FRO_ANM_AnmResSetArcLoad()
 * @param	fes		FE_SYS
 * @param	ranm	FRO_ANM
 ...
 */
//--------------------------------------------------------------
void FE_FROAnm_AnmResSetArcLoad( FE_SYS *fes, FRO_ANM *ranm, 
		u32 idx, u32 arc_idx, u32 alloc_type )
{
	FRO_ANM_AnmResSetArcLoad(ranm, idx, fes->arc_handle, arc_idx, fes->heap_id, alloc_type);
}

//--------------------------------------------------------------
/**
 * フィールドエフェクトパラメタ指定 FE_FROAnm_AnmResObjInitSame()
 * @param	fes		FE_SYS
 * @param	ranm	FRO_ANM
 * @param	rmdl	FRO_MDL
 * @param	pRes	アニメリソース
 ...
 */
//--------------------------------------------------------------
void FE_FROAnm_AnmResObjInitSame( FE_SYS *fes, FRO_ANM *ranm, const FRO_MDL *rmdl,
		FRO_ANM *m_ranm, u32 idx )
{
	FRO_ANM_AnmResObjInitSame( ranm, rmdl, m_ranm, idx, fes->heap_id );
}
