...
static void Arrow_GraphicInit( FE_ARROW_PTR arrow )
{
	FE_FROMdl_ResSetArcLoadTexTrans(
		arrow->fes, &arrow->rmdl, 0, NARC_fldeff_h_exit_nsbmd, ALLOC_FR );
	FRO_OBJ_InitInMdl( &arrow->robj, &arrow->rmdl );
}
...
static void EoaArrow_Draw( EOA_PTR eoa, void *wk )
{
	...
	
	{
		...
		FRO_OBJ *robj = &work->head.arrow->robj;
