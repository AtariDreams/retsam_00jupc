...
struct TV_WORK {
	...
	//バトルステージで50連勝？した時のポケモンを保持
	TOPICTEMP_STAGE StageTemp;				///<質問型：バトルステージ用状況保持ワーク★
	
	//友達の名前、遊んだ施設を保持
	TOPICTEMP_FRIEND FriendTemp;			///<質問型：フロンティアで仲良し用状況保持ワーク★
	
	//一日に稼いだバトルポイントを保持
	TOPICTEMP_BP BPTemp;					///<ギネス型：バトルポイント名人用状況保持ワーク★
	
	//一日に交換した回数を保持
	TOPICTEMP_GTS GTSTemp;					///<ギネス型：GTS名人用状況保持ワーク★
};
