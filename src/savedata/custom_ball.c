...
#if (CRC_LOADCHECK && CRCLOADCHECK_GMDATA_ID_CUSTOM_BALL)
	SVLD_SetCrc(GMDATA_ID_CUSTOM_BALL);
#endif //CRC_LOADCHECK
}
...
#if (CRC_LOADCHECK && CRCLOADCHECK_GMDATA_ID_CUSTOM_BALL)
	SVLD_CheckCrc(GMDATA_ID_CUSTOM_BALL);
#endif //CRC_LOADCHECK
	return cbs;
}
...
void Debug_CB_SaveData_Sample(CB_SAVE_DATA* save, int mode)
{
	...
	
	{
		...
		//	break;
			for (i = CB_DEF_INDEX_ORIGIN; i < CB_DEF_ITEM_DATA_NUM; i++){
				if (i % 2){
					item->seal[ i - CB_DEF_INDEX_ORIGIN ] = 99;
				}
				else {
					item->seal[ i - CB_DEF_INDEX_ORIGIN ] = 99;
				}
			}
			for (i = CB_DEF_INDEX_ORIGIN; i < 9; i++){
				item->seal[ i - CB_DEF_INDEX_ORIGIN ] = 8;
			}
		...
		case DEBUG_CB_MODE_ITEM_FULL:
			for (i = CB_DEF_INDEX_ORIGIN; i < 10; i++){
				item->seal[ i - CB_DEF_INDEX_ORIGIN ] = 8;
			}
			for (;i< CB_DEF_ITEM_DATA_NUM; i++){
				item->seal[ i - CB_DEF_INDEX_ORIGIN ] = 0;
		...
		case DEBUG_CB_MODE_ITEM_RANDOM:
			for (i = CB_DEF_INDEX_ORIGIN; i < CB_DEF_ITEM_DATA_NUM; i++){
				item->seal[ i - CB_DEF_INDEX_ORIGIN] = 99; ///< 適当
			}
			if ( 0 )
			{
				CB_CORE core;				
