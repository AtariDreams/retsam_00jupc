...
{
	//OS_Printf( "TV_WORK = %d\n", sizeof(TV_WORK) );
	//OS_Printf( "TOPICTEMP_CONTEST = %d\n", sizeof(TOPICTEMP_CONTEST) );
	//OS_Printf( "TOPICTEMP_STAGE = %d\n", sizeof(TOPICTEMP_STAGE) );
	//OS_Printf( "TOPICTEMP_FRIEND = %d\n", sizeof(TOPICTEMP_FRIEND) );
	//OS_Printf( "TOPICTEMP_BP = %d\n", sizeof(TOPICTEMP_BP) );
	//OS_Printf( "TOPICTEMP_GTS = %d\n", sizeof(TOPICTEMP_GTS) );
...
static MY_TOPIC * GetMyTopicByTopicType(TV_WORK * tvwk, TVTOPICTYPE topic_type)
{
	...
	case TVTOPICTYPE_VARIETY:			//★バラエティ型追加
		GF_ASSERT(0);
	}
	return mt;
}
		...
		int topic_id, BOOL see_flag, u8 * array)
{
	...
	OS_Printf( "max = %d\n", max );
	OS_Printf( "topic_id = %d\n", topic_id );

	for (n = 0, i = 0; i < max; i++) {

		OS_Printf( "topic_id[%d] = %d\n", i, topic[i].topic.topic_id );
		...
		int topic_id, BOOL mine, BOOL see, u8 * array)
{
	...
	case TVTOPICTYPE_EVENT:
		...
		return 0;
	case TVTOPICTYPE_VARIETY:		//★
		//GF_ASSERT(0);
		//万が一バラエティ型を検索しても０＝＝データなしで返す
		return 0;
	}
	return 0;
}
...
//----------------------------------------------------------
/**
 * @brief	状況保持ワーク取得：サファリ
 * @param	tvwk
 * @return	TOPICTEMP_SAFARIへのポインタ
 */
//----------------------------------------------------------
...
//----------------------------------------------------------
/**
 * @brief	状況保持ワーク取得：バトルステージ★
 * @param	tvwk
 * @return	TOPICTEMP_STAGEへのポインタ
 */
//----------------------------------------------------------
TOPICTEMP_STAGE * TVWORK_GetStageTemp(TV_WORK * tvwk)
{
	return &tvwk->StageTemp;
}
//----------------------------------------------------------
/**
 * @brief	状況保持ワーク取得：フロンティアで仲良し★
 * @param	tvwk
 * @return	TOPICTEMP_FRIENDへのポインタ
 */
//----------------------------------------------------------
TOPICTEMP_FRIEND * TVWORK_GetFriendTemp(TV_WORK * tvwk)
{
	return &tvwk->FriendTemp;
}
//----------------------------------------------------------
/**
 * @brief	状況保持ワーク取得：バトルポイント名人★
 * @param	tvwk
 * @return	TOPICTEMP_BPへのポインタ
 */
//----------------------------------------------------------
...
//----------------------------------------------------------
/**
 * @brief	状況保持ワーク取得：GTS名人★
 * @param	tvwk
 * @return	TOPICTEMP_GTSへのポインタ
 */
//----------------------------------------------------------
