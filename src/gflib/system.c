...
#endif	//AFTERMASTER_070122_WARNING_DISP_FIX
...
//----------------------------------------------------------------
#if CRC_LOADCHECK
#define HEAPSIZE_SYSTEM		(0x00d200)
#define HEAPSIZE_SAVE		(0x020e00)
#else //CRC_LOADCHECK
#define HEAPSIZE_SYSTEM		(0x00d000)
#define HEAPSIZE_SAVE		(0x021000)
#endif //CRC_LOADCHECK
#define HEAPSIZE_DEBUG		(0x000010)  //必要が無いので最小に変更することが可能08.05.16 tomoya	
...
void sys_InitSystem(void)
{
	...
	sys.pDontSioError = NULL;	// 通信エラー禁止を示すフラグへのポインタ
...
//--------------------------------------------------------------
/**
 * @brief   通信エラーにしない為のマジックキーをセット
 *
 * @param   heap_id		マジックキーをセットする為のフラグを確保するヒープ
 */
//--------------------------------------------------------------
void sys_SioErrorNG_PtrSet(int heap_id)
{
#if 1
	GF_ASSERT(sys.pDontSioError == NULL);
	sys.pDontSioError = sys_AllocMemoryLo(heap_id, sizeof(u32));
	*(sys.pDontSioError) = SYS_DONT_SIO_ERROR_KEY;
#endif
}

//--------------------------------------------------------------
/**
 * @brief   通信エラーにしない為のマジックキーを解放
 */
//--------------------------------------------------------------
void sys_SioErrorNG_PtrFree(void)
{
	...
	GF_ASSERT(sys.pDontSioError != NULL);
	*(sys.pDontSioError) = 0;
	sys_FreeMemoryEz(sys.pDontSioError);
	sys.pDontSioError = NULL;
#endif
}

//--------------------------------------------------------------
/**
 * @brief   通信エラーにしてはいけない状態かチェック
 *
 * @retval  TRUE:通信エラーにしてはいけない
 * @retval  FALSE:通信エラーにして良い
 */
//--------------------------------------------------------------
BOOL sys_SioErrorNG_Get(void)
{
	if(sys.pDontSioError != NULL && *(sys.pDontSioError) == SYS_DONT_SIO_ERROR_KEY){
		return TRUE;
	}
	return FALSE;
}
