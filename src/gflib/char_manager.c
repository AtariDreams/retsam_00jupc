...
#ifdef PM_DEBUG
//#define DEBUG_PRINT_VRAM_CONT	// VRAM管理情報をプリント
#endif
...
BOOL CharVramAreaAlloc( int szByte, int cont_type, int type, CHAR_MANAGER_ALLOCDATA* allocData )
{
	...
	if( cont_type == CHARM_CONT_OFFSET ){
		...
		if(check){
			...
			allocData->conttype = CHARM_CONT_OFFSET;
		}
		...
		if(check){
			...
			if( type == NNS_G2D_VRAM_TYPE_2DMAIN ){
				...
				allocData->alloc_ofs	= offs_m + CharManager->MainVramTransStart;
			}else{
				allocData->alloc_size	= size_s;
				allocData->alloc_ofs	= offs_s + CharManager->SubVramTransStart;
			}
			allocData->conttype = CHARM_CONT_AREACONT;
		}
	}
...
#ifdef DEBUG_PRINT_VRAM_CONT
		DEBUG_Print_VitArea( p_trans_flag );
#endif
	}
}
...
static void DEBUG_Print_VitArea( u8* p_trans_flag )
{
	...
	if( p_trans_flag == CharManager->TransAreaFlagMain ){
		OS_TPrintf( "main vram  bit cont\n" );
		char_vram_max = CharManager->TransAreaMainSize;
	}else{
		OS_TPrintf( "sub vram  bit cont\n" );
		char_vram_max = CharManager->TransAreaSubSize;
	}
