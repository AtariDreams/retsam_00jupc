...
fx32 Sin360(u16 deg)
{
...
#if CALCTOOL_SINCOSTBLDEL
	{
		...
		ans = FX_SinIdx( _RotKey(deg) );
		sin = FX16_TO_FX32( ans );
		return sin;
	}
#else
...
fx32 Cos360(u16 deg){
...
#if CALCTOOL_SINCOSTBLDEL
	{
		...
		ans = FX_CosIdx( _RotKey(deg) );
		cos = FX16_TO_FX32( ans );
		return cos;
	}
#else
...
#if CALCTOOL_SINCOSTBLDEL
	return FX_DEG_TO_IDX( deg*FX32_ONE );
#else
...
#if CALCTOOL_SINCOSTBLDEL
	u16 deg_u16 = deg >> FX32_SHIFT;
	return Sin360R( deg_u16 );
#else
...
{
    return MATH_CalcCRC16CCITT(&_pCalcTable->crc_table, adrs, size);
}
...
{
    GF_ASSERT(_pCalcTable==NULL);
    OS_TPrintf("size %d \n",sizeof(_CALCTOOL_TABLE));
    _pCalcTable = sys_AllocMemory(heapID, sizeof(_CALCTOOL_TABLE));
	MATH_CRC16CCITTInitTable(&_pCalcTable->crc_table);
}
...
{
    GF_ASSERT(_pCalcTable);
    sys_FreeMemoryEz(_pCalcTable);
    _pCalcTable=NULL;
}
