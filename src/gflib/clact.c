...
u32 CLACT_GetUserAttrAnimSeq( CONST_CLACT_WORK_PTR act, u32 seqidx )
{
	...
	if( pExBankData ){
		...
		if( pExSeqData ){
			return NNS_G2dGetUserExAnimSeqAttrValue( pExSeqData );
		}
	}
	return CLACT_USERATTR_NONE;
}

//----------------------------------------------------------------------------
/**
 *	@brief	今のシーケンスのアニメーションシーケンス拡張アトリビュート取得
 *	@retval	ユーザー拡張アトリビュートデータ
 *	@retval	CLACT_USERATTR_NONE		データなし
 */
//-----------------------------------------------------------------------------
...
{
	u32 seq;
	seq = CLACT_AnmGet( act );
	return CLACT_GetUserAttrAnimSeq( act, seq );
}
...
u32 CLACT_GetUserAttrAnimFrame( CONST_CLACT_WORK_PTR act, u32 seqidx, u32 frameidx )
{
	...
	if( pExBankData ){
		pExSeqData = NNS_G2dGetUserExAnimSequenceAttr( pExBankData, seqidx );
		if( pExSeqData ){
			pExFrameData = NNS_G2dGetUserExAnimFrameAttr( pExSeqData, frameidx );
			if( pExFrameData ){
				return NNS_G2dGetUserExAnimFrmAttrValue( pExFrameData );
			}
		}
	}
	return CLACT_USERATTR_NONE;
...
u32 CLACT_GetUserAttrNowAnimFrame( CONST_CLACT_WORK_PTR act )
{
	...
	u32 frame;
	seq = CLACT_AnmGet( act );
	frame = CLACT_AnmFrameGet( act );
	return CLACT_GetUserAttrAnimFrame( act, seq, frame );
}
