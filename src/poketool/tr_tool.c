	...
	case DATATYPE_NORMAL:
		{
			...
			for(i=0;i<bp->trainer_data[num].poke_count;i++){
				...
				PokeParaSet(pp,monsno,ptn[i].level,pow,RND_SET,rnd,ID_NO_RARE,0);
				TrainerCBSet(ptn[i].custom,pp,heapID);
				PokeParaPut(pp,ID_PARA_form_no,&form_no);
	...
	case DATATYPE_WAZA:
		{
			...
			for(i=0;i<bp->trainer_data[num].poke_count;i++){
				...
				PokeParaSet(pp,monsno,ptw[i].level,pow,RND_SET,rnd,ID_NO_RARE,0);
				for(j=0;j<4;j++){
					PokeWazaSetPos(pp,ptw[i].waza[j],j);
				}
				TrainerCBSet(ptw[i].custom,pp,heapID);
				PokeParaPut(pp,ID_PARA_form_no,&form_no);
				PokeParty_Add(bp->poke_party[num],pp);
			}
		}
	...
	case DATATYPE_ITEM:
		{
			...
			for(i=0;i<bp->trainer_data[num].poke_count;i++){
				...
				PokeParaSet(pp,monsno,pti[i].level,pow,RND_SET,rnd,ID_NO_RARE,0);
				PokeParaPut(pp,ID_PARA_item,(u8 *)&pti[i].itemno);
				TrainerCBSet(pti[i].custom,pp,heapID);
				PokeParaPut(pp,ID_PARA_form_no,&form_no);
				PokeParty_Add(bp->poke_party[num],pp);
			}
		}
	...
	case DATATYPE_MULTI:
		{
			...
			for(i=0;i<bp->trainer_data[num].poke_count;i++){
				...
				PokeParaSet(pp,monsno,ptm[i].level,pow,RND_SET,rnd,ID_NO_RARE,0);
				PokeParaPut(pp,ID_PARA_item,(u8 *)&ptm[i].itemno);
				for(j=0;j<4;j++){
					PokeWazaSetPos(pp,ptm[i].waza[j],j);
				}
				TrainerCBSet(ptm[i].custom,pp,heapID);
				PokeParaPut(pp,ID_PARA_form_no,&form_no);
				PokeParty_Add(bp->poke_party[num],pp);
			}
		}
