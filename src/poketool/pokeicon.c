...
#define	POKEICON_TAMAGO		( 494 )		// タマゴ
#define	POKEICON_TAMAGO_MNF	( 495 )		// マナフィのタマゴ
#define	POKEICON_DEOKISISU	( 496 )		// デオキシス
#define	POKEICON_ANNOON		( 499 )		// アンノーン
#define	POKEICON_MINOMUTTI	( 527 )		// ミノムッチ
#define	POKEICON_MINOMESU	( 529 )		// ミノメス
#define	POKEICON_SIIUSI		( 531 )		// シーウシ
#define	POKEICON_SIIDORUGO	( 532 )		// シードルゴ
#define	POKEICON_GIRATHINA	( 533 )		// ギラティナ
#define	POKEICON_SHEIMI		( 534 )		// シェイミ
#define	POKEICON_ROTOMU		( 535 )		// ロトム
...
u32 PokeIconCgxArcIndexGetByMonsNumber( u32 mons, u32 egg, u32 pat )
{
	...
	pat = PokeFuseiFormNoCheck(mons, pat);
	...
	if( pat != 0 ){
		...
		if( mons == MONSNO_KIMAIRAN ){
			return ( NARC_poke_icon_poke_icon_509_01_NCGR + pat - 1 );
		}
		if( mons == MONSNO_EURISU ){
			return ( NARC_poke_icon_poke_icon_516_01_NCGR + pat - 1 );
		}
		if( mons == MONSNO_PURAZUMA ){
			return ( NARC_poke_icon_poke_icon_519_01_NCGR + pat - 1 );
		}
	}
...
const u8 PokeIconPalNumGet( u32 mons, u32 form, u32 egg )
{
	if( egg == 1 ){
		...
		if( mons == MONSNO_DEOKISISU ){
			...
			mons = POKEICON_GIRATHINA + form - 1;
		}else if( mons == MONSNO_EURISU ){
			mons = POKEICON_SHEIMI + form - 1;
		}else if( mons == MONSNO_PURAZUMA ){
			mons = POKEICON_ROTOMU + form - 1;
		}
	}
