/*---------------------------------------------------------------------------*
  ...
  OS_Printf などを iris/os/printf.h へ分離しそれをインクルードするよう変更

  Revision 1.8  2003/11/17 00:00:42  yada
  OS_Printf()、 OS_Warning()、 OS_Panic() のプロトタイプ宣言追加

  Revision 1.7  2003/11/14 07:54:35  yada
  #ifdef NITRO_DEBUG   →  #ifdef SDK_DEBUG に修正。
  ...
  SDK_WARNING(), SDK_ASSERT(), OS_Halt(), OS_Panic() 定義追加

  Revision 1.3  2003/11/14 04:31:10  yada
  SDK_WARNING(x,msg) を追加。
  (中身はまだない)

  Revision 1.2  2003/11/11 08:56:31  yada
  SDK_ASSERTMSG(x,msg) を追加
