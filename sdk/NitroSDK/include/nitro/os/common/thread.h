/*---------------------------------------------------------------------------*
  ...
  OS_KillThreadWithPriority の追加

  Revision 1.66  2005/07/05 09:43:13  yasu
  version.h を参照し SDK のバージョンによって define を切り替える
  ...
  OS_CreateThreadQueue() → OS_InitThreadQueue() と改名。
  OS_GetMaxThreadId() を inline ではないようにした。

  Revision 1.11  2003/12/11 11:37:51  yada
  OS_GetThread, OS_GetThreadId, OS_GetMaxThreadId, OS_GetThreadContext, OS_GetThreadState を追加。
  ...
  OSThreadQueue の最低サイズを 16bit にした
