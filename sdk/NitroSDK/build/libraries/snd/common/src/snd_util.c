/*---------------------------------------------------------------------------*
  ...
  $Log: snd_util.c,v $
  Revision 1.9  2006/01/18 02:12:29  kitase_hirotake
  do-indent

  Revision 1.8  2005/09/22 06:53:18  ida
  scale0の時のデシベル値が十分に小さくなかった問題修正
  ...
  Revision 1.7  2004/07/16 06:42:49  ida
  fix Project name

  Revision 1.6  2004/07/05 06:31:30  ida
  SND_CalcRandom become ARM7 only

  Revision 1.5  2004/06/23 06:29:35  ida
  remove SND_CopyMemory16

  Revision 1.4  2004/05/17 04:05:07  ida
  fix SND_SinIdx wrong return type

  Revision 1.3  2004/05/07 07:36:11  ida
  add SND_CalcRandom

  Revision 1.2  2004/05/05 23:59:20  ida
  add comments

  Revision 1.1  2004/03/26 06:25:52  ida
  create snd_drv

  Revision 1.5  2004/03/11 01:51:38  ida
  SND_CopyMemory -> SND_CopyMemory16

  Revision 1.4  2004/03/10 06:29:44  ida
  SND_Snd -> SND_

  Revision 1.3  2004/03/08 05:40:27  ida
  rename files

  Revision 1.2  2004/02/25 05:22:24  ida
  SVC_SND_ -> SVC_

  Revision 1.1  2004/02/25 04:25:56  ida
  snd_util.c ARM7 -> common

  Revision 1.12  2004/02/13 00:18:01  ida
  header moved

  Revision 1.11  2004/02/10 02:04:07  ida
  Add comments

  Revision 1.10  2004/02/10 01:21:52  ida
  remove SNDChannelVolume

  Revision 1.9  2004/02/10 01:08:25  ida
  u32 -> int

  Revision 1.8  2004/02/06 00:00:56  ida
  remove sin table

  Revision 1.7  2004/02/05 07:09:02  yasu
  change SDK prefix iris -> nitro

  Revision 1.6  2004/02/03 00:56:25  ida
  reduced sine table size
 update decibel table

  Revision 1.5  2004/01/30 08:05:57  ida
  Changed table

  Revision 1.4  2004/01/30 07:03:13  ida
  Changed volume shift setting

  Revision 1.3  2004/01/14 04:09:31  ida
  Modified OscTable

  Revision 1.2  2004/01/13 06:09:33  ida
  VolumeTable variable BUG fix

  Revision 1.1  2004/01/13 02:06:58  ida
  Initial version
...
/*---------------------------------------------------------------------------*
  ...
  Description:  元となるタイマー値に対して、ピッチ変換した値を求める

  Arguments:    base_timer : 元となるタイマー値
                pitch_     : 変換ピッチ

  Returns:      ピッチ変換後のタイマー値
 *---------------------------------------------------------------------------*/
u16 SND_CalcTimer(int base_timer, int pitch_)
{
    u64     timer;                     // 結果タイマー値
    int     shift = 0;                 // シフト量
    int     pitch = -pitch_;           // 逆数を求める
...
/*---------------------------------------------------------------------------*
  ...
  Description:  デシベル値からチャンネルレジスタにセットするボリューム値を求める

  Arguments:    dB : デシベル値

  Returns:      上位８ビット： SNDChannelDataShift
                下位８ビット： 0 - 127 scale factor
 *---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------*
  ...
  Description:  sin のテーブル値を参照します

  Arguments:    index - テーブルインデックス
