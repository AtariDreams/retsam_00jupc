...
#define	ID_PER_basic_hp		(0)		//基本ＨＰ
#define	ID_PER_basic_pow	(1)		//基本攻撃力
#define	ID_PER_basic_def	(2)		//基本防御力
#define	ID_PER_basic_agi	(3)		//基本素早さ
#define	ID_PER_basic_spepow	(4)		//基本特殊攻撃力
#define	ID_PER_basic_spedef	(5)		//基本特殊防御力
#define	ID_PER_type1		(6)		//属性１
#define	ID_PER_type2		(7)		//属性２
#define	ID_PER_get_rate		(8)		//捕獲率
#define	ID_PER_give_exp		(9)		//贈与経験値
#define	ID_PER_pains_hp		(10)	//贈与努力値ＨＰ
#define	ID_PER_pains_pow	(11)	//贈与努力値攻撃力
#define	ID_PER_pains_def	(12)	//贈与努力値防御力
#define	ID_PER_pains_agi	(13)	//贈与努力値素早さ
#define	ID_PER_pains_spepow	(14)	//贈与努力値特殊攻撃力
#define	ID_PER_pains_spedef	(15)	//贈与努力値特殊防御力
#define	ID_PER_item1		(16)	//アイテム１
#define	ID_PER_item2		(17)	//アイテム２
#define	ID_PER_sex			(18)	//性別ベクトル
#define	ID_PER_egg_birth	(19)	//タマゴの孵化歩数
#define	ID_PER_friend		(20)	//なつき度初期値
#define	ID_PER_grow			(21)	//成長曲線識別
#define	ID_PER_egg_group1	(22)	//こづくりグループ1
#define	ID_PER_egg_group2	(23)	//こづくりグループ2
#define	ID_PER_speabi1		(24)	//特殊能力１
#define	ID_PER_speabi2		(25)	//特殊能力２
#define	ID_PER_escape		(26)	//逃げる率
#define	ID_PER_color		(27)	//色（図鑑で使用）
#define	ID_PER_reverse		(28)	//反転フラグ
#define	ID_PER_machine1		(29)	//技マシンフラグ１
#define	ID_PER_machine2		(30)	//技マシンフラグ２
#define	ID_PER_machine3		(31)	//技マシンフラグ３
#define	ID_PER_machine4		(32)	//技マシンフラグ４
...
///<アセンブラソースからも利用されるようになったので、define定義に変更
#if 0
...
extern	void	PokeGraArcDataGetPPP(SOFT_SPRITE_ARC *ssa,POKEMON_PASO_PARAM *ppp,u8 dir,int imc_mode);
extern	void	PokeGraArcDataGet(SOFT_SPRITE_ARC *ssa,u16 mons_no,u8 sex,u8 dir,u8 col,u8 form_no,u32 rnd);
extern	u8		PokeFuseiFormNoCheck(u16 mons_no,u8 form_no);
extern	void	PokeGraArcDataGetPP_IMC(SOFT_SPRITE_ARC *ssa,POKEMON_PARAM *pp,u8 dir);
extern	u8		PokeParaHeightGet(POKEMON_PARAM *pp,u8 dir);
extern	u8		PokeParaHeightGet_IMC(POKEMON_PARAM *pp,u8 dir);
extern	u8		PokePasoParaHeightGet(POKEMON_PASO_PARAM *ppp,u8 dir, int imc_mode);
...
extern	CATS_ACT_PTR	TrCLACTSet(CATS_SYS_PTR csp,CATS_RES_PTR crp,PALETTE_FADE_PTR pfd,int pos_x,int pos_y,int trtype,int dir,int id,int heapID);
...
extern	void	PokeAnmDataSetDP(SOFT_SPRITE_ANIME_DP *ssanm,u16 mons_no);
...
extern	void	PokeWazaSetPosPPCountClear(POKEMON_PARAM *pp,u16 wazano,u8 pos);
...
extern int	PokeParaGirathinaFormChange(POKEMON_PARAM *pp);
extern int	PokePasoParaGirathinaFormChange(POKEMON_PASO_PARAM *ppp);
extern void PokeParaGirathinaOriginFormForceSet(POKEMON_PARAM *pp);
extern void PokePartyGirathinaFormUpdate(POKEPARTY *party, int force_change);

extern BOOL PokeParaRotomuFormChange(POKEMON_PARAM *pp, int new_form_no, int del_waza_pos);

extern void	PokeParaSheimiFormChange(POKEMON_PARAM *pp, int form_no);
extern void	PokePasoParaSheimiFormChange(POKEMON_PASO_PARAM *ppp, int form_no);
extern void PokePartySheimiNormalFormChange(POKEPARTY *ppt);
extern BOOL PokePartySheimiNormalFormChange_TimeUpdate(POKEPARTY *ppt, int min_diff, const RTCTime * now);
...
extern void	PokeVoicePlaySet(PERAPVOICE *p_voice,
	int voice,u16 monsno,int form_no,int pan,int vol,int flag,int heapID);
extern void	PokeVoiceWaitPlaySet(PERAPVOICE *p_voice,
	int voice,u16 monsno,int form_no,int pan,int vol,int flag,int heapID, u8 wait);
...
extern void PokeEditData_AnimeSet( ARCHANDLE* handle, SOFT_SPRITE_ANIME* ssanm, u16 mons_no, u16 client_type );
extern void PokeEditData_PrgAnimeSet( ARCHANDLE* handle, POKE_ANM_SYS_PTR pasp, SOFT_SPRITE* ss, u16 mons_no, int dir, int reverse, int index );
extern void PokeEditData_VoiceWaitSet( ARCHANDLE* handle, u8* wait, u16 mons_no, u16 client_type );
extern void PokeEditData_AnmPackSet( ARCHANDLE* handle, POKE_ANIME_DATA* pad, u16 mons_no, u16 client_type );
extern void PokeEditData_HeightSet( ARCHANDLE* handle, s8* height, u16 mons_no );
extern void PokeEditData_ShadowOfsXSet( ARCHANDLE* handle, s8* ofs, u16 mons_no );
extern void PokeEditData_ShadowSizeSet( ARCHANDLE* handle, u8* size, u16 mons_no );


///< カスタムボール
extern BOOL TrainerCBSet( int index, POKEMON_PARAM* pp, int heap );

//戦闘録画用ポケモンパラメータ
///<戦闘録画用ポケモンパラメータの構造体
typedef struct rec_pokepara REC_POKEPARA;
extern void PokePara_to_RecPokePara(POKEMON_PARAM *pp, REC_POKEPARA *rec);
extern void RecPokePara_to_PokePara(REC_POKEPARA *rec, POKEMON_PARAM *pp);

extern BOOL GrashideaFeasibleCheck(POKEMON_PARAM *pp);

extern BOOL PokeVoicePlay_PP(POKEMON_PARAM *pp);
extern BOOL PokeVoicePlayEx_PP(int ptn, POKEMON_PARAM *pp, int pan, int vol, int heap_id);
extern BOOL PokeVoicePlayWaitPlay_PP(POKEMON_PARAM *pp, u8 wait);
extern void PokeVoicePlayWaitPlayEx_PP(int ptn, POKEMON_PARAM *pp, int pan, int vol, int heap_id, u8 wait);
