...
//	バグ対処
//--------------------------------------------------------------
///通信BTS#0467の対処の為のウェイト。各部門移行時、終了命令受信後の同期取りをDPでは行えていなかったので(main_endがTRUEになったら返事をTransしないで次に行く)、通信の遅延状況によっては次の部門の最初の通信と同時に受信する場合があるため、一定のウェイトを入れて、発生する可能性を下げる
#define CON_DP_SIO_WAIT			(120)
...
typedef struct{
	...
	u8 sio_dp_num;					///<通信対戦時、参加しているDPの数 ※プラチナで追加
...
//-- con_battle.c --//
extern BOOL ContestPokeFlipCheck(u32 monsno, u32 form_no);
