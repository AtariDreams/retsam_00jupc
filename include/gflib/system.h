...
#define CRC_LOADCHECK  (1) //CRCによる検査を有効にする場合 1 無効にする場合 0
...
enum {  //ソフトウエアリセットをしたくない状態の定義
  ...
  SOFTRESET_TYPE_VIDEO = 0x08, // 対戦録画時
};

///通信エラーにしない場合のマジックキー
#define SYS_DONT_SIO_ERROR_KEY		(0x2f93a1bc)
...
//	appFunc pSubProc;		// アプリケーション呼び出し関数ポインタ
	...
	u32		*pDontSioError;		//通信エラーにしない事を示すフラグへのポインタ
}SystemArea;
...
// キーリピートの速度とウェイトをセット
#define SYS_KEYREPEAT_SPEED_DEF	( 4 )
#define SYS_KEYREPEAT_WAIT_DEF	( 8 )
...
GLOBAL void sys_SioErrorNG_PtrSet(int heap_id);
GLOBAL void sys_SioErrorNG_PtrFree(void);
GLOBAL BOOL sys_SioErrorNG_Get(void);
